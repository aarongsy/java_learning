...
<%
	...
	String relaship=StringHelper.null2String(request.getParameter("relaship"));
	String school=StringHelper.null2String(request.getParameter("school"));
	...
	if (action.equals("toSAP")){	//修改Kids抛SAP
		...
		if( "1".equals(chgflag) ) { //add
			...
			try {
				...
				functionName = "ZHR_PA002_KIDS_INS_MY"; //PA002:员工个人信息-Kids新增
				function = null;
				try {
				...
				jcotable.setValue("FANAM", name);
				jcotable.setValue("FGBDT", birthday);
				jcotable.setValue("FASEX", gender);
				jcotable.setValue("KDSVH", relaship);
				jcotable.setValue("FASAR", school);
		...
		if( "2".equals(chgflag) ) {	
			...
			try {
				...
				functionName = "ZHR_PA002_KIDS_MOD_MY"; //PA002:员工个人信息-Kids修改
				function = null;
				try {
					function = SapConnector_EN.getRfcFunction(functionName);
				} catch (Exception e) {
