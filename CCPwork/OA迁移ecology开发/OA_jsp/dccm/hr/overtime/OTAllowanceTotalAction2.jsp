...
<%
	...
	if (action.equals("batchtoSAP")) {	//批量抛SAP
		try {			
			...
			String allowsql = "select * from uf_dmhr_allowtotal where requestid='"+requestid+"' order by empsapid asc,sno asc";
			//String allsapcode = "";
			...
			if ( allowlist.size()>0 )  {
				try {
					...
					for ( int i = 0; i < rejcotable.getNumRows(); i++ ) {
						...
						String upsql = "update uf_dmhr_allowtotal  set msgty='"+resapmsgtype+"',message='"+resapmsg+"' where requestid='"+requestid+"' and empsapid='"+reempsapid+"' ";
						...
						upsql = "update uf_dmhr_allowdetail set msgty='"+resapmsgtype+"',message='"+resapmsg+"' where requestid='"+requestid+"' and empsapid='"+reempsapid+"'";
						baseJdbcDao.update(upsql);
	...
	if (action.equals("singletoSAP")) { //单个抛SAP
		...
		try {
			...
			System.out.println("~~~~~~~~~~~~~~~~OTAllowance Single~~~~~~~~");
			if ( "I".equals(sapmsgtype) ){
				...
				String upsql = "update uf_dmhr_allowtotal  set msgty='"+sapmsgtype+"',message='"+sapmsg+"' where requestid='"+requestid+"' and empsapid='"+empsapid+"' and sapcode='"+sapcode+"'";
				System.out.println("~~~~~~~~~~~~~~~~OTAllowance Single~~~~~~~~upsql:"+upsql);
				baseJdbcDao.update(upsql);
				...
				upsql = "update uf_dmhr_allowdetail  set msgty='"+sapmsgtype+"',message='"+sapmsg+"' where requestid='"+requestid+"' and empsapid='"+empsapid+"' and allowcode='"+sapcode+"'";
				System.out.println("~~~~~~~~~~~~~~~~OTAllowance Single~~~~~~~~upsql:"+upsql);
				baseJdbcDao.update(upsql);	
				...
				if ( sublist.size()>0 ) {
					for ( int k=0;k<sublist.size();k++ ) {
						...
						String subupsql = "update uf_dmhr_otappsub set msgty='"+msgty+"',message='"+message+"' where requestid='"+otapp+"' and sapcode='"+allowcode+"'";
						baseJdbcDao.update(subupsql);
						System.out.println("~~~~~~~~~~~~~~~~OTAllowance Single~~~~~~~~subupsql:"+subupsql);
					}
				}
				...
				String upsql = "update uf_dmhr_allowtotal  set msgty='"+sapmsgtype+"',message='"+sapmsg+"' where requestid='"+requestid+"' and empsapid='"+empsapid+"' and sapcode='"+sapcode+"'";
				baseJdbcDao.update(upsql);
				System.out.println("2~~~~~~~~~~~~~~~~OTAllowance Single~~~~~~~~upsql:"+upsql);
				//upsql = "update uf_dmhr_otappsub set msgty='"+sapmsgtype+"',message='"+sapmsg+"' where  requestid in (select otapp from  uf_dmhr_allowdetail where  requestid='"+requestid+"' and empsapid='"+empsapid+"'  and NVL(msgty,'0')!='I' and allowcode='"+sapcode+"' and NVL(msgty,'0')!='I'";
				//baseJdbcDao.update(upsql);
				//upsql = "update uf_dmhr_allowdetail  set msgty='"+sapmsgtype+"',message='"+sapmsg+"' where requestid='"+requestid+"' and empsapid='"+empsapid+"'  and NVL(msgty,'0')!='I' and allowcode='"+sapcode+"'";
				upsql = "update uf_dmhr_allowdetail  set msgty='"+sapmsgtype+"',message='"+sapmsg+"' where requestid='"+requestid+"' and empsapid='"+empsapid+"' and allowcode='"+sapcode+"'";
				...
				if ( sublist.size()>0 ) {
					for ( int k=0;k<sublist.size();k++ ) {
						...
						String subupsql = "update uf_dmhr_otappsub set msgty='"+msgty+"',message='"+message+"' where requestid='"+otapp+"' and sapcode='"+allowcode+"'";
						baseJdbcDao.update(subupsql);
						System.out.println("2~~~~~~~~~~~~~~~~OTAllowance Single~~~~~~~~subupsql:"+subupsql);
					}
				}				
				jsonObject.put("info",sapmsg);
				jsonObject.put("sapflag",sapmsgtype);	
