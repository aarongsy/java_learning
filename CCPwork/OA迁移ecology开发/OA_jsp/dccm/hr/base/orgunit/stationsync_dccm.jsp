...
<%
String action=request.getContextPath()+"/ServiceAction/com.eweaver.app.dccm.dmhr.base.orgunit.DMHR_StationSyncAction?action=search";
%>
<%
	...
	//if(rolist.size()>0){
		pagemenustr +=  "addBtn(tb,'Synchronize the position selected','T','application_start',function(){syncSigSta()});";	//Synchronize the position selected 同步选中的岗位
	//}
         ...
         <script language="javascript">
          Ext.LoadMask.prototype.msg='<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>';//加载中,请稍后...
			...
			var cm = new Ext.grid.ColumnModel([sm,
			         {header:'Station Name',dataIndex:'stext',hidden:false,width:30,sortable:true},
			         {header:'Start at',dataIndex:'begda',hidden:false,width:30,sortable:true},
			         {header:'End at',dataIndex:'endda',hidden:false,width:30,sortable:true},
			         ...
			         {header:'ChangerOrg',dataIndex:'chgor',hidden:false,width:30,sortable:true},
			         {header:'Number',dataIndex:'number',hidden:false,width:30,sortable:true}
			         ]);       
...
sm.on('rowselect',function(selMdl,rowIndex,rec ){
		var reqstr = rec.get('stext')+'~!~'+rec.get('endda')+'~!~'+rec.get('orgeh')+'~!~'+rec.get('seqnr')+'~!~'+rec.get('chgor')+'~!~'+rec.get('number');
        for(var i=0;i<selectedstr.length;i++){
			if(reqstr == selectedstr[i]){
				return;
			}
		}		
		selectedstr.push(reqstr)
    }
            );
...
<script language="javascript">
   ...
   var myMask1 = new Ext.LoadMask(document.body, {msg:'<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>',removeMask:true});//系统加载中，请稍后...... 请稍等,数据加载中...
   ...
   function syncSigSta(){
		if(<%=rolist.size() %>==0) {
			...
			myMask1.show();
			 syncDo('<%=request.getContextPath()%>/ServiceAction/com.eweaver.app.dccm.dmhr.base.orgunit.DMHR_StationSyncAction','synstabysingle');
		}
   }   
   function syncDo(surl,action){
		if(selectedstr==''){
			alert('Please select the station at first!');
			myMask1.hide();
			return;
		}else{
			try{
