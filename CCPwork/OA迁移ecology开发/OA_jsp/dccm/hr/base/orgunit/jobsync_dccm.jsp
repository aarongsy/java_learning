...
<%
	...
	//if(rolist.size()>0){
		pagemenustr +=  "addBtn(tb,'Synchronize the job selected','T','application_start',function(){syncSigSta()});";	//Synchronize the position selected 同步选中的岗位
	//}
         ...
         <script language="javascript">
          Ext.LoadMask.prototype.msg='<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>';//加载中,请稍后...
			...
			var cm = new Ext.grid.ColumnModel([sm,
			         {header:'Job Name',dataIndex:'stext',hidden:false,width:30,sortable:true},
...
<script language="javascript">
   ...
   function syncDo(surl,action){
		if(selectedstr==''){
			alert('Please select the job at first!');
			myMask1.hide();
			return;
		}else{
			try{
