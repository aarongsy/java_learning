...
<%
	...
	String sql = "select sno,subject,money,sjjs,payitem,paydate,payfreeze,paytype,currency,paymoney,taxcaode,costcenter,purorderid,purorderitem,banktype,text1,iskmsupplier from uf_dmph_dlineiteminfo where requestid = '"+requestid+"' order by sno asc ";
	...
	if(list1.size()>0)
	{
		for(int i=0;i<list1.size();i++)
		{
			...
			String purorderid = StringHelper.null2String(map1.get("purorderid"));//采购订单号
			
			String purorderitem = StringHelper.null2String(map1.get("purorderitem"));//采购订单项次
			purorderitem = purorderitem.replaceFirst("^0*", "");//去掉项次开头的0 
			...
			retTable1.setValue("PAY_REF", payfreeze);//付款冻结
			...
			retTable1.setValue("PO_NO", purorderid);//采购订单号
			retTable1.setValue("PO_ITEM", purorderitem);//采购订单行项目
	...
	sql = "select sno,custsuppflag,custsuppcode,ledgerflag,clearreceiptid,fiscalyear,clearreceiptitem,surplusmoney,cleartext from uf_dmph_uncleariinfo   where requestid = '"+requestid+"' order by sno asc";
	...
	String insql = "update uf_dmph_importchargemain  set receiptid = '"+AC_DOC_NO+"',succflag = '"+FLAG+"',errorid = '"+ERR_MSG+"' where requestid = '"+requestid+"'";
