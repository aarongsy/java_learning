	...
	String sql="";
		...
		if(list.size()>0){
			for(int k=0;k<list.size();k++)
			{
				...
				sql1="select nvl(sum(clearmoney),0) hyfee from uf_tr_exfeeclearsub a left join uf_tr_exfeeclearmain  c on a.requestid=c.requestid where 0=(select isdelete from requestbase where id=a.requestid) and  1=(select isfinished from requestbase where id=a.requestid) and  (c.isvalid is null or c.isvalid='40288098276fc2120127704884290210') and a.exlistid='"+expno+"' and a.feetype like '%海运费%' and a.payobject='"+hycode+"'"; 
				...
				sql1="select nvl(sum(clearmoney),0) bgfee from uf_tr_exfeeclearsub a left join uf_tr_exfeeclearmain  c on a.requestid=c.requestid where 0=(select isdelete from requestbase where id=a.requestid) and  1=(select isfinished from requestbase where id=a.requestid) and  (c.isvalid is null or c.isvalid='40288098276fc2120127704884290210') and a.exlistid='"+expno+"' and a.feetype like '%包干费%' and a.payobject='"+hycode+"'";  
                list1 = baseJdbc.executeSqlForList(sql1);
				if(list1.size()>0)
				{
					map1 = (Map)list1.get(0);
				...
				sql1="select nvl(sum(clearmoney),0) fjfee from uf_tr_exfeeclearsub a left join uf_tr_exfeeclearmain  c on a.requestid=c.requestid where 0=(select isdelete from requestbase where id=a.requestid) and  1=(select isfinished from requestbase where id=a.requestid) and  (c.isvalid is null or c.isvalid='40288098276fc2120127704884290210') and a.exlistid='"+expno+"' and a.feetype not like '%海运费%'and a.feetype not like '%保险%' and a.estcurrency='USD' and a.payobject='"+hycode+"'";
                list1 = baseJdbc.executeSqlForList(sql1);
				if(list1.size()>0)
				{
					map1 = (Map)list1.get(0);
