...
sql = sql +") v where (select count(*) from uf_tr_feeclearsub c left join uf_tr_feeclearmain  d on c.requestid=d.requestid where c.kjrequest=v.kjrequest and c.imgoodsid=v.imgoodsid and c.imgoodsitem=v.imgoodsitem and c.feetype=v.feetype and c.payobject=v.paycode and c.estcurrency=v.currency and c.requestid <>'"+requestid+"' and 1<>(select isdelete from requestbase where id = c.requestid) and (d.isvalid is null or d.isvalid='40288098276fc2120127704884290210')) =0 order by v.imgoodsid asc,v.feetype asc ,v.imgoodsitem asc";
...
if(sublist.size()>0){
	for(int k=0,sizek=sublist.size();k<sizek;k++){
		...
		java.math.BigDecimal b1=new java.math.BigDecimal(rate);
		java.math.BigDecimal b2=new java.math.BigDecimal(Double.toString(qzmon));
        qzrmbmon=b1.multiply(b2).doubleValue();
		java.math.BigDecimal b3 = b1.multiply(b2);
		qzrmbmon = b3.setScale(2, java.math.BigDecimal.ROUND_HALF_UP).doubleValue();	//2017-01-03 xxy add 解决JKQZ20161200106 暂估本位币347.475 更改为347.48
		...
		insql = insql +"'"+qzmon+"',"+qzrmbmon+",'"+df.format(ywhbce)+"','"+df.format(bwbce)+"','"+materialid+"','"+costcenter+"','"+assetid+"','"+innerorderid+"','"+genledger+"','"+purchaseid+"','"+purchaseitem+"','"+kjrequest+"')";
	...
	%>
		...
		<TD   class="td2"  align=center><input type="checkbox" id="<%="checkbox_"+no %>" name="no"><span id="<%="no_"+no+"span" %>"><%=no %></span></TD>
		...
		<TD   class="td2"  align=center><input type="hidden" id="<%="estmoney_"+no %>" value="<%=amount %>"><span id="<%="estmoney_"+no+"span" %>"><%=amount %></span></TD>
		
		<TD   class="td2"  align=center><%=rate %></TD>

		<TD   class="td2"  align=center><input type="hidden" id="<%="estamount_"+no %>" value="<%=rmbamount %>"><span id="<%="estamount_"+no+"span" %>"><%=rmbamount %></span></TD>

		<TD   class="td2"  align=center><input type="text" id="<%="qzmon_"+no %>" value="<%=qzmon %>" onblur="changeqzje  ('<%=no%>')"></TD>
		<TD   class="td2"  align=center><input type="hidden" id="<%="qzrmbmon_"+no %>" value="<%=qzrmbmon%>"><span id="<%="qzrmbmon_"+no+"span" %>"><%=df.format(qzrmbmon) %></span></TD>

		<TD   class="td2"  align=center><input type=hidden id="<%="ywhbce_"+no %>" value="<%=df.format(ywhbce)%>"><span id="<%="ywhbce_"+no+"span" %>" ><%=df.format(ywhbce) %></span></TD>

		<TD   class="td2"  align=center><input type=hidden id="<%="bwbce_"+no %>" value="<%=df.format(bwbce) %>"><span id="<%="bwbce_"+no+"span" %>"><%=df.format(bwbce) %></span></TD>
