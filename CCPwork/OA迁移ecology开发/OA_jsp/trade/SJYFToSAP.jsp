...
<%
	...
	/*	JCoFunction function = null;
		...
		*/
		String sql = "select no,materialno,money,orderno,item,notesid from uf_tr_taxsub where requestid='"+requestid+"' order by no";
		List list2 = baseJdbc.executeSqlForList(sql);
		if(list2.size()>0){
			for(int m=0;m<list2.size();m++)
			{
				...
				String money = StringHelper.null2String(map2.get("money"));
				String orderno = StringHelper.null2String(map2.get("orderno"));
				String item = StringHelper.null2String(map2.get("item"));
				...
				String insql = "update uf_tr_taxsub  set sapreceiptid = '"+cwpz+"',msgtype='"+msgty+"',msg='"+message+"' where requestid = '"+requestid+"' and no = '"+sno+"'";
		...
		//返回值
		sql = "select no,sapreceiptid from uf_tr_taxsub where requestid = '"+requestid+"' order by no asc";
