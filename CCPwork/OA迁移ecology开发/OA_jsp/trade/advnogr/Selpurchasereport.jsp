          ...
          Ext.LoadMask.prototype.msg='<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>';//加载中,请稍后...
             ...
             store = new Ext.data.Store({
                 ...
                 reader: new Ext.data.JsonReader({
                     ...
                     fields: ['pono','poitem','comcode','company','ordertype','ordertypedes','supcode','supname','currency','rate','wlh','shorttxt','quantity','price','amount','kmfp','taxcode','taxprice','totalamt','totaltaxamt']
                 })
			...
			var cm = new Ext.grid.ColumnModel([sm,			                                   
			                                   ...
			                                   {header:'采购单项次/行项次',dataIndex:'poitem',hidden:false,width:30,sortable:true},
			                                   ...
			                                   {header:'采购订单类型',dataIndex:'ordertype',hidden:true,width:30,sortable:true},
			                                   ...
			                                   {header:'税率    ',dataIndex:'rate',hidden:false,width:30,sortable:true},
			                                   {header:'物料号  ',dataIndex:'wlh',hidden:true,width:30,sortable:true},
			                                   ...
			                                   {header:'单价    ',dataIndex:'price',hidden:true,width:30,sortable:true},
											   ...
											   {header:'税码',dataIndex:'taxcode',hidden:false,width:30,sortable:true},											   
											   {header:'含税单价',dataIndex:'taxprice',hidden:true,width:30,sortable:true},
...
sm.on('rowselect',function(selMdl,rowIndex,rec ){
        var reqid=rec.get('pono')+'^@^'+rec.get('poitem')+'^@^'+rec.get('comcode')+'^@^'+rec.get('company')+'^@^'+rec.get('supcode')+'^@^'+rec.get('supname')+'^@^'+rec.get('currency')+'^@^'+rec.get('shorttxt')+'^@^'+rec.get('quantity')+'^@^'+rec.get('amount')+'^@^'+rec.get('kmfp')+'^@^'+rec.get('taxcode')+'^@^'+rec.get('taxprice');
