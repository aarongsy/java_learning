...
<%
	...
	if (action.equals("getData")){
		...
		BaseJdbcDao baseJdbc=(BaseJdbcDao)BaseContext.getBean("baseJdbcDao");
			...
			//更新数据库中对应的行项信息
			String upsql="delete uf_wkpayfeeinfo  where requestid='"+requestid+"'";
			System.out.println(upsql);
			baseJdbc.update(upsql);
			upsql = "insert into uf_wkpayfeeinfo   (id,requestid,comcode,creno,year,paymon,cur,subject,payterm,payway,paydate,frozen,bank,text) values((select sys_guid() from dual),'"+requestid+"','"+BUKRS+"',"+BELNR+",'"+GJAHR+"','"+WRBTR+"','"+PSWSL+"','"+HKONT+"','"+ZTERM+"','"+ZLSCH+"','"+ZFBDT+"','"+ZLSPR+"','"+BVTYP+"','"+SGTXT+"')";
			baseJdbc.update(upsql);
			System.out.println(upsql);
			System.out.println(orderno);
			System.out.println("fusdhfusdh");
					}
					else
					{
						newretTable.nextRow();//获取下一行数据
						String HKNT = newretTable.getString("HKONT");//科目
					...
					String upsql = "insert into uf_wkpayfeeinfo   (id,requestid,comcode,creno,year,paymon,cur,subject,payterm,payway,paydate,frozen,bank,text) values((select sys_guid() from dual),'"+requestid+"','"+BUKRS+"',"+BELNR+",'"+GJAHR+"','"+WRBTR+"','"+PSWSL+"','"+HKONT+"','"+ZTERM+"','"+ZLSCH+"','"+ZFBDT+"','"+ZLSPR+"','"+BVTYP+"','"+SGTXT+"')";
						baseJdbc.update(upsql);
						//System.out.println("fqwfwefuweuf");
						System.out.println(upsql);
					}
			/*while(!newretTable.isLastRow());//如果bu是最后一行
			{
				//newretTable.nextRow();//获取下一行数据
			...
			upsql = "insert into uf_wkpayfeeinfo   (id,requestid,comcode,creno,year,paymon,cur,subject,payterm,payway,paydate,frozen,bank,text) values((select sys_guid() from dual),'"+requestid+"','"+BUKRS+"',"+BELNR+",'"+GJAHR+"','"+WRBTR+"','"+PSWSL+"','"+HKONT+"','"+ZTERM+"','"+ZLSCH+"','"+ZFBDT+"','"+ZLSPR+"','"+BVTYP+"','"+SGTXT+"')";
				...
				//System.out.println("fqwfwefuweuf");
				System.out.println(upsql);
				System.out.println("412423142134");
			}*/
				}
			}
