...
<%
String delsql = "delete uf_oa_unrepair where requestid = '"+requestid+"'";
...
function.getImportParameterList().setValue("ZTYPE","");

//function.getImportParameterList().setValue("LIFNR",subject);//供应商简码
...
for(int i = 0;i<aa.length;i++)
{
	...
	System.out.println("数组为:"+bb);
	String pzh = bb[0];//凭证号
	System.out.println("pzh"+pzh);

	String year = bb[2];//会计年度
	System.out.println("year"+year);
	String subject = bb[1];//总账科目
	...
	//retTable.setValue("DTYPE", "K");//凭证类型
...
if(newretTable.getNumRows() >0)
{
	for(int j = 0;j<newretTable.getNumRows();j++)
	{
		...
		//{
			...
			int no = j+1;
			String insql = "insert into uf_oa_unrepair(id,requestid,subject,Specialid,pznum,year,unrepair,money,text,jznum)values('"+IDGernerator.getUnquieID()+"','"+requestid+"','"+HKONT+"','"+UMSKZ+"','"+BELNR+"','"+GJAHR+"','"+BUZEI+"','"+WRBTR+"','"+SGTXT+"','"+KOART+"')";
			System.out.println(insql);
			baseJdbc.update(insql);
	%>
			<TR id="<%="dataDetail_"+no %>">
			<TD   class="td2"  align=center><input type="checkbox" id="<%="checkbox_"+no %>" name="no"><%=no %></TD>
			<TD  class="td2"   align=center ><%="K" %></TD>
