    ...
    Ext.onReady(function() {
       ...
       <%}%>
        var replytpl = new Ext.XTemplate(
            '<p><b>{creator}</b> 发表于  {createtime}</p>',
            '{reply}'<%=replytplstr%>
        );
...
<input type="hidden" name="action" value="updateformbase">
         ...
         Ext.Ajax.request({
                     ...
                     success: function(res) {
                         if(res.responseText == 'noright')
                         {
                           ...
                           Ext.MessageBox.alert('','编辑权限的人才可以变更卡片数据！') ;
                         }else{
                              Ext.MessageBox.alert('','变更卡片数据成功！',function(){
...
function getFileSize(filepath){
 ...
 return null;
  ...
  window.onload=function(){
	//如果是收入合同登记
	if(document.getElementById("field_2c91a0302b278cea012b291c84260025span")){
		document.getElementsByName("field_2c91a0302b278cea012b291c84260024")[0].value='<%=currentuser.getId()%>';
		document.getElementById("field_2c91a0302b278cea012b291c84260024span").innerHTML=
			"<a href=javascript:onUrl('/humres/base/humresview.jsp?id=<%=currentuser.getId()%>','<%=currentuser.getObjname()%>','tab2c91a0302b278cea012b291c84260024')><%=currentuser.getObjname()%></a>";
		document.getElementsByName("field_2c91a0302b278cea012b291c84260025")[0].value=new Date().format('Y-m-d');
		document.getElementById("field_2c91a0302b278cea012b291c84260025span").innerHTML=new Date().format('Y-m-d');
		document.getElementById("field_2c91a0302d510d0c012d5e2e97c91d3e").disabled=true;

	}else if(document.getElementById("field_2c91a0302a8cef72012a8eb5a58403ffspan")){//如果是支出合同登记
		document.getElementsByName("field_2c91a0302b278cea012b291c84260024")[0].value='<%=currentuser.getId()%>';
		document.getElementById("field_2c91a0302b278cea012b291c84260024span").innerHTML=
			"<a href=javascript:onUrl('/humres/base/humresview.jsp?id=<%=currentuser.getId()%>','<%=currentuser.getObjname()%>','tab2c91a0302b278cea012b291c84260024')><%=currentuser.getObjname()%></a>";
		document.getElementsByName("field_2c91a0302a8cef72012a8eb5a58403ff")[0].value=new Date().format('Y-m-d');
		document.getElementById("field_2c91a0302a8cef72012a8eb5a58403ffspan").innerHTML=new Date().format('Y-m-d');
		document.getElementById("field_2c91a0302d510d0c012d5e2e97c91d3e").disabled=true;
        for(var i=0;i<=document.getElementsByName('fukuanstate').length;i++){
			document.getElementsByName('fukuanstate')[i].style.display='none';
		}
	}
  }
 </script>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               