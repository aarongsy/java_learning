...
String sql="select (select objname from orgunit where id=a.firstdepart)as employdepart,a.employno,(select objname from humres where id=a.employname) as employname,a.intofactorydate,(select objname from selectitem where id=a.education) as education,(select nametext from uf_profe where requestid=a.currecttitle) as currecttitle,a.currectrank,a.currectdate,a.yearscore,(select nametext from uf_profe where requestid=a.recommtitle) as recommtitle,a.recommrank,a.addrank,(select nametext from uf_profe where requestid=a.hrmodifytitle) as hrmodifyposition,a.hrmodifyrank,a.addrankaftermod,a.hrleadmodadvice from uf_hr_promochild a where a.requestid='"+requestid+"' and a.checkyear='"+year+"' order by a.employno";
 ...
 if(sublist.size()>0)
 {
	 for(int i=0;i<sublist.size();i++)
	 {
		 ...
		 String Hrmodifytitle=StringHelper.null2String(map.get("hrmodifyposition"));//人事主管修改升调职称
		 String Hrmodifyrank=StringHelper.null2String(map.get("hrmodifyrank"));//人事主管修改升调职级数
         ...
         %>
		  ...
		  <!--<button type=button  class=Browser id="button_title" name="button_title" onclick="javascript:getrefobj('<%="field_advicetitle"+i%>','<%="field_advicetitle" +i+"span"%>','40285a8f489c17ce0148bae540106e53','','','1');HrChangeRank('<%=i%>','<%=Employno%>','<%=Currectrank%>')">
		  </button>
		  <img src="/images/base/checkinput.gif" align=absMiddle>-->
		  		  <%
			if(Hrmodifytitle.equals(Recommtitle))
		 {
			  %>
		  <input type="hidden" id="<%="field_advicetitle"+i%>" name="<%="field_advicetitle"+i%>" value="" ><span id="<%="field_advicetitle" +i+"span"%>" name="<%="field_advicetitle" +i+"span"%>"></span></TD>
		 ...
		 else
		 {
          ...
          <TD class="td2" align=center><input type="hidden" id="<%="field_diradvice"+i%>" name="<%="field_diradvice"+i%>" value=""><span id="<%="field_diradvice"+i+"span"%>" name="<%="field_diradvice"+i+"span"%>"><%=Hrleadmodadvice%></span></TD><!-- 人事分管领导修改建议-->

          <TD class="td2" align=center><input type="text" id="<%="field_manadvice"+i%>" name="<%="field_manadvice"+i%>" value="" onchange="HrManagerAdvice('<%="field_manadvice"+i%>','<%=Employno%>')"><span id="<%="field_manadvice"+i+"span"%>" name="<%="field_manadvice"+i+"span"%>"></span></TD><!-- 总经理修改建议-->
          </TR>
<%	   }
    }
	else{%>
