...
if(sublist.size()>0){
	for(int k=0,sizek=sublist.size();k<sizek;k++){
		...
		if("get".equals(action)){	//获取时更新，显示时不更新
			//获取已付金额=已上抛成功的相同订单项次的本期请付金额
			String hasamount = ds.getValue(" select nvl(sum(a.acceptmon),0) hasamount from uf_yz_engindetail a,uf_yz_enginaccept b where a.requestid=b.requestid and b.flag='X'  and exists(select id from requestbase where id= b.requestid and isdelete=0 and isfinished=1) and a.orderno='"+orderno+"' and a.orderrow='"+orderrow+"' and b.requestid!='"+requestid+"'");
			paid = hasamount;
			upsql = "update uf_yz_engindetail set paid = '"+hasamount+"' where id = '"+id+"' and requestid='"+requestid+"'";
			baseJdbc.update(upsql);
		}
