...
function showInfo() {
	...
	var searchtype = trim(document.getElementById("searchtype").value);
	var comtype = trim(document.getElementById("comtype").value);
	//alert("carno="+carno+" ladingno="+ladingno+" loadingno="+loadingno+" curdate="+curdate +" isself="+isself +" printtype="+printtype+" searchtype="+searchtype+" comtype"+comtype);
	var flag = true;
	var errmsg = '';
	...
	if(flag){
		...
		Ext.Ajax.request({                                                                                                 
			 ...
			 params:{carno:carno,ladingno:ladingno,loadingno:loadingno,sdate:curdate,edate:curdate,isself:isself,printtype:printtype,searchtype:searchtype,comtype:comtype},
...
<body >
	...
	<table width="100%" border="1" cellspacing="0" cellpadding="0" class="header" style="border-right:0px solid black;border-top:0px solid black;">
		...
		<tr>
			...
			<td><SPAN class="STYLE4">精确查询/模糊查询</SPAN></td>
			<td><SPAN class="STYLE3"><select type="text" id="searchtype" name="searchtype" value="0"><option value="0" selected >精确查询</option><option value="1">模糊查询</option></select></span></td>
			<td><SPAN class="STYLE4">厂区别</SPAN></td>
			<td><SPAN class="STYLE3">
			<%
			if (adminflag==1) { %>
			<select type="text" id="comtype" name="comtype" value="4028804d2083a7ed012083ebb988005b"><option value="4028804d2083a7ed012083ebb988005b" selected >常熟厂</option><option value="40285a90488ba9d101488bbdeeb30008">长沙厂</option><option value="40285a904b3902dd014b5e085e7756a5">仪征厂</option><option value="">不限</option></select>
			<%
			} else { 
			%>
