...
<%
	...
	if(action.equals("getData"))
	{
		...
		if(FLAG.equals("X"))
		{
			...
			String DZEIT = newretTable.getString("DZEIT");//自制生产天数

			String ALTSL = newretTable.getString("ALTSL");//BOM展开方式
             
			String UNETO = newretTable.getString("UNETO");//不足交货允差

			String UEETO = newretTable.getString("UEETO");//过量交货允差
             
			String UEETK = newretTable.getString("UEETK");//无限制过量交货

			String WERKS = newretTable.getString("WERKS");//工厂

                        String LGORT = newretTable.getString("LGORT");//ckdd

			String LGPRO = newretTable.getString("LGPRO");//fhckd
 
			String LGFSB = newretTable.getString("LGFSB");//wbcgck

			String BZJG = newretTable.getString("BZJG");//bzjg

			String VTWEG = newretTable.getString("VTWEG");//分销渠道

			String upsql = "update uf_oa_cpbcpkgs set saperrinfo ='"+ERR_MSG+"',sapreturnflag='"+FLAG+"',sappl='"+DISLS+"',saplrzx='"+PRCTR+"',sapmz='"+BRGEW+"',sapwlhms='"+MAKTX+"',sapsrz='"+BSTRF+"' ";
			...
			upsql = upsql +",sapzzsc='"+DZEIT+"',sapbom='"+ALTSL+"',sapbzjh='"+UNETO+"',sapgljh= '"+UEETO+"',sapwxzgl='"+UEETK+"',sapfxqd='"+VTWEG+"'";
			upsql = upsql +",sapaqkc='"+EISBE+"',sapcpdl= '"+SPART+"',sapxszz='"+VKORG+"',sapwbcgccd='"+LGFSB+"',sapccdd='"+LGORT+"',sapscccdd='"+LGPRO+"',sapbzjg= '"+BZJG+"'";
