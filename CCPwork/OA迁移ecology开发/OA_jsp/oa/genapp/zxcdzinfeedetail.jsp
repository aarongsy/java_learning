...
if(sublist.size()>0){
	for(int k=0,sizek=sublist.size();k<sizek;k++){
		...
		String psncost=StringHelper.null2String(mk.get("psncost"));
		...
		String price = StringHelper.null2String(mk.get("price"));//含税单价
		String fee1 = StringHelper.null2String(mk.get("fee1"));//配件费
		...
		String bnum=StringHelper.null2String(mk.get("bnum"));
		String zxcuser=StringHelper.null2String(mk.get("zxcuser"));
		Double amount=0.00;//含税金额
		Double ratenum=0.00;
		Double fee2=0.00;//配件费
     
		if(price.equals("null")||price.equals("")||price==null)  
		{  
			price="0";   
		}  
		if(fee1.equals("null")||fee1.equals("")||fee1==null)  
		{  
			fee1="0";   
		} 
		...
		fee2=Double.valueOf(price)*Double.valueOf(receivenum);
		if(notax.equals(""))//定价
		{
			amount=Double.valueOf(price)*Double.valueOf(receivenum)+Double.valueOf(fee1);//单价
			ratenum=amount/(Double.valueOf(taxrate)/100+1);//未税
		}
		else
		{
			amount=Double.valueOf(fee1)+Double.valueOf(price);
			ratenum=Double.valueOf(notax);
		}
		
		String insql="";   
		if(ucstcen==null||ucstcen.equals("null")||ucstcen.equals(""))  
		{  
			insql ="insert into uf_oa_gensupcheckdetail (ID, REQUESTID, NO, COLNO, COSTCENTER, REQNAME, REQDEPT, SUBCATEGORY, SPECIFY, UNIT, SENDDATE, ACCEPTNUM, CURRENCY, TAXPRICE, TAXAMOUNT, TAXCODE, TAX, NOTAXAMOUNT, INNERORDER, LEGDERSUBJ, REQAPPFLOWNO, GOODSID, GOODSNAME,taxstyle,jzcode,isfinished,fee1,fee2,fee3,zxcnum,zxcuser) values ((select sys_guid() from dual), '"+requestid+"',  "+(k+1)+","+no+", '"+exttextfield9+"', '"+applyer+"', '"+dept+"', '"+subcate+"', '"+sepcify+"', '"+unit+"', '"+excptdate+"', "+receivenum+", '"+currency+"', "+price+", "+amount+",'"+taxcode+"', '"+taxrate+"', "+ ratenum+", '"+inorder+"', '"+ledgersubj+"', '"+id+"', '"+goodsname+"', '"+goods+"','"+taxstyle+"','40','"+isfinish+"',"+fee2+","+fee1+",0,'"+bnum+"','"+zxcuser+"')";  

		}  
		else  
		{  
			insql ="insert into uf_oa_gensupcheckdetail (ID, REQUESTID, NO, COLNO, COSTCENTER, REQNAME, REQDEPT, SUBCATEGORY, SPECIFY, UNIT, SENDDATE, ACCEPTNUM, CURRENCY, TAXPRICE, TAXAMOUNT, TAXCODE, TAX, NOTAXAMOUNT, INNERORDER, LEGDERSUBJ, REQAPPFLOWNO, GOODSID, GOODSNAME,taxstyle,jzcode,isfinished,fee1,fee2,fee3,zxcnum,zxcuser) values ((select sys_guid() from dual), '"+requestid+"',  "+(k+1)+","+no+", '"+psncost+"', '"+applyer+"', '"+dept+"', '"+subcate+"', '"+sepcify+"', '"+unit+"', '"+excptdate+"', "+receivenum+", '"+currency+"', "+price+", "+amount+",'"+taxcode+"', '"+taxrate+"', "+ ratenum+", '"+inorder+"', '"+ledgersubj+"', '"+id+"', '"+goodsname+"', '"+goods+"','"+taxstyle+"','40','"+isfinish+"',"+fee2+","+fee1+",0,'"+bnum+"','"+zxcuser+"')";  
		}  
		baseJdbc.update(insql);
		insql=" update uf_oa_goodsdetailapp  set price="+price+" where requestid='"+id+"' and no="+no;
		//baseJdbc.update(insql);
