...
String userid=currentuser.getId();//当前用户
	...
	if(action.equals("createPJD")){
		String sql = "select a.requestid,a.carappnos,a.cartype,a.supplyname,a.carno,a.driver,a.drivertel,a.comtype from uf_oa_cararrange a where (a.stateflag is null or a.stateflag>0) and exists(select b.id from formbase b where a.requestid=b.id and b.isdelete=0) and a.comtype='"+comtype+"' and to_date(a.arrsdate,'yyyy-MM-dd')  between to_date('"+sdate+"','yyyy-MM-dd') and to_date('"+edate+"','yyyy-MM-dd')";
		...
		if(list.size()>0){ 
			for(int i=0; i < list.size(); i++){		
				...
				String carappsql = "select distinct a.requestid,a.reqman,(select to_char(sysdate,'YYYY-MM-DD hh24:mi:ss') from dual) curtime,a.flowno from uf_oa_carapp a,requestbase b where a.requestid=b.id and b.isdelete=0 and a.stateflag='3' and instr('"+carappnos+"', a.requestid)>0";
