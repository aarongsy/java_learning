...
<%
	...
	if (action.equals("getData")){
		...
		String theType=StringHelper.null2String(request.getParameter("theType"));//计算规则
		String qjtype=StringHelper.null2String(request.getParameter("qjtype"));//请假类别
		String comtype=StringHelper.null2String(request.getParameter("comtype"));//厂区别		
		double hours = 0.0;
		boolean flag = true;
		
		//xxy 重新获取请假时间段对应的班别开始日期和班次数
		String bcbeginDate="";//班表开始日期
		String bcendDate="";//班表结束日期
		...
		for(int i=0;i<nums;i++){
			...
			String sql = "select a.classno,a.pstime,a.petime,a.rstime,a.retime,b.yesno from uf_hr_classinfo a,uf_hr_classplan b where a.requestid=b.classno and b.objname='"+reqman+"' and b.thedate='"+today+"'";

			List ls = baseJdbcDao.executeSqlForList(sql);
			if (ls.size()>0){
				Map m = (Map)ls.get(0);
				pstime = StringHelper.null2String(m.get("pstime"));//计划开始
				petime = StringHelper.null2String(m.get("petime"));//计划结束
				rstime = StringHelper.null2String(m.get("rstime"));//休息开始
				retime = StringHelper.null2String(m.get("retime"));//休息结束
				classno = StringHelper.null2String(m.get("classno"));//班别编号
				yesno = StringHelper.null2String(m.get("yesno"));//是否公共假日
				...
				if(comtype.equals("4028804d2083a7ed012083ebb988005b")||comtype.equals("40285a90488ba9d101488bbdeeb30008"))//常熟厂  
				{
					if(qjtype.equals("40285a9053a1793101545186ae2d4e67")||qjtype.equals("40285a9053a179310154518ab3da547e")||qjtype.equals("40285a9053a179310154518b9d6a568c")||qjtype.equals("40285a9053a1793101545189f7c8538c")||qjtype.equals("40285a9053a1793101545188ece35230"))//护理假，婚假，产假，流产假 含双休不含法定
					{
						System.out.println("--------------------");
						System.out.println("theType"+theType+"   yesno:"+yesno+" pstime:"+pstime+"  petime:"+petime);
							//if(theType.equals("40285a904931f62b014936582eae18d3")&&(yesno.equals("40288098276fc2120127704884290211")||(yesno.equals("40288098276fc2120127704884290210")&&!classno.equals("OFF"))) || (!theType.equals("40285a904931f62b014936582eae18d3") && !classno.equals("OFF"))){//含休假或（不含休假且班别编号不为OFF）
							if(theType.equals("40285a904931f62b014936582eae18d3")&&(yesno.equals("40288098276fc2120127704884290211")) || (!theType.equals("40285a904931f62b014936582eae18d3") && !classno.equals("OFF"))){//含休假或（不含休假且班别编号不为OFF）
	...
	if(action.equals("getQuo")){
		...
		System.out.println("vacation getQuo objvalue="+objvalue+"  hid="+hid+"  sday="+sday);
		//System.out.println("bcbeginDate="+bcbeginDate+"  bcendDate="+bcendDate);
		//System.out.println("bcbeginDate="+bcbeginDate+"  bcendDate="+bcendDate);
		double total = 0.00;
		...
		for(int i=0;i<obj.length;i++){
			...
			System.out.println("vacation getQuo quoid="+quoid);
			if(quoid.length()>1){		
				//创建SAP对象
				...
				if (retTable != null) {
					for (int n = 0; n < retTable.getNumRows(); n++) {
						...
						System.out.println(ZEINH);
						System.out.println("请假定额时数"+ANZHL);
						if(ZEINH.equals("010")){
