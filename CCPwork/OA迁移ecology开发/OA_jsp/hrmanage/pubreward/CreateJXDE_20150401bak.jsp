		...
		for(int i=0;i<list.size();i++){
			...
			if(ls.size()>0){
				...
				if(mm==1){
					...
					totalquota = (float)(lastquota + beginnum*0.3*(12-mm+1)/2); 
				}else{
					...
					if(ls3.size()>0){
						...
						diffnum = beginnum-lastbeginnum; //异动人数			
						totalquota = (float)(lastquota + diffnum*0.3*(12-mm+1)/2);  
					}else{
						iniflag = "40288098276fc2120127704884290210"; //是
						lastno = "";
						lastbeginnum = beginnum;
						lastquota = 0;
						diffnum = beginnum-lastbeginnum; //异动人数			
						totalquota = (float)(beginnum*0.3*(12-mm+1)/2); 
					}				
				}
				usedquota = 0;
				leftquota = totalquota;			
