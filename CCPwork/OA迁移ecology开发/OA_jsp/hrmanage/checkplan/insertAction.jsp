...
<%
	...
	for(int j=0;j<len;j++)
	{
		...
		String sql2 = "select objno,id,(select max(a.jcdedept)  from uf_hr_pundept a where a.dept =orgid and 0=(select isdelete from formbase where id=a.requestid) and a.comtype='"+com+"') as orgid,extrefobjfield4,extdatefield0 from humres where isdelete=0 and hrstatus='4028804c16acfbc00116ccba13802935' and instr('"+com+"',extrefobjfield5)>0 and instr('"+posi+"',extrefobjfield4)>0 and extdatefield0<='"+day+"' and instr('"+employgroup+"',extselectitemfield11)>0";//这里增加查询条件，暂时没加，后面加上
