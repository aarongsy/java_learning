...
<%
	...
	if(action.equals("newdata")){//分类新建时
		...
		//清空历史数据
		String sql = "delete from uf_hr_unitfootsub where flowno='"+flowno+"'";
		baseJdbc.update(sql);
		//获取数组
		...
		for(int i=0;i<str1.length;i++){			
			String isql = "insert into uf_hr_unitfootsub (ID,flowno,reqdept,jobno,objname,thedate,breakfast,lunch,dinner,num) values (sys_guid(),'"+flowno+"',replace('"+str1[i]+"','null',''),replace('"+str2[i]+"','null',''),replace('"+str3[i]+"','null',''),replace('"+str4[i]+"','null',''),replace('"+str5[i]+"','null',''),replace('"+str6[i]+"','null',''),replace('"+str7[i]+"','null',''),"+i+")";
	...
	if(action.equals("tosub")){//把转存表的数据插入到子表中
		...
		String sql = "select * from uf_hr_unitfootsub where flowno='"+flowno+"' order by num asc";
		...
		if(list.size()>0){
			...
			//	String isql = "insert into uf_hr_unitfootsub (id,requestid,reqdept,jobno,objname,thedate,breakfast,lunch,dinner,num) values (sys_guid(),'"+requestid+"','"+reqdept+"','"+jobno+"','"+objname+"','"+thedate+"','"+breakfast+"','"+lunch+"','"+dinner+"',"+i+")";
			//	baseJdbc.update(isql);
			//}
			String isql = "update uf_hr_unitfootsub set requestid='"+requestid+"' where flowno='"+flowno+"'";
			baseJdbc.update(isql);
