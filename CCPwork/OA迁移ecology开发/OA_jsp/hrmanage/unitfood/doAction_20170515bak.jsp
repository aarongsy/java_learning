...
<%
	...
	if(action.equals("newdata")){//分类新建时
		...
		for(int i=0;i<str1.length;i++){			
			String isql = "insert into uf_hr_forunitfoodsub (flowno,reqdept,jobno,objname,thedate,breakfast,lunch,dinner,num) values ('"+flowno+"',replace('"+str1[i]+"','null',''),replace('"+str2[i]+"','null',''),replace('"+str3[i]+"','null',''),replace('"+str4[i]+"','null',''),replace('"+str5[i]+"','null',''),replace('"+str6[i]+"','null',''),replace('"+str7[i]+"','null',''),"+i+")";
	...
	if(action.equals("tosub")){//把转存表的数据插入到子表中
		...
		String sql = "select * from uf_hr_forunitfoodsub where flowno='"+flowno+"' order by num asc";
