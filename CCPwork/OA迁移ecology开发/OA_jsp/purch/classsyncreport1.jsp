...
<%
	...
	String humreid=BaseContext.getRemoteUser().getId();
	 ...
	 String[] tmpArr={"",""};
	 String  QueryString="";
	 //String orderno="";//初始化采购订单编号
	 String suppcode="";//初始化供应商简码
	 String flower="";//初始化流程单号
	 String URL =request.getQueryString();//得到的是地址栏?后面的内容
	 //URL:suppcode='+XX+'&flower='+YY
	 if(URL!=null&&!URL.equals("null"))
	 {
		 tmpArr=URL.split("&");
		 if(tmpArr[0].split("=").length>1)
		 {
			 suppcode=tmpArr[0].split("=")[1];
		 }
		 if(tmpArr[1].split("=").length>1)
		 {
			flower=tmpArr[1].split("=")[1];
		 }
	 }
%>
         ...
         <script language="javascript">
          ...
          Ext.LoadMask.prototype.msg='<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>';//加载中,请稍后...
			 ...
			 var cm = new Ext.grid.ColumnModel([sm,
										   ...
										   {header:'供应商简码',dataIndex:'supcode',hidden:false,width:30,sortable:true},
										   {header:'供应商名称',dataIndex:'supname',hidden:false,width:30,sortable:true},
										   {header:'采购员',dataIndex:'supperson',hidden:false,width:30,sortable:true}
			         ]);       
...
sm.on('rowselect',function(selMdl,rowIndex,rec ){
        ...
        var reqid=rec.get('pono')+'^@^'+rec.get('poitem')+'^@^'+rec.get('supcode')+'^@^'+rec.get('supname')+'^@^'+rec.get('supperson');
...
<div id="divSearch">
 ...
 <table>
   ...
   <tr>
 	<td  class="FieldName" nowrap>采购订单编号</td>
    <td  class="FieldValue"  nowrap>
           <input type="text" class=inputstyle style="width:80%" name="ebeln" id="ebeln" value="" >
		     <!--<input type="hidden" class=inputstyle name="suppcode" id="suppcode" value="<%=suppcode%>" >
			 <input type="hidden" class=inputstyle name="flower" id="flower" value="<%=flower%>" >-->
    </td>
   ...
   function syncdo(){
	   try{
		   var myMask = new Ext.LoadMask(Ext.getBody(), {  
        	...
        	jQuery.ajax({
        		async:true,
	    	    url: "http://192.168.218.13:6666/ServiceAction/com.eweaver.app.sap.orgunit.ClassSyncAction",
	    	    data: {
	    	     jsonstr : selected+"" ,
	    	     action : "synchronous",
	    	     kssj	: $("#kssj").val(),
