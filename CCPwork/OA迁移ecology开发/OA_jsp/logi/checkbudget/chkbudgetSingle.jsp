...
<%
	...
	if (action.equals("chkbudgetSingleforce")){	//强制物流费用暂估单校验与修正
		JSONObject jsonObject = new JSONObject();		
		String requestid=StringHelper.null2String(request.getParameter("requestid"));
		//Boolean force=new Boolean(StringHelper.null2String(request.getParameter("force")));	
		String sql = "select a.requestid,a.invoiceno,a.loadplanno,a.createdate,a.companyno,a.principle from uf_lo_budget a where a.requestid='"+requestid+"' and a.invoicestatue='402864d149e039b10149e080b01600c0' and exists(select 1 from formbase where id=a.requestid and isdelete=0) ";
		...
		if ( list.size()>0 ) {		      
			...
			String invoiceno = StringHelper.null2String(map.get("invoiceno"));
			String loadplanno = StringHelper.null2String(map.get("loadplanno"));			
			String loadplanreqid = StringHelper.null2String(ds.getValue("select p.requestid from uf_lo_loadplan p where p.reqno = '" + loadplanno + "'"));
			String companyno = StringHelper.null2String(map.get("companyno"));
			String principle = StringHelper.null2String(map.get("principle"));
			try {
				...
				if ( !"0".equals(a[0]) ) {
					//flag = app.checkbudget(loadplanno,requestid);	
					app.crebudgetdetail(requestid ,loadplanno, loadplanreqid, companyno, principle,flag );
					flag1 = "修正后：强制修正完成 修正前："+a[1] ;
				} else {
					flag1 = flag;
				}
