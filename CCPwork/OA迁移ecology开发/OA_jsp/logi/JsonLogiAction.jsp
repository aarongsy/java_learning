...
<%@ page import="com.eweaver.app.weight.service.Uf_lo_loadplanFeeShare"%>
...
//				ckmode="ck3";//直接跳开
		    ...
		    for(int i=0;i<list.size();i++){	    	
	    	   ...
	    	   if(ckmode.equals("ck2")&&state.equals("402864d14931fb790149328a92bd0016")){
	    		   ...
	    		   */
					String ck2sql = "select state from uf_lo_loadplan where requestid in(select requestid from uf_lo_loaddetail where cardetailid in (select id from uf_lo_dgcardetail where requestid='"+requestid+"')) and state != '402864d1493b112a01493bfaf09b000c'";
		    	...
		    	double deliverdnum = NumberHelper.string2Double(((Map)list.get(i)).get("deliverdnum"), 0);
	...
	if (action.equals("back")){//成品交运,重量证明
		...
		String cusweight = StringHelper.null2String(request.getParameter("cusweight"));
			String flag = "true";
			...
			String upsqly="";	
			if(NumberHelper.string2Double(sendreqtime.replace("-", ""),0)-NumberHelper.string2Double(signdate.replace("-", ""),0)>=0){
			...
			if(!StringHelper.isEmpty(cusweight)){
				upsqly = "update uf_lo_loaddetail set clientgeepound='"+cusweight+"' where requestid in(select a.requestid from uf_lo_loadplan a,uf_lo_provedoc b where a.reqno=b.loadno and b.requestid='"+reqid+"')";
				unum = baseJdbcDao.update(upsqly);
				if(unum<=0){
					flag = "false";
				}
			}
	...
	*/
	if(action.equals("checkSpot")){
		...
		String ladno = "";
		String ispond = "";
		String sql = "select a.deliverdnum,a.clientgeepound,a.realnums,b.ladingno,a.runningno,b.ispond,a.cardetailid,(select needtype from uf_lo_loadplan where reqno=b.loadingno) needtype,(select ladingno from uf_lo_ladingmain where requestid = b.ladingno) ladno,(select requestid from uf_lo_loadplan where reqno=b.loadingno) loadingno from uf_lo_spotdetail a,uf_lo_spotmanager b where a.requestid=b.requestid and a.requestid ='"+requestid+"' and b.state='402881f34a566549014a5846f1ef085e'";
		List list=baseJdbcDao.executeSqlForList(sql);
		if(list.size()>0){
			Map m0 = (Map)list.get(0);
			for(int i=0;i<list.size();i++){
				Map mi = (Map)list.get(i);
				double deliverdnum =NumberHelper.string2Double(mi.get("deliverdnum") ,0.0);
				double realnum =NumberHelper.string2Double(mi.get("realnums") ,0.0);
				//fjl add
				double clientgeepound = NumberHelper.string2Double(mi.get("clientgeepound") ,0.0);
				//end 
				loadno = StringHelper.null2String(mi.get("loadingno")); 
				if(deliverdnum!=realnum){
				
					String ladingno = StringHelper.null2String(mi.get("ladingno")); 
					loadno = StringHelper.null2String(mi.get("loadingno")); 
					ladno = StringHelper.null2String(((Map) list.get(i)).get("ladno")); 
					//ispond = StringHelper.null2String(mi.get("ispond")); 
					...
					if(cardetailid.length()>10){
						sql = "update uf_lo_loaddetail  t set odeliverdnum=deliverdnum where requestid='"+requestid+"' and runningno='"+runningno+"'   and cardetailid='"+cardetailid+"' and nvl(odeliverdnum,0)<=0";
					}
					else{
						sql = "update uf_lo_loaddetail  t set odeliverdnum=deliverdnum where requestid='"+requestid+"' and runningno='"+runningno+"'   and nvl(odeliverdnum,0)<=0";		
					}
					ds.executeSql(sql);
					//更新提入单明细qeaaa
					...
					//更新装卸计划明细
					String upsql2 = "update uf_lo_loaddetail set deliverdnum="+realnum+",clientgeepound="+ clientgeepound +",plannum='"+realnum+"' where requestid = '"+loadno+"' and runningno='"+runningno+"'";
					ds.executeSql(upsql2);		
					System.out.println("现场收发审核：提入单ladno="+ladno+"sql="+sql+" upsql="+upsql+" upsql2="+upsql2);
					//更新计划、派车单及单据相关联数据
					LoadPublicService loadPublicService = new LoadPublicService();
					loadPublicService.refreshBillStateNumByPlanOne(needtype,loadno,runningno,cardetailid);
				}
				
				/* baseJdbcDao.update(upsql); */
			}
			//更新现场收发主表的数量总计
			sql = "update uf_lo_spotmanager t set totalnum=(select sum(realnums) from uf_lo_spotdetail a where a.requestid=t.requestid ) where t.requestid='"+requestid+"'";
			ds.executeSql(sql);
			System.out.println("现场收发审核：更新现场收发主表的数量总计 sql="+sql);
			//更新提入单的数量总计
			String trdid = StringHelper.null2String(m0.get("ladingno"));
			sql = "update uf_lo_ladingmain t set totalnum=(select sum(deliverdnum) from uf_lo_ladingdetail a where a.requestid=t.requestid ) where t.requestid='"+trdid+"'";
			ds.executeSql(sql);		
			System.out.println("现场收发审核：更新提入单的数量总计 sql="+sql);
		}else{
			jo.put("msg","该单据已经审核或作废，不允许操作！");
			response.getWriter().write(jo.toString());
			response.getWriter().flush();
			response.getWriter().close();
			return;
		...
		if(sqlList.size()>0){
			...
			try{ 
				...
				tm.commit(status);
				//System.out.println( "total="+total+ "  total == 0.0 "+(total == 0.0));
				if("40288098276fc2120127704884290211".equals(ispond)){
					Uf_lo_proveService ps = new Uf_lo_proveService();
					ps.createProveByPlan(ladno, "");
					Uf_lo_budgetService bs = new Uf_lo_budgetService();
					//判断装卸计划明细中所有提入单是否都已过磅，再确认是否生产暂估单--汤烈
					BaseJdbcDao daseDB =(BaseJdbcDao)BaseContext.getBean("baseJdbcDao");
					daseDB.update("update uf_lo_ladingmain set sfygb = 'y' where ladingno = '"+ladno+"'");
					String sql_sfygb = "select sfygb,count(*) total from uf_lo_ladingmain "
					+"where exists (select 1 from formbase where id = requestid and isdelete <> 1) "
					+"and sfygb is null and ladingno in("
					+"select pondno from uf_lo_loaddetail where requestid = '"+bs.getRequestidByLadingno(ladno)+"'"
					+") group by sfygb";
					List list_sfygb = daseDB.executeSqlForList(sql_sfygb);
					Double total = 0.0;
					if(list_sfygb.size()>0){
						Map pMap = (Map) list_sfygb.get(0);
						total = NumberHelper.string2Double(pMap.get("total"),0.0);
					}					
					if(total == 0.0){						
						System.out.println("---------------- 现场收发货暂估生成开始 -----------------");
						bs.createBudgetByPlan(bs.getRequestidByLadingno(ladno));
						System.out.println("---------------- 现场收发货暂估生成结束 -----------------");						
					}
				}
				jo.put("msg",flag);
				response.getWriter().write(jo.toString());
		...
		//2017-10-19 xxy add 重新计算装卸计划明细的分摊金额，按 装卸数对应的KG数*城市点里程数 作为分摊基准
		Uf_lo_loadplanFeeShare app = new Uf_lo_loadplanFeeShare();
		String zxplanreqid = loadno;
		String errmsg = app.FeeShare(zxplanreqid);
		System.out.println(errmsg);
		DontWeigh_logs weighLogs = new DontWeigh_logs();
		weighLogs.getTheSingleAtList(requestid);
	}
