		...
		for(int i = 0; i < list.size(); i++){
           ...
           String o = NumberHelper.getSequenceNo("40285a8d5842e03f015851da962d2a0c", 3);
			...
			if(!supoacode.equals("")){
				...
				if(!"".equals(bjrequestid))
				{
					srequest="select cabtype,isdanger,dangelv,hxarea,line,startport,endport,require,goods,cabnum,gxty from uf_tr_xunjiasub where requestid='"+requestid+"' order by sno";
					...
					if(listl.size()>0){ 
						for(int j = 0; j < listl.size(); j++){
							...
							String insql="insert into uf_tr_baojiachild (id,requestid,sno,gx,isdanger,dengerllv,hxarea,hx,qygang,mdgang,equair,pro,xjrequestid,gl,gxty)values((select sys_guid() from dual),'"+bjrequestid+"',"+(j+1)+",'"+cabtype+"','"+isdanger+"','"+dangelv+"','"+hxarea+"','"+line+"','"+startport+"','"+endport+"','"+require+"','"+goods+"','"+requestid+"','"+cabnum+"','"+gxty+"')";
