...
String sql = "";
	if(SQLMap.getDbtype().equals(SQLMap.DBTYPE_ORACLE))
		sql = "select id requestid,objname,moduleid,nvl((select count(*) from requestbase where workflowid=t.id and isdelete=0 and isfinished=0 and creater='"+userCndID+"'),0) nums,nvl((select count(*) from requestbase where workflowid=t.id and isdelete=0 and isfinished=1 and creater='"+userCndID+"'),0) finishnums from workflowinfo t where isactive=1 and isdelete=0 order by moduleid,dsporder";
	else
		...
		for(int i=0;i<size;i++){
			...
			for(int j=0;j<size2;j++){
				...
				if(typeid.indexOf(requestid)>-1)
				{
					...
					buf.append("<a href=\"javascript:onUrl('"+url+requestidsub+"&creator="+userCndID+"&isfinished=0','"+m1.get("objname").toString()+ "')\">"+m1.get("objname").toString()+"</a>( "+nums+" / "+(Integer.parseInt(nums)+Integer.parseInt(finishnums))+")</li></ul>");
				}
			}
			buf.append("</ul>");
			if(i==5)
		...
		for(int i=0;i<size3;i++){
			...
			for(int j=0;j<size2;j++){
				...
				if(typeid.indexOf(requestid)>-1)
				{
					...
					buf1.append("<a href=\"javascript:onUrl('"+url+requestidsub+"&creator="+userCndID+"&isfinished=1','"+m1.get("objname").toString()+ "')\">"+m1.get("objname").toString()+" </a>( "+finishnums+" / "+(Integer.parseInt(nums)+Integer.parseInt(finishnums))+")</li></ul>");
				}
			}
			buf1.append("</ul>");
			if(i==5)
