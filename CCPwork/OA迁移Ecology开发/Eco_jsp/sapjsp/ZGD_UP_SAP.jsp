...
<%
	...
	try {
		...
		String sql = " select t1.*,(select a.lastname from HrmResource a where a.id =t1.creditpsn) username,t2.*,t2.notaxamt as money,to_char(sysdate,'yyyyMMdd') today,to_char(sysdate,'MM') curmon from uf_zgfy t1  left join uf_zgfy_dt1 t2 on t1.id = t2.mainid ";
		...
		if (rs.execute(sql)) {
			while (rs.next()) {
				if (index == 0) {
					...
					mainMap.put("HEADER_TXT", Util.null2String(rs.getString("djtitle")));//凭证抬头文本
					//mainMap.put("PSTNG_PERIOD", "12");//记账期间
					mainMap.put("PSTNG_PERIOD", curmon);//记账期间
					mainMap.put("USER_NAME",  Util.null2String(rs.getString("username"))); //用户名
