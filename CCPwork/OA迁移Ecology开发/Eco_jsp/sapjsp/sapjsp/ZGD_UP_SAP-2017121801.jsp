...
<%
	...
	try {
		...
		String sql = " select t1.*,t2.*,t2.notaxamt as money from uf_zgfy t1  left join uf_zgfy_dt1 t2 on t1.id = t2.mainid ";
		...
		if (rs.execute(sql)) {
			while (rs.next()) {
				if (index == 0) {
					...
					mainMap.put("DOC_DATE", "20170701");//凭证日期
					mainMap.put("PSTNG_DATE", "20170701");//记账日期
					//测试可先用1010
					//mainMap.put("COMP_CODE", Util.null2String(rs.getString("comcode")));//公司代码 
					mainMap.put("COMP_CODE", "1010");//公司代码 
					//测试可先用RMB	
					//mainMap.put("CURRENCY", Util.null2String(rs.getString("comcode")));//货币马
				...
				if ("0".equals(djlx)) {//so 上传销售
					...
					map.put("PO_NO", "123");//销售订单号
					map.put("PO_ITEM", "");//销售项次
				} else if ("1".equals(djlx)) {//po 上传采购
					map.put("PO_NO", Util.null2String(rs.getString("pono")));//销售订单号
					map.put("PO_ITEM", Util.null2String(rs.getString("orderitem")));//销售订单项次
					map.put("SO_NO", "");//采购订单号
					map.put("SO_ITEM", "");//采购订单项次
				} else {
					//TODO
				}

				map.put("ITEM_TEXT", Util.null2String(rs.getString("itemtext")));//项目文本				
				map.put("MATERIAL", Util.null2String(rs.getString("wlh")));//物料号
		...
		if ("X".equals("FLAG")) {
			String updateSql = "";
			if ("0".equals(djlx)) {
				updateSql = "update formtable_main_45 set sfsp = '1' where zxjhh ='" + djbh + "'";
			} else if ("1".equals(djlx)) {
				updateSql = "update  formtable_main_61 set sfsp = '1' where zxjhh ='" + djbh + "'";
			}
		...
		String updateSql = "update uf_zgfy set sappzh = '" + AC_DOC_NO + "',message = '" + ERR_MSG
				+ "',msgty = '" + FLAG + "',djstatus = '1',gzrq='" + nowDate + "', creditmonth='"
				+ currdate.substring(4, 6) + "' where djbh = '" + djbh + "'";
		if (!rs.execute(updateSql)) {
			log.writeLog("更新sap凭证号错误!");
		}
