...
<%
	...
	try {
		...
		String qkqzid = request.getParameter("id");//请款清账单id
		...
		String sql = "select COMCODE,T1.pzrq,PZLX,T1.JZRQ,HBDM,CZ,PZTT,t1.bsrq from uf_qkqz t1 left join uf_qkqz_dt2 t2 on t1.id = t2.mainid where t1.id="+qkqzid+" and t1.yddzd = '"
				+ yddzd + "'  and t1.sqbh='"+sqbh+"'";
		log.writeLog(sql);
		rs.executeSql(sql);
		if (rs.next()) {
			...
			VATDATE =  Util.null2String(rs.getString("bsrq")); //报税日期
		...
		String checkSql1 = "select sum(t2.je) as je,t2.fph from uf_qkqz t1 left join uf_qkqz_dt2 t2 on t1.id = t2.mainid where t2. jzdm = '40' and t1.yddzd = '"
				+ yddzd + "' and t1.sqbh='"+sqbh+"' group by t2.fph" + " order by t2.fph";
		...
		StringBuffer sqllist=new StringBuffer()
				...
				.append("where t1.id=t2.mainid and t1.zt!='5' and a.id='"+qkqzid+"' and t1.yddzd='"+yddzd+"' and t1.sqbh='"+sqbh+"' order by t2.id");
		StringBuffer headerSql = new StringBuffer().append("select a.*,(select lastname from HrmResource  where id =a.fhm) username from uf_qkqz a where a.zt!='5' and a.id='"+qkqzid+"' and a.yddzd='" + yddzd + "' and a.sqbh='"+sqbh+"'");
		...
		if (rs.next()){
			...
			VATDATE=Util.null2String(rs.getString("bsrq"));//Tax Reporting Date
			...
			REF_DOC_NO=Util.null2String(rs.getString("cz"));//参考凭证号
			HEADER_TXT=Util.null2String(rs.getString("pztt"));//凭证抬头文本
			PSTNG_PERIOD=Util.null2String(rs.getString("jzqj"));//计账期间
			USER_NAME=Util.null2String(rs.getString("username"));//用户名
			...
			headers.put("VATDATE",getSapChange(VATDATE));//Tax Reporting Date
			...
			headers.put("PSTNG_PERIOD",PSTNG_PERIOD);//记账期间
			headers.put("USER_NAME",USER_NAME);//用户名
			headers.put("NOTESID",NOTESID);//NOTESID
			headers.put("OA_TYPE",OA_TYPE);//EX代表出口，IM代表进口  LO代表物流
		}
		...
		while (rs.next()){
			...
			ZOA_DOC_NO=Util.null2String(rs.getString("zgdsap"));//暂估号码
			REF_DOC_NO=Util.null2String(rs.getString("fph"));//发票号
			//HEADER_TXT=Util.null2String(rs.getString("pzttwb"));//凭证抬头文本
			...
			item.put("VATDATE",getSapChange(VATDATE));//Tax Reporting Date
		...
		for (int i = 0; i < retable.getNumRows(); i++) {
			...
			jsonArr.add(jsonObject);
			    ...
			    //更新明细表2
				StringBuffer updateDetail2=new StringBuffer().append("UPDATE uf_qkqz_dt2 SET  sapqzpzh='"+AC_DOC_NO+"'")
						.append(",sapfhfph='"+REF_DOC_NO+"',sapfhbs='"+FLAG+"' ")
