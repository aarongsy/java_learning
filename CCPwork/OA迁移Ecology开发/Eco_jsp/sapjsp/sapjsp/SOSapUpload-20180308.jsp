...
<%
	...
	try {
		...
		continueFor:for (String set : sets) {
			DELIVERYNO = set;
			bs.writeLog("开始校验:"+DELIVERYNO);
			...
			while (rs.next()) {
				...
				if (SAPUPLOAD.equals("1")) {
					message += DELIVERYNO + "，该交运单已上抛过SAP， 不可重复上抛！\n";
				...
				//2、包装类型≠Z02并且≠Z01，则提示“交运单号XXXX项次XX包装类别无法识别，无法上抛”（Z02：固定包装 Z01 ：散装 ）
					...
					message += DELIVERYNO + "项次号：" + DELIVERYITEM + "包装类别无法识别，无法上抛\n";
				...
				//3、如果货物剩余数量大于0,则不能SAP上抛
					...
					message+= DELIVERYNO + ",项次号：" + DELIVERYITEM + "，仍有货物尚未装卸，无法上抛！\n";
					...
					while (rs3.next()) {
						...
						*/
							...
							while (rs2.next()) {
								...
								//判断提入单是否打印
								if (sfdy.equals("0")) {
									message += DELIVERYNO+",项次号："+DELIVERYITEM+",装卸计划" + zxjhh + "提入单" + trdh + ", 不需要过磅的装卸计划存在未实际出货的产品明细不计入总量!\n";
                    ...
                    if (totalDouble2 < LFIMG) {
                        message += DELIVERYNO + "项次号：" + DELIVERYITEM + ", 实际装卸数量" + totalDouble2 + "未达到上抛标准"
