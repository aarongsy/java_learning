...
<%
	...
	try {
		if(lx.equals("0")){
			if(!shipping.equals("")){
				sql+="SELECT a.shipping,";
				sql+="(SELECT B1.SJGH from FORMTABLE_MAIN_45 a1,FORMTABLE_MAIN_45_DT1 b1 where a1.id = b1.MAINID and a1.MODEUUID is not NULL and a1.REQUEST_ID > 0 and b1.SHIPPING=a.SHIPPING and b1.GBH = b.CODE ) as code,";
				sql+=" a.cabtype, ";
				sql+=" (SELECT jz from uf_gbjl where gh = b.code  and jz is not null and ROWNUM = 1 ) as jz, ";
				sql+=" (SELECT gbrq from uf_gbjl where gh = b.code  and jz is not null and ROWNUM = 1 ) as gbrq from UF_GHLR a,UF_GHLR_DT1 b ";
				sql+=" where a.id = b.mainid and a.shipping = '"+shipping+"'";
			...
			if(!shipping.equals("0")){
				...
				sql+= "SELECT a.zxjhh,to_char(wm_concat(distinct b.shipping)) YGSHIPNO,to_char(wm_concat(distinct b.sjgh))  ghcx,sum(b.gz) gz,a.SJYSRQ,a.CRZ,a.ccz,a.cp,(SELECT CARTYPE from UF_CLCXGL where id = a.cx) cx from formtable_main_45 a,formtable_main_45_dt1 b where a.id=b.mainid and a.SFZF = 0 and NVL(a.requestid,-1)=-1 and a.sfyg=1 AND SJYSRQ BETWEEN  '"+ksrq+"' and '"+jsrq+"' group by a.zxjhh,a.SJYSRQ,a.CRZ,a.ccz,a.cp,a.cx ";
				...
				while(rs.next()){
					...
					obj.put("gz", Util.null2String(rs.getString("gz")));
					jsonArr.add(obj);
				}
			}
		}
