		...
		while(rs.next()){
			Map<String,String> map = new HashMap<String,String>();
			 map.put("SHIPADVICENO", Util.null2String(rs.getString("SHIPADVICENO")));//SHIPPING
	            ...
	            map.put("SHIPTO", Util.null2String(rs.getString("SHIPTO")));//送达方名称
	            map.put("SHIPTOADDR", Util.null2String(rs.getString("SHIPTOADDR")));//送达方地址
	            map.put("NTGEW", Util.null2String(rs.getString("NTGEW")));//数量
	            map.put("COSTCENTER", Util.null2String(rs.getString("COSTCENTER")));//成本中心
	            map.put("LCEN", Util.null2String(rs.getString("LCEN")));//利润中心
	            map.put("MATERDES", Util.null2String(rs.getString("MATERDES")));//客户物料描述
	            map.put("CUSORDNO", Util.null2String(rs.getString("CUSORDNO")));//客户订单编号
...
if(type.equals("checkCount")){
	...
	for(int i=0;i<jsonArray.size();i++){
		...
		sql="select SHIPNUM,BRGEW from uf_spghsr where DELIVERYNO='"+sono+"' and DELIVERYITEM='"+soitem+"'";
		rs.writeLog(sql);
		rs.execute(sql);
		while(rs.next()){
			String LFIMG=rs.getString("SHIPNUM");//实际出货
			String BRGEW=rs.getString("BRGEW");//毛重
			Double nowCounts=calCulate(BRGEW, LFIMG,"sub");
			rs.writeLog("实际出货为："+LFIMG+",毛重为："+BRGEW+",剩余数量为："+nowCounts);
			Double insertSL=Double.valueOf(jsObject.getString("plannum")).doubleValue();
			if(nowCounts<insertSL){
				out.write("插入数据库中时发现 订单号为："+sono+",项次为："+soitem+",本次插入数量为:"+insertSL+",超过了系统实际剩余量:"+nowCounts);
				return;
			}
...
if(type.equals("updateFqh")){
	...
	for(int i=0;i<jsonArray.size();i++){
		...
		sql="SELECT a.SHIPPING,b.code,b.fqh,b.id FROM UF_GHLR a,UF_GHLR_DT1 b where a.id=b.MAINID and a.SHIPPING='"+shipno+"' and b.code='"+gh+"'";
		rs.writeLog(sql);
		rs.execute(sql);
		String fqh2="";
		String ghid="";
