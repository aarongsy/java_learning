...
rs.writeLog("费用类型："+costtype + "承运商简码："+cyscode);
...
//	.append(",(select x.itemtext from UF_ZGFY_DT1 x where x.mainid=a.id and x.jzdm='40' and rownum=1 ) feename,a.djtitle")
 ...
 //    .append(" from uf_zgfy a,UF_ZGFY_DT1 b  where a.id=b.MAINID  AND a.DJSTATUS='1' and b.jzdm='50'");
	...
	sb.append("SELECT a.zxplanno,a.lgbh,a.djlx,a.fylx,(select fylx from uf_fylx where id=a.fylx) fylxname,a.cysname")
	.append(",(select conname from uf_dmlo_consolidat where id=a.cysname) cysnametxt,a.cx,(select cartype from uf_clcxgl where id=a.cx) cxtxt,a.dw,a.carno,a.amount,sum(b.notaxamt) notaxamt  ,b.itemtext feename,a.djtitle")
	.append(",a.HL,a.cyscode,b.pono,sum(b.zl) zl,b.khdz,b.wlh,a.currency,a.sappzh,a.djbh,b.JZDM from uf_zgfy a,UF_ZGFY_DT1 b  where a.id=b.MAINID ")
	.append(" AND a.DJSTATUS='1' and b.jzdm='40' ");
if(!compcode.equals("")) {
...
//	}
	...
	sb.append(" group by a.zxplanno,a.lgbh,a.djlx,a.fylx,a.cysname,a.cx,a.dw,a.carno,a.amount,b.itemtext,a.djtitle,a.HL,a.cyscode,b.pono,b.khdz,b.wlh,a.currency,a.sappzh,a.djbh,b.JZDM ");
	sql = sb.toString();
	try {
		rs.writeLog(sql);
		rs.execute(sql);
		...
		while (rs.next()) {
			...
			String pono = Util.null2String(rs.getString("pono"));//订单号
			...
			//String costcenter = Util.null2String(rs.getString("costcenter"));//成本中心 取消
			//String account = Util.null2String(rs.getString("account"));//总账科目	取消
			...
			String feename = Util.null2String(rs.getString("feename")); //行项目文本
			String djtitle = Util.null2String(rs.getString("djtitle")); //抬头文本
			...
			//jsObject2.put("costcenter", costcenter);	//取消
			//jsObject2.put("account", account);	//取消
			jsObject2.put("currency", currency);

			jsObject2.put("djbh", djbh);
			jsObject2.put("sappzh", sappzh);
			jsObject2.put("lgbh", lgbh);
			jsObject2.put("feename", feename);
			jsObject2.put("djtitle", djtitle);
			...
			*/
			...
			if (djlx.equals("0") && !"".equals(zxplanno)) { //SO建模表
				...
				//sql = " SELECT distinct a.zxjhh,a.gbrq,a.gbzl,a.jfms,(select selectname from workflow_selectItem where fieldid=9686 and cancel=0 and selectvalue=a.JFMS) jfmstxt, b.shipping,b.sjgh gh,(select to_char(wm_concat(distinct x.cp)) from formtable_main_45_dt3 x where x.mainid=a.id and b.shipping=x.shipno  ) product,(select to_char(wm_concat(distinct x.xsddh)) from formtable_main_45_dt3 x where x.mainid=a.id and b.shipping=x.shipno  ) saporderno FROM formtable_main_45 a,formtable_main_45_dt1 b where  b.mainid=a.id and NVL(requestid,-1)=-1 and a.sfzf=0 and a.sfyyf=1 and a.zxjhh='" + zxplanno + "'";
				sql = " SELECT distinct a.zxjhh,a.gbrq,a.gbzl,a.jfms,(select selectname from workflow_selectItem where fieldid=9686 and cancel=0 and selectvalue=a.JFMS) jfmstxt, b.shipping,b.sjgh gh,(select to_char(wm_concat(distinct x.cp)) from formtable_main_45_dt3 x where x.mainid=a.id and b.shipping=x.shipno  ) product FROM formtable_main_45 a,formtable_main_45_dt1 b where  b.mainid=a.id and NVL(requestid,-1)=-1 and a.sfzf=0 and a.sfyyf=1 and a.zxjhh='" + zxplanno + "'";
			}
			if (djlx.equals("0") && !"".equals(lgbh)) { //SO 理货单
				//sql = " SELECT distinct a.lcbh zxjhh,a.ksrq gbrq,a.lolo gbzl,'' jfms,'' jfmstxt, a.shipping,case when a.actcontainer is null then a.gh else a.actcontainer end gh,b.cpmc product,to_char(wm_concat(distinct b.SALEORDER)) saporderno FROM uf_solhsq a,uf_solhsq_dt1 b where b.mainid=a.id and NVL(requestid,-1)=-1 and a.sfzf=0 and a.sfyg=1 and a.lcbh='" + lgbh + "'  group by a.lcbh,a.ksrq,a.lolo,a.shipping,a.actcontainer,a.gh,b.cpmc"; //有柜明细
				sql = " SELECT distinct a.lcbh zxjhh,a.ksrq gbrq,a.lolo gbzl,'' jfms,'' jfmstxt, a.shipping,case when a.actcontainer is null then a.gh else a.actcontainer end gh,b.cpmc product FROM uf_solhsq a,uf_solhsq_dt1 b where b.mainid=a.id and NVL(requestid,-1)=-1 and a.sfzf=0 and a.sfyg=1 and a.lcbh='" + lgbh + "'  group by a.lcbh,a.ksrq,a.lolo,a.shipping,a.actcontainer,a.gh,b.cpmc"; //有柜明细
				sql = sql +" union all ";
				//sql = sql +"SELECT a.lcbh zxjhh,a.ksrq gbrq,a.lolo gbzl,'' jfms,'' jfmstxt, a.wgshipping shipping,case when a.actcontainer is null then a.xngh else a.actcontainer end gh ,b.cpmc product,to_char(wm_concat(distinct b.xsdh)) saporderno from uf_solhsq a,uf_solhsq_dt2 b where a.id=b.mainid and a.sfzf=0 and a.sfyg=0 and a.lcbh='" + lgbh + "' group by   a.lcbh,a.ksrq,a.lolo,a.wgshipping,a.actcontainer,a.xngh,b.cpmc "; //无柜明细
				sql = sql +"SELECT a.lcbh zxjhh,a.ksrq gbrq,a.lolo gbzl,'' jfms,'' jfmstxt, a.wgshipping shipping,case when a.actcontainer is null then a.xngh else a.actcontainer end gh ,b.cpmc product  from uf_solhsq a,uf_solhsq_dt2 b where a.id=b.mainid and a.sfzf=0 and a.sfyg=0 and a.lcbh='" + lgbh + "' group by   a.lcbh,a.ksrq,a.lolo,a.wgshipping,a.actcontainer,a.xngh,b.cpmc "; //无柜明细
			}
			if (djlx.equals("1") && !"".equals(zxplanno)) { //PO流程表
				//sql = "SELECT distinct a.zxjhh,a.gbrq,a.gbzl,a.jfms,(select selectname from workflow_selectItem where fieldid=9686 and cancel=0 and selectvalue=JFMS) jfmstxt,a.jkdhbh shipping,a.jktdgh gh,'' product,b.pono saporderno from formtable_main_61 a,formtable_main_61_dt1 b where a.id=b.mainid and a.sfzf=0 and a.jfyyf=1 and a.sfyg=1 and a.zxjhh='" + zxplanno + "' "; //有柜明细
				sql = "SELECT distinct a.zxjhh,a.gbrq,a.gbzl,a.jfms,(select selectname from workflow_selectItem where fieldid=9686 and cancel=0 and selectvalue=JFMS) jfmstxt,a.jkdhbh shipping,a.jktdgh gh,'' product from formtable_main_61 a,formtable_main_61_dt1 b where a.id=b.mainid and a.sfzf=0 and a.jfyyf=1 and a.sfyg=1 and a.zxjhh='" + zxplanno + "' "; //有柜明细
				sql = sql +" union all ";
				//sql = sql +"SELECT distinct a.zxjhh,a.gbrq,a.gbzl,a.jfms,(select selectname from workflow_selectItem where fieldid=9686 and cancel=0 and selectvalue=JFMS) jfmstxt,'' shipping,'' gh,'' product,b.pono saporderno from formtable_main_61 a,formtable_main_61_dt2 b where a.id=b.mainid and a.sfzf=0 and a.jfyyf=1 and a.sfyg=0 and a.zxjhh='" + zxplanno + "' "; //无柜明细
				sql = sql +"SELECT distinct a.zxjhh,a.gbrq,a.gbzl,a.jfms,(select selectname from workflow_selectItem where fieldid=9686 and cancel=0 and selectvalue=JFMS) jfmstxt,'' shipping,'' gh,'' product from formtable_main_61 a,formtable_main_61_dt2 b where a.id=b.mainid and a.sfzf=0 and a.jfyyf=1 and a.sfyg=0 and a.zxjhh='" + zxplanno + "' "; //无柜明细
			}
			...
			if (rs2.next()) {
				...
				gh = Util.null2String(rs2.getString("gh"));
				product = Util.null2String(rs2.getString("product"));
			}
			...
			if (djlx.equals("1")) { //PO
				...
				sql = "select wlh,''KUNAG ,'' SOLDTO,'' SOLDTOADDR,gyscode KUNNR, gysname SHIPTO,'' shiptoaddr from uf_jmclxq where PONO='" + pono + "' and rownum=1";
			}
