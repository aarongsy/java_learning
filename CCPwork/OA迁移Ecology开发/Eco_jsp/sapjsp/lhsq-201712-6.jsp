...
while(iterator.hasNext()){
	...
	for(int i=0;i<jsonArray2.size();i++){
		...
		sql="select REALSHIPNUM,BRGEW from uf_spghsr where DELIVERYNO='"+jsObject.getString("ddh")+"' and DELIVERYITEM='"+jsObject.getString("xc")+"'";
		rs.writeLog(sql);
		rs.execute(sql);
		while(rs.next()){
			String LFIMG=rs.getString("REALSHIPNUM");//实际出货
			...
			Double insertSL2=calCulate(LFIMG, jsObject.getString("bczxsl"),"add");
			sql="UPDATE uf_spghsr SET REALSHIPNUM='"+insertSL2+"' WHERE DELIVERYNO='"+jsObject.getString("ddh")+"' and DELIVERYITEM='"+jsObject.getString("xc")+"'";
			rs.writeLog(sql);
			rs.execute(sql);
