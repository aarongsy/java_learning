...
<%
	...
	try {
		...
		String sql = "select DISTINCT(HANDINGPLANCODE),(SELECT concode from uf_dmlo_consolidat where id = T.cysbh) as zzkm,t1.fpse,t.*,t2.* from uf_invocemag t left join uf_invocemag_dt2 t2 on t.id = t2.mainid LEFT JOIN uf_invocemag_dt1 t1 on t.id = t1.mainid where t.yddzddh ='"
				+ yddzddh + "'";
		...
		if (rs.executeSql(sql)) {
			while (rs.next()) {
				...
				jsonObj1.put("zzkm", "21810999");//总账科目
				...
				if(!"".equals(fpse)&&(Double.parseDouble(fpse)>0)) {
					...
					jsonObj2.put("zzkm", "21810999");//总账科目
