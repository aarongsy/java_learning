		...
		<div id="mainWrapper">
			...
			<p>You may schedule 1 pickup per day. Pickup hours are between 9am and 9pm with last pickups being accepted at 7pm. Any pickups scheduled after 7pm will be first the following morning</p>
					<?php
						include "../dbconn.php"; 
						//Gets current pickup
						$stmt = $dbh->prepare('SELECT pickups.details, pickups.sizeOfPickup, pickups.timeScheduled, drivers.firstName, drivers.lastName FROM pickups LEFT JOIN drivers ON pickups.driverID = drivers.driverID WHERE pickups.userID = :userID');
						...
						if($stmt2->rowCount() == 0){
							if($stmt->rowCount() != 0){
								...
								foreach($stmt as $row){
									...
									echo "<p>Size: ".$row['sizeOfPickup']."</p><br>";
								...
								echo '<form role="form" id="login" class="form-horizontal" action="addPickup.php" method="POST">
									<div class="form-group">
										...
										<div class="col-sm-10">
											<input type="text" class="form-control" name="details" maxlength="50" placeholder="Enter Details"></input>
										</div>
									</div>
									<div class="form-group">
										<label for="sizeOfPickup" class="control-label col-sm-2">Size of Pickup*: </label>
										<div class="col-sm-10">
											<input type="text" class="form-control" name="sizeOfPickup" maxlength="50" placeholder="Enter Size of Pickup">
										</div>
									</div>
						...
						echo "<th class='hidden-xs'>Size of Pickup</th>";
						...
						foreach($stmt as $row){
								...
								echo "<td class='hidden-xs'>".$row['sizeOfPickup']."</td>";
