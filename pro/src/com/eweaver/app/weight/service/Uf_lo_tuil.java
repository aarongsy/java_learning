...
public class Uf_lo_tuil
{
  public String getworth(String plate, String weight)
  {
    ...
    double tare = 0.0D;
    List list003 = baseJdbc.executeSqlForList("select tare from uf_lo_pondrecord where ladingno = '" + plate + "'");
    if (list003.size() > 0) {
    ...
    baseJdbc.update("update uf_lo_provedoc set inweight = '" + tare + "',outweight = '" + weight + "',lastout = to_char(sysdate,'yyyy-MM-dd HH24:mi:ss') where matchno = '" + plate + "' ");
  ...
  public String getWeightNote(String runningno, String requestid)
  {
    ...
    String remark = "";
    String materialtype = "";
    List list01 = baseJdbc.executeSqlForList("select memo1,materialtype  from uf_lo_delivery where runningno = '" + runningno + "'");
    if (list01.size() > 0) {
    ...
    List list02 = baseJdbc.executeSqlForList("select bulkflag materialtype from uf_lo_purchase where runningno = '" + runningno + "'");
    if (list02.size() > 0) {
      Map map02 = (Map)list02.get(0);
      materialtype = StringHelper.null2String(map02.get("materialtype"));
    }
    List list03 = baseJdbc.executeSqlForList("select materialtype from uf_lo_salesorder  where runningno = '" + runningno + "'");
    if (list03.size() > 0) {
