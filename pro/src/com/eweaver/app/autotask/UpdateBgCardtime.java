...
public class UpdateBgCardtime {
	public void doAction(){
        ...
        //
		String sql="select a.id,a.requestid,a.identify,a.ddate,a.stime,a.etime,a.infactorytime,a.outfactorytime,b.factype  from uf_oa_forwarderlist a left join uf_oa_fbgcclsryrc  b on a.requestid=b.requestid  where 1<>(select isdelete from requestbase where id = a.requestid) and ((select to_date(to_char(sysdate,'yyyy-mm-dd'),'yyyy-mm-dd') from dual) = to_date(a.ddate,'yyyy-mm-dd')+1 or (select to_date(to_char(sysdate,'yyyy-mm-dd'),'yyyy-mm-dd') from dual) = to_date(a.ddate,'yyyy-mm-dd')) and a.identify is not null and a.stime is not null and a.etime is not null";
				...
				if(listc.size()>0)
				{
					...
					for(int j=0;j<listc.size();j++)
					{
						...
						if(inout.toString().equals("进"))
						{
							...
							{
								String upsql1="update uf_oa_forwarderlist set infactorytime='"+cardtime.toString()+"' where id='"+id.toString()+"' ";
						...
						{
							String upsql2="update uf_oa_forwarderlist set outfactorytime='"+cardtime.toString()+"' where id='"+id.toString()+"'";
