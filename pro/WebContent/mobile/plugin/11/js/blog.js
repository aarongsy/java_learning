    function getDiscuddItem(item,isCurrentUser){
		...
		if(discussid!=""){
		  ...
		  listItemString ="<table class='tblItemTitle' width='100%' height='100%' border='0' cellspacing='0' cellpadding='0'>"
					...
					+"							<TD class='ictwz' valign='top'>"
					+"								<span class='ictwz_gw' onclick='openBlog("+userid+")'>"+username+"</span>"
					+"								<span class='ictwz_img'><IMG src='/mobile/plugin/11/images/blog/stateNo.png'></span>"
	    		  ...
	    		  if(action=="saveBlog"){
	    		    if(isToday=="true"){
	    		      if(todayItem.length==0){
		    		      var itemstr="<div class='listitem' onclick='javascript:showItemDetailed();' workdate='"+workdate+"' discussid='"+discussid+"' userid='"+blogid+"' isTodayItem='true'>";
	    		          ...
	    		          todayItem.find(".blogCotent").append(content);
	    		      }    
	    		      item.find("textarea[name='content']").val("");    
	    		    }
	    		    else{
	    		       var itemstr=getDiscuddItem(data.discussItem,blogid);
	    		       item.html(itemstr);
					...
					item.find(".blogCotent").html(content);
...
function bindNavEvent() {
 ...
 function getReplyItem(replyItem){
  ...
  var replaystr="<div class='commentonblock listitemco'>"
		...
		+"											<span class='ictwz_img'><IMG src='/mobile/plugin/11/images/blog/stateRe.png' style='vertical-align: top;margin-top:1px;'></span>"
 ...
 function doAppend(obj){
	   ...
	   var appendBox="<div class='itemReplayBox'>"
					...
					+"		<textarea  style='width:100%;margin-top:5px;height:80px' name='content'></textarea>"
					+" </div>"
	   ...
	   item.find(".itemcontenttitle:first").hide();	
