...
if(StringHelper.isEmpty(detailid)&&StringHelper.isEmpty(workflowid)) {
	String url = "/mobile/plugin/1/infolist.jsp";
...
input[type=text] {
	...
	function validateTime(obj){
		if(obj){
			...
			//使用正规表达式验证时间格式
			var reg = /^(([0-1][0-9])|([1-2][0-3])):([0-5][0-9]):([0-5][0-9])$/;
			if (!reg.test(val)){
				obj.value = "";
...
<SCRIPT LANGUAGE="JavaScript">
	...
	<body>
		...
		<div class="tblBlock" style="width:100%;background:#fff;">
				...
				<form id="workflowfrm"  method="post" enctype="multipart/form-data">
		...
		<%
					...
					%>
						<!-- 流程明细结束 -->
						<%
						Map<String, Map<String, String>>  attachMaps = workflowRequestInfo.getAttachments();
						if(attachMaps != null && !attachMaps.isEmpty()) {
							Map<String,String> attachMap = attachMaps.get("0");
							if(attachMap != null) {						
							String attid = attachMap.get("FILEID");
							if(!StringHelper.isEmpty(attid)) {							
							%>
							<div class="tblBlock" style="width:100%;background:#fff;" id="workflowrequestsignblock">
								<div style="width:100%;" >
									<div class="signRow" style="font-size:14px;font-weight:bold;color:#000;"></div>					
									<div class="signRow">						
										<b><a href="/download.do?fileid=<%=attid%>">正文附件下载</a></b>	
									</div>
								</div>
							</div>
							<%
