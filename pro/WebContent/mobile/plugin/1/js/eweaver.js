...
function regCheck(regExp,obj,msg) {
	//alert(regExp);
	var patrn = new  RegExp(regExp);
	var content = obj.value;
    var rs = patrn.test(content);	
    //alert(patrn+','content+",");
	if (!rs) {
		asyncbox.alert(content + ',' + msg,'数据验证提示');
		obj.value = '';
		obj.focus();
	} 	
}
	...
	if(document.all(elementname)){
		if(tmpvalue){
			...
			if(tmpvalue!=""&&tmpvalue!='null'){
			 ...
			 document.all(spanid).innerHTML="<IMG src='../../../images/base/checkinput.gif' align=absMiddle>";
