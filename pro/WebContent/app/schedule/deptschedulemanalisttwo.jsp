...
%>
 <div class="maindiv" style="text-align: center;">
 	 <div class="detaildiv">
 	    <div>
 	    	<p style="text-align: left;">日期：<input type="text" id="date" name="date" value="<%=date %>" onclick="WdatePicker()"/>
 	    		<a href="javascript:onsearch()" id="newasso"><span>搜 索</span></a>
 	    		<a href="javascript:void(0)" id="newasso" onclick="ontime()"><span>按人员列表</span></a>
 	    		</p>
 	    </div>
 	    <table class="maintable" border="0px;" cellpadding="0" cellspacing="0">
 	       ...
 	       <col width="80%"/>
 	    	...
 	    	<tbody>
 	    		<td colspan="2" class="manger">公司部门负责人行程安排表(<%=startdate %>-<%=enddate %>)</td>
 	    		</tr>
					<%
...
<script>
function onsearch(){
	document.location.href="deptschedulemanalisttwo.jsp?date="+document.getElementById("date").value;
}
function ontime(){
	  var gettime = $('#date').val();
	  location.href='deptschedulemanalist.jsp?date='+gettime+'';
}
</script>
