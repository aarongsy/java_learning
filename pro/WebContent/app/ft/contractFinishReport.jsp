...
String implementdatebeg = request.getParameter("implementdatebeg");
if(implementdatebeg==null) implementdatebeg=startdate;
String implementdateend = request.getParameter("implementdateend");
if(implementdateend==null) implementdateend="";
...
if(implementdatebeg.length()>0)
{
	where = where +" and a.implementdate>='"+implementdatebeg+"'";
}
if(implementdateend.length()>0)
{
	where = where +" and a.implementdate<='"+implementdateend+"'";
}
...
<form action="/app/ft/contractFinishReport.jsp" name="formExport" method="post">
<input type="hidden" name="action" value="search"/>
<input type="hidden" name="exportType" id="exportType" value=""/>
<TABLE id=myTable width="100%" bgcolor="#E7E9EB">
<TBODY>
<TR class=title>
		<TD class=FieldName noWrap width="10%"><%=labelService.getLabelNameByKeyId("402883d934c2275b0134c2275c7f005a") %><!-- 完成日期 --> </td>
		<TD class=FieldValue noWrap>
			<input type=text class=inputstyle readonly size=10 name="implementdatebeg"  value="<%=implementdatebeg%>" onclick="WdatePicker()">-<input type=text class=inputstyle size=10 readonly name="implementdateend"  value="<%=implementdateend%>" onclick="WdatePicker()">
		</TD>   
		...
		<td width=15% class='FieldValue'>
			...
			<option value=""   ></option>
			<%
			selectData=ds.getValues("select id,objname from selectitem where typeid='2c91a0302a8cef72012a8ea97e5f03c9' and pid is null and nvl(col1,0)=0 and isdelete=0 order by dsporder");
     ...
     <td  class="FieldName" width=10% nowrap><%=labelService.getLabelNameByKeyId("402883d934c2275b0134c2275c7f005e") %><!-- 合同状态 --></td>
     <td width=15% class='FieldValue'>
		<select style="width:120" class="inputstyle2" id="statecnd"  name="statecnd">
		<option value=""   ></option>
		<%
		 selectData=ds.getValues("select id,objname from selectitem where typeid='2c91a0302a8cef72012a8eab512b03d5' and pid is null and nvl(col1,0)=0 and isdelete=0 order by dsporder");
		tempstr=statecnd;
		for(int i=0,size=selectData.size();i<size;i++)
		{
			Map m = (Map)selectData.get(i);
...
<!-- 	<col width="80" /> -->
   ...
   <tr style="background:#E0ECFC;border:1px solid #c3daf9;height:20;">
			...
			<td colspan="1" rowspan="1" align="center"><%=labelService.getLabelNameByKeyId("402883d934c2275b0134c2275c800060") %><!-- 执行情况 --></td>
...
for(int i=0,size=realNum-pageNum*(pageno-1);i<size;i++)
{
	...
	String notes="任务:&nbsp;<a href=\"javascript:openchild('taskView.jsp?projectid="+requestid+"&sqlwhere=1=1');\"><u>"+tasknum+"</u></a>&nbsp;完成:&nbsp;<a href=\"javascript:openchild('taskView.jsp?projectid="+requestid+"&sqlwhere=status in  (\\'2c91a0302aa21947012aa232f1860011\\',\\'2c91a0302aa21947012aa232f1860013\\' ,\\'2c91a0302aa21947012aa232f1860012\\')');\"><u>"+(finishnum+qzfinishnum)+"</u></a>&nbsp; 强制结项:&nbsp;<a href=\"javascript:openchild('taskView.jsp?projectid="+requestid+"&sqlwhere=status=\\'2c91a0302aa21947012aa232f1860012\\'');\"><u>"+qzfinishnum+"</u></a>&nbsp;";
	...
	buf1.append("<td align=\"center\"><a href=\"javascript:onUrl('/workflow/request/formbase.jsp?requestid="+requestid+"','"+name+"')\">"+name+"</td>");
