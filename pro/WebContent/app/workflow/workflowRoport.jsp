...
<body>
<form action="/app/workflow/workflowRoport.jsp" name="formExport" method="post">
...
<div style="font-size:15;font-weight:bold;height:20;" valign='middle'><CENTER>工作处理响应总表</CENTER></div>
<CENTER>
<table cellspacing="0" cellpadding="0" border="1" style="border-collapse:collapse;width:900" bordercolor="#333333" width="100%">
  ...
  <col width="80" />
   ...
   <tr style="background:#E0ECFC;border:1px solid #c3daf9;height:30;">
		<td rowspan="2"  align="center">工号</td>
	 <td rowspan="2"  align="center">姓名</td>
	 <td rowspan="2"  align="center">部门</td>
	 <td colspan="2"  align="center">统计</td>
	 <td rowspan="2"  align="center">详情</td>
  </tr>
	<tr height="39">
	<td colspan="1" height="30"  align="center">工作处理数</td>
	<td colspan="1" align="center" >平均耗时(小时)</td>
	</tr>
<%

		String sql = "select A.ID,A.OBJNAME,a.orgid,a.objno from humres a  where  a.hrstatus='4028804c16acfbc00116ccba13802935'  and a.id<>'4028803b21381b8d0121385df3070066' and a.isdelete=0 order by a.objno";
		...
		//sqlserver版本
			sql = "select count(*) nums,a.operator, cast(round(sum((datediff(n,cast(f.submitdate+' '+f.submittime as datetime),cast(b.submitdate+' '+b.submittime as datetime))))/(60.0*count(*)),2) as float) timeout  from requestlog a,requeststep b,requestbase c ,requeststatus e,requeststep f where A.STEPID=B.ID and C.ID=A.REQUESTID and b.id=e.curstepid  and f.id=e.laststepid and  a.stepid=b.id and logtype<>'402881e50c5b4646010c5b5afd170008' and b.submitdate is not null and f.submitdate is not null and datediff(n,cast(f.submitdate+' '+f.submittime as datetime),cast(b.submitdate+' '+b.submittime as datetime))>0 "+where+"  group by a.operator";
		}else{
		//oracle版本
			sql="select count(*) nums,a.operator, cast(round(sum(((to_date(f.submitdate||' '||f.submittime, 'yyyy-mm-dd hh24:mi:ss')-to_date(b.submitdate||' '||b.submittime, 'yyyy-mm-dd hh24:mi:ss'))))/(60.0*count(*)),2) as float) timeout  from requestlog a,requeststep b,requestbase c ,requeststatus e,requeststep f where A.STEPID=B.ID and C.ID=A.REQUESTID and b.id=e.curstepid  and f.id=e.laststepid and  a.stepid=b.id and logtype<>'402881e50c5b4646010c5b5afd170008' and b.submitdate is not null and f.submitdate is not null and (to_date(f.submitdate||' '||f.submittime, 'yyyy-mm-dd hh24:mi:ss')-to_date(b.submitdate||' '||b.submittime, 'yyyy-mm-dd hh24:mi:ss'))>0 "+where+"  group by a.operator";
		}
		...
		for(int i=0,size=mnglist.size();i<size;i++)
		{
			...
			String post = StringHelper.null2String(m.get("orgid"));
			...
			cont.append("<td colspan=\"1\"  align=\"center\" style=\"color:#C92BDB\">"+getBrowserDicValue("orgunit","id","objname",post)+"</td>");
			cont.append("<td colspan=\"1\"  align=\"center\">"+nums+"</td>");
			cont.append("<td colspan=\"1\"  align=\"center\">"+timeout+"</td>");
			cont.append("<td colspan=\"1\"  align=\"center\"><a href=\"javascript:top.frames[1].onUrl('/app/workflow/workflowRoportDetail.jsp?operator="+userid+"&senddatebeg="+senddatebeg+"&senddateend="+senddateend+"','"+name+"工作处理详情');\">查看</a></td>");
			cont.append("</tr>");
		}
