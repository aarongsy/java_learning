...
<%
    String action=request.getContextPath()+"/ServiceAction/com.eweaver.workflow.form.servlet.ForminfoAction?action=getschedulesetlist";
    pagemenustr += "addBtn(tb,'"+labelService.getLabelName("402881e60aa85b6e010aa85e6aed0001")+"','C','add',function(){onPopup('"+request.getContextPath()+"/calendar/schedulesetmodify.jsp')});";
...
<html>
 ...
 <style type="text/css">
   ...
   <%}%>
       ...
       var cm = new Ext.grid.ColumnModel([sm, {header: "设置名称", sortable: false,  dataIndex: 'objname'},
           {header: "表单名称", sortable: false,   dataIndex: 'formname'},
           {header: "操作说明",  sortable: false, dataIndex: 'description'}
       ]);
          ...
          function onDelete()
               {
                   ...
                   Ext.MessageBox.confirm('', '您确定要删除吗?', function (btn, text) {
                       if (btn == 'yes') {
                           Ext.Ajax.request({
                               url: '<%=request.getContextPath()%>/ServiceAction/com.eweaver.workflow.form.servlet.ForminfoAction?action=delscheduleset',
