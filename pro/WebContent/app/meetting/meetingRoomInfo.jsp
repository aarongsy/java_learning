...
<%
	...
	String getMRInfoSql = "select m.mname,(select objname from humres where id = m.msponsor) msponsor," + 
	...
	//boolean f = false;
	//if (request.getParameter("mloc") != null) {
	  // if (f) {
		//  getMRInfoSql += "where mloc = '" + request.getParameter("mloc") + "'";   
	  // } else {
		//  getMRInfoSql += "and mloc = '" + request.getParameter("mloc") + "'";   
	  // }
	//}
	//if (request.getParameter("mloc") != null) {
	//	
...
.mettingtab tr.bg th, tr.bg td{background:#f6f4ec;}
  ...
  if(fck>-1){
    ...
    function addMonth() {
        var disDate = document.getElementById("yearmonth").innerHTML.toString().trim();
    	window.location.href = "/app/meetting/meetingRoomInfo.jsp?disDate=" + disDate + "&len=1";
    }
    ...
    function meetRoomInfo(day) {
    	...
    	window.location.href = "/app/meetting/meetingRoomInfo.jsp?date=" + date + "&disDate=" + disDate + "&len=0";
    }
	    ...
	    <tbody>
	      ...
	      %>
	             <%if(count%2==0){%><tr height="25" class="bg"><%}else{%><tr height="25"><%} %>
	               <td align="left"><a href="javascript:void(0);"><%=mname%></a></td>
