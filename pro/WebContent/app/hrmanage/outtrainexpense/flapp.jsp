...
<%
	...
	if (action.equals("getData")){
		...
		String sql = " select t.flowno,t.pzdate,t.jzdate,t.pztype,t.comcode,t.jzmonth,t.currencycode,t.hl,t.pznumber,t.pztitle,t.username from uf_hr_welfare t where t.requestid='"+requestid+"'";
		...
		if(tlist.size()>0){
			...
			String currencycode = StringHelper.null2String(map.get("currencycode"));//币种
		...
		/*	System.out.println("凭证日期"+pzdate);
			...
			System.out.println("公司代码"+companycode);
...
*/
			...
			sql = "select t.jzcode,t.genledger,t.num,t.rate,t.costcenter,t.innerorder,t.payterm,t.paydate,t.paymanner,t.bank,t.paytext from uf_hr_flcreditdetails t where t.requestid='"+requestid+"'";
			...
			if(list.size()>0){
				for(int i=0;i<list.size();i++){
					...
					//
					getsql = "select objdesc from selectitem where id= '"+pzpayway+"'";
					getlist = baseJdbc.executeSqlForList(getsql);
					if(getlist.size() >0)
					{
						getmap = (Map)getlist.get(0);
						pzpayway = StringHelper.null2String(getmap.get("objdesc"));//
					}
			...
			System.out.println("123AAAAAAAAAAAAAAAAAAAAAAAAAA123123"+pzpaydate);
			System.out.println("payway"+pzpayway );
			...
			String upsql="update uf_hr_welfare set pznumber='"+AC_DOC_NO+"',succflag='"+FLAG+"',errorcode='"+ERR_MSG+"' where requestid='"+requestid+"'";
