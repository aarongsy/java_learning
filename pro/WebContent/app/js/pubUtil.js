...
{ 
   ...
   function browserToCheckbox(fieldid,inputtype,disvalue,sqlwhere,checkonly,readonly,textdirect,style)
   {
	...
	//转换日期时间格式为：'2012/12/12 08:01:01' 此格式支持所有浏览器，解决浏览器兼容问题
	function dateFormatChange(DateOne){                
		var reg = new RegExp('-','g');
		var DateOne = DateOne.replace(reg,'/');         
	    return DateOne;                     
	}
	//获取指定天数后的日期
	function getTheDate(DateOne,nums){     
		var DateOne = dateFormatChange(DateOne);
	    var allnum=Date.parse(DateOne)+1000*60*60*24*nums; 
		var date = new Date(allnum);
		var a = (date.getMonth()+1)+'';
		var b = date.getDate()+'';
		if(a.length<2) a='0'+a;
		if(b.length<2) b='0'+b;
	    return date.getFullYear()+'-'+a+'-'+b;   

	}  
	//获取两个日期间的天数（不排除假期）
	function getBetweenDays(start,end){
		var DateOne = dateFormatChange(end);
		var DateTwo = dateFormatChange(start);
	    var cha=Math.round(parseInt((Date.parse(DateOne+' 23:59:59')- Date.parse(DateTwo+' 00:00:01'))*10/(1000*60*60*24))/10) ;  
	    return cha;   
	} 
