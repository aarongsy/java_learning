...
String	action = request.getContextPath()+"/ServiceAction/com.eweaver.workflow.report.servlet.SysModelReportAction?action=search&browser=1&pagesize=100000&from=list&browser=1&reportid=" + reportid;
String	action2 = request.getContextPath()+"/ServiceAction/com.eweaver.workflow.report.servlet.SysModelReportAction?action=search&browser=1&pagesize=100000&reportid=" + reportid;
...
if(!StringHelper.isEmpty(reportwhere)){
	String[] scArray= reportwhere.split(",");
	for(int i=0; i< scArray.length; i++ ){
		String[] scItem=scArray[i].split("=");
		if(scItem.length==2 && scItem[0].indexOf("con")!=-1 && !"".equals(scItem[1])){
			request.setAttribute(scItem[0], scItem[1]);
		}
	}
}
...
%>
    <td  class="FieldValue" nowrap> 
      <input type=text class=inputstyle size=12 name="con<%=id%>_value" value="<%=StringHelper.null2String(fieldvalue)%>"/>
    </td>
   <%
...
%>
    ...
    <%}
			...
			String selectedValue=StringHelper.null2String(request.getAttribute("con"+_fieldid+"_value"));
			...
			for(int i=0;i<list.size();i++){
				...
				if(_selectvalue.equalsIgnoreCase(StringHelper.null2String(fieldvalue)))
					selected = " selected ";
				if(selectedValue.equalsIgnoreCase(_selectvalue))
					selected = " selected ";
