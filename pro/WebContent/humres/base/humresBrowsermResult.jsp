...
if(fieldsearchMap != null){
        ...
        if(!fieldsearchMap.containsKey(orgInputName) || StringHelper.isEmpty(StringHelper.null2String(fieldsearchMap.get(orgInputName)))){
...
returnmenustr += "addBtn(tb1,'"+labelService.getLabelNameByKeyId("402881eb0bcbfd19010bcc6e71870022")+"','R','accept',function(){onSubmit()});";//确定
%>
<html>
<head>
<style type="text/css">
...
<script type="text/javascript">
	...
	if(typeof(idsin)=='string' && !Ext.isEmpty(idsin)){
     Ext.Ajax.request({
      url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.humres.base.servlet.HumresAction?action=getidsins',
        sync:true,
        method:'POST',
...
if(!StringHelper.isEmpty(sysmodel)){
    ...
    orgTree = new Ext.tree.TreePanel({
            ...
            root:new Ext.tree.AsyncTreeNode({
                ...
                expanded:true,
                id: 'Orgunit_<%=rootid%>',//Orgunit_**
                allowDrag:false,
                allowDrop:false
            })]
            }),
            loader:new Ext.tree.TreeLoader({
                dataUrl: "<%=request.getContextPath()%>/ServiceAction/com.eweaver.base.orgunit.servlet.OrgunitTreeAction?action=getChildrenExt&type=browser",
    ...
    var groupTree = new Ext.tree.TreePanel({
            ...
            loader:new Ext.tree.TreeLoader({
                dataUrl: '<%=request.getContextPath()%>/ServiceAction/com.eweaver.base.security.servlet.SysGroupAction?action=getChildrenExt',
    ...
    var tabPanel = new Ext.TabPanel({
            ...
            items:[orgTree,groupTree,
                new Ext.Panel(
                {
                  title:'<%=labelService.getLabelName("402883d934cf81d90134cf81da140077")%>',//自定义
                  autoScroll:true,                  contentEl:divSearch
                }
                )
            ]
        });
	...
	var viewport = new Ext.Viewport({
        layout: 'border',
        items: [tabPanel,grid,selectedGrid]
	});
    ...
    //d&d
        ...
        var selectedGridDropTarget = new Ext.dd.DropTarget(selectedGridDropTargetEl, {
                ...
                notifyEnter:function(ddSource, e, data){
                    ...
                    proxy.update(ddSource.dragData.selections[0].get('objname'));
                }
            });
           ...
           //系统表Humres,Docbase下拉框联动字段验证保存的是字段名称，需要转换为ID。
             ...
             if(refobj != null){
				...
				if("GET".equalsIgnoreCase(request.getMethod()) && "402881821197b641011197b644690015".equalsIgnoreCase(_fieldid)){
                 ...
                 StringBuffer sb = new StringBuffer("");
                 sb.append("<td width=15% class='FieldValue'> \n\r<input type=\"button\"  class=Browser onclick=\"javascript:getrefobj('con"+_fieldid+"_value','con"+_fieldid+"span','"+_refid+"','"+_viewurl+"','0');\"/>");
                 sb.append("\n\r<input type=\"hidden\" name=\"con"+_fieldid+"_value\" value=\""+StringHelper.null2String(fieldvalue)+"\"  "+_style+"  >");
...
<div id="divReftype" style="display:none">
  ...
  <script language="javascript" type="text/javascript">
   ...
   function fillotherselect(elementobj,fieldid,rowindex){
	...
	//var sql ="<%=SQLMap.getSQLString("humres/base/humresBrowsermResult.jsp")%>";
  ...
  function onSubmit(){
       ...
       if(keyfield=='iskeyfieldidflag'){
        getArray(ids,names);
        /* window.parent.returnValue = ids; 缺点没有把names传过去*/
       // window.parent.close();*/
       }else{
        getArray(ids,names);//修改过后的 
        /*  getArray(ids,names);不支持IE10*/ 
       }
      }
   }
   function onClear(){
   ...
   function getrefobj(inputname,inputspan,refid,viewurl,isneed){
    ...
    try{
		...
		if (id[0] != '0') {
			...
			document.getElementsByName(inputspan)[0].innerHTML = id[1];
	    }else{
			document.getElementsByName(inputname)[0].value = '';
			if (isneed=='0')
			document.getElementsByName(inputspan)[0].innerHTML = '';
			else
			document.getElementsByName(inputspan)[0].innerHTML = '<img src=<%=request.getContextPath()%>/images/base/checkinput.gif>';
...
case '1':obj.checked=false;obj.indeterminate=false;obj.flag='0';
    ...
    function getArray(id,value){
        ...
        temp[0] = id;
        temp[1] = value; 
        window.parent.returnValue = temp;/*//这种写法目前在IE10下不支持*/
        window.parent.close();
    }
</script>
  </body>
