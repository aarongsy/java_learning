...
var eWebEditorFrame;	//eWebEditor iframe对象
var eWebEditorWin;	//eWebEditor iframe中的window对象
var contentTableHeight;
...
function document.onreadystatechange(){
	...
	//初始化
	eWebEditorFrame = document.getElementById("eWebEditor");
	eWebEditorWin = eWebEditorFrame.contentWindow;
	
	//初始化内容区域表格的高度
	contentTableHeight = document.getElementById("eWebEditor_Content").offsetHeight;
	...
	eWebEditor.document.body.ondblclick = markSelectedElement ;
...
function markSelectedElement(){
	if (isControlSelected("INPUT")){
	   for(i = 0; i < parent.document.all.length; i++){
	   	if((parent.document.all(i).id).indexOf("_initvaluespan")!=-1){
		...
		for (var i=0; i<selectedRange.length; i++){
			...
			parent.document.all("objformid").value = formid;
			parent.document.all("objformid").fireEvent("onchange");
			if(parent.document.all(fieldid+"_initvalue")!=null){
				if(parent.document.all(fieldid+"_initvaluespan").style.display == "none")
					parent.document.all(fieldid+"_initvaluespan").style.display ="";
			}
			if(fieldid.indexOf("FORM_")==-1) {
				setTimeout("changeFormfield('" + fieldid + "')", 300);
		//	if(parent.document.all("objformid").fireEvent("onchange")){
		//		parent.document.all("objfieldid").value = fieldid;
		//	}
...
function changeFormfield(fieldid){
	parent.document.all("objfieldid").value = fieldid;
	
	if(parent.document.all(fieldid+"_fieldstyle")!=null){
		parent.document.all("fieldstyle").value=parent.document.all(fieldid+"_fieldstyle").value;
		parent.document.all("oTRfield").style.display="";
		parent.document.all("oTRform").style.display="none";
	}else{
		parent.document.all("fieldstyle").value="2";
		parent.document.all("oTRfield").style.display="none";
		parent.document.all("oTRform").style.display="none";
	}
}
	//**end add
...
function onPaste() {
	...
	if (sCurrMode=="EDIT"){
		...
		if(isOpenCodeSyntax){
			getCodeMirrorObj().replaceSelection(clipboardData.getData("Text"));
		}else{
...
function setHTML(html) {
	...
	case "CODE":
		if(isOpenCodeSyntax){
			...
			bEditMode=false;
			eWebEditorFrame.src = "/plugin/ewe/jsp/layoutcode.jsp?" + (new Date()).getTime() + "&contentTableHeight=" + contentTableHeight;
			if (eWebEditorFrame.attachEvent){
				eWebEditorFrame.attachEvent("onload", eWebEditorFrameOnload);
			}else { 
				eWebEditorFrame.onload = eWebEditorFrameOnload;
			}
			function eWebEditorFrameOnload(){
				html = html.ReplaceAll("type=text/javascript","type=\"text/javascript\"");
				html = html.ReplaceAll("<\\?xml","<xml");
				eWebEditor.document.body.innerText=html;
				eWebEditorWin.doCodeMirror();
				setHTMLInnerMethod(false);
				if(eWebEditorFrame.detachEvent){
					eWebEditorFrame.detachEvent("onload", eWebEditorFrameOnload);
				}else{
					eWebEditorFrame.onload = null;
				}
			}
		}else{
	...
	function setHTMLInnerMethod(isBindDblclickEvent){
		...
		if(isBindDblclickEvent){
			eWebEditor.document.ondblclick=new Function("return markSelectedElement();");
		}
...
function setMode(NewMode){
	if (NewMode!=sCurrMode){
		...
		case "CODE":
			if (NewMode=="TEXT"){
				eWebEditor_Temp_HTML.innerHTML = getCodeContent();
				sBody = eWebEditor_Temp_HTML.innerText;
			}else{
				sBody = getCodeContent();
			}
			break;
		case "TEXT":
...
function getCodeContent(){
	if(isOpenCodeSyntax){
		return getCodeMirrorObj().getValue();
	}else{
		return eWebEditor.document.body.innerText;
	}
}

function getCodeMirrorObj(){
	return eWebEditorWin.myCodeMirror;
}