...
function showContextMenu(event){
	...
	var sMenu="";
	//增加右键插入语言标签
	sMenu += getMenuRow("", "parent.insertLanguageLabel()", "label.gif", lang["LanguageLabel"]);
	...
	if (isControlSelected("IMG")){
		...
		//alert(parent.document.all("objformid").value);
		...
		var formid = parent.document.all("forminfoid").value;
		if(fieldid.indexOf("FORM_")==-1) {
			var funNames = "";
			var content = eWebEditor.document.body.innerHTML;
			while(content && content.indexOf("function") != -1){
				var index = content.indexOf("function");
				var content = content.substring(index + 8);
				if(content.indexOf("(") != -1){
					var funName = content.substring(0, content.indexOf("("));
			...
			sMenu += getMenuRow("", "showDialog('dlg_attribute.jsp?forminfoid="+formid+"&fieldid="+fieldid+funNamesParam+"', true)", "formtext.gif", lang["CMenuForm"]);
			sMenu += sMenuHr;
			height += 24;
		}
	}
