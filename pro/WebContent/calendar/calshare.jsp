...
int gridWidth=700;
int gridHeight=330;
String calformid = request.getParameter("calformid");
...
Ext.onReady(function() {
    ...
    calendarstore = new Ext.data.JsonStore({
        ...
        url: '<%=action%>',
        root:'result',
        fields:['recid','subject','description','startdate','enddate','color','priority','parent','resources','requestid','creator','humresid']
    });
 ...
 var grid = new Ext.grid.GridPanel({
            ...
            displayMsg: '<%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd67f5e310002")%> {0} - {1}<%=labelService.getLabelNameByKeyId("402883d934c0fe860134c0fe868d0000")%> / {2}',//显示
    ...
    sm.on('rowselect',function(selMdl,rowIndex,rec ){
        ...
        calendarstore.baseParams.humresid = humresid;
          calendarstore.baseParams.share = '<%=share%>';
           calendarstore.baseParams.calformid='<%=calformid%>';
           calendarstore.baseParams.categoryid = '<%=categoryid%>';
           calendarstore.baseParams.iscalshare = '1';
            calendar.show();
        calendarstore.load();
		selectuserid = humresid;
    });
	...
	var calendar = new Ext.ECalendar({
		...
		sview:{
				...
				listItems: {
					...
					periodFormats:{
						...
						QuarterTPL:  '<tpl for="."><%=labelService.getLabelNameByKeyId("402883d934c111b80134c111b9100000")%>{numperiod}({datestart}-{dateend})</tpl>'//期间
					},
					useStoreColor:false,
		...
		mview:{
			...
			TaskList_tplqTip: new Ext.XTemplate( '<tpl for=".">{starxl}{startval}<br>{endxl}{endval}<br><%=labelService.getLabelNameByKeyId("402881eb0bd712c6010bd7215e7b000a")%>:{creator}<hr color=\'#003366\' noshade>{details}</tpl>' ),//创建人 //0.0.11   
		...
		dview:{
			header:true,
			headerFormat:'Y - F  - d (<%=labelService.getLabelNameByKeyId("402881ee0c765f9b010c768a4f830025")%>l)',//周
			headerButtons: true,
            moreMenuItems:[],
	...
	calendar.viewweek.on({
		'dblClickTaskAllDay':{
				fn: function(task,dxview,calendar) {
                    ...
                    var url = '<%= request.getContextPath()%>/workflow/request/formbase.jsp?requestid=' + requestid+'&iscalshare=1';
	...
	calendar.on({
		...
		'taskDblClick':{
				fn: function (task,dxview,calendar,refviewname){
                    var requestid=task[1];
                     //var url = '<%= request.getContextPath()%>/workflow/request/formbase.jsp?iscalshare=1&requestid=' + requestid;
					 var url = '<%= request.getContextPath()%>/workflow/request/workflow.jsp?from=report&iscalshare=1&requestid=' + requestid +'&shareuserid='+selectuserid;
