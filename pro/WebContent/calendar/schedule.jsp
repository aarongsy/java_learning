...
%>
 ...
 var calendarstore;
<%
 String action=request.getContextPath()+"/ServiceAction/com.eweaver.workflow.request.servlet.FormAction?action=getschedule&categoryid="+categoryid+"&workflowid="+workflowid+"&formid="+formid;
%>
 ...
 var grid = new Ext.grid.GridPanel({
   ...
   <% if(StringHelper.isEmpty(isresource)){%>  //日程
	...
	var calendar = new Ext.ECalendar({
        ...
        <%}else{%>
        title:'<%=labelService.getLabelNameByKeyId("402881eb0bcbfd19010bcc8438f7002c")%>',//资源管理
        <%}%>
        fieldsRefer:{ //0.0.11
			id:'recid',
			subject:'subject',
		...
		mview:{
			...
			ShowMenuItems:[1,1,1,0,0,0],  //0.0.11  - ADD, nextmonth, prevmonth, chg Week , CHG Day, chg Sched,
		...
		wview:{
			...
			ShowMenuItems:[1,1,1,0,0,0],	//0.0.11  add, go next w , go prev w , chg month , chg day, chg sched
		...
		dview:{
		    ...
		    task_clsOver:'test_taskovercss',
			ShowMenuItems:[1,1,1,0,0,0],		//0.0.11 ADD, next day, prev day , chg Month , CHG Week, chg Sched, (for daybody menu)
			task_DDeditable:true, 			    //0.0.11
...
//					prueba.store.commitChanges();
  ...
  <%}else{%>
    ...
    calendarstore.on('exception', function() {
        myMask.hide();
       alert('<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f8001e")%>');//数据加载失败，请检查网络连接是否正常或日程配是否正确!
    });
