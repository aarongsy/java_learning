...
<html>
	...
	<style type="text/css">
      ...
      .x-panel-btns-ct table {width:0}
		...
		<title><%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80013")%></title><!-- 共享日程设置 -->
	...
	<%
		...
		if(list.isEmpty()) {
			out.println(labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80014"));//附加资源中的日程表单没有配置
			return;
		} else {
			Scheduleset scheduleset = (Scheduleset)list.get(0);
			String type = scheduleset.getType();
			if(StringHelper.isEmpty(type)) {
				out.println(labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80015"));//附加资源中的日程表单中的类型字段没有配置
				return;
			} else {
				Formfield formfield = formfieldService.getFormfieldById(type);
				if(formfield == null) {
					out.println(labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80016"));//附加资源中的日程表单中的类型字段配置无效
					return;
				} else {
				...
				<table id="myTable" class=viewform width=100%>
					<tr class=title>
						<td class="FieldName" width=15% nowrap>
							<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80017")%><!-- 日程类型 -->
						</td>
						<td class="FieldValue" width=20% nowrap>
							<select class="inputstyle2"
								...
								<option value="all"  <%=StringHelper.null2String(settingobj.getCalendartype()).equals("all")?"selected":"" %>>
									<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80018")%><!-- 全部日程 -->
								</option>
							</select>
						</td>
					</tr>
					<tr class=title>
						<td class="FieldName" width=15% nowrap>
							<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80019")%><!-- 共享对象 -->
						</td>
						<td width=20% class='FieldValue'>
							<select class="inputstyle2"
								name="shareobj" id="shareobj"
								onchange="changebtn(this.value)">
								<option value="1" <%=StringHelper.null2String(settingobj.getShareobj()).equals("1")?"selected":"" %>>
									<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f8001a")%><!-- 个人 -->
								</option>
								<option value="2" <%=StringHelper.null2String(settingobj.getShareobj()).equals("2")?"selected":"" %>>
									<%=labelService.getLabelNameByKeyId("402881e510e569090110e56e72330003")%><!-- 岗位 -->
								</option>
								...
								<option value="4" <%=StringHelper.null2String(settingobj.getShareobj()).equals("4")?"selected":"" %>>
									<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f8001c")%><!-- 所有人 -->
								</option>
							</select>
						...
						<td class="FieldName" width=15% nowrap>
							<%=labelService.getLabelNameByKeyId("402881e70b774c35010b774cceb80008")%><!-- 安全级别 -->
						</td>
						<td>
					...
					<tr class=title>
						<td class="FieldName" width=15% nowrap>
							<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f8001b")%><!-- 共享级别 -->
							<td width=15% class='FieldValue'>
								<select class="inputstyle2"
									id="sharelevel"
									name="sharelevel">
									<option value="1" <%=StringHelper.null2String(settingobj.getSharelevel()).equals("1")?"selected":"" %>>
										<%=labelService.getLabelNameByKeyId("402881eb0bcd354e010bcd7a2f730012")%><!-- 查看 -->
									</option>
									<option value="2" <%=StringHelper.null2String(settingobj.getSharelevel()).equals("2")?"selected":"" %>>
										<%=labelService.getLabelNameByKeyId("402881e70b774c35010b7750a15b000b")%><!-- 编辑 -->
									</option>
								</select>
							</td>
						</td>
	...
	function onSubmit() {
		var shareobj = document.getElementById('shareobj').value;
		var shareobjid = document.getElementById('shareobjid').value;
		if(shareobj=='1'||shareobj=='2'||shareobj=='3'){
			if(shareobjid==''){
				Ext.Msg.buttonText={ok:'<%=labelService.getLabelNameByKeyId("402881eb0bcbfd19010bcc6e71870022")%>'};//确定
                Ext.MessageBox.alert('', '<%=labelService.getLabelNameByKeyId("402883373633d671013633d6727d0291")%>');//请选择共享对象！
				return;
			}
		}
		EweaverForm.submit();
