	...
	<%
		if(list!=null&&list.size()>0){
			for(int i=0;i<list.size();i++){
				WorkflowVersion workflowVersion=(WorkflowVersion)list.get(i);
				%>
				<td>
					<iframe id="iframe<%=i%>" scrolling="auto" width="<%=divwidth%>" height="<%=divheight%>" style="border-style:inset;display:block;" src="<%=request.getContextPath()%>/wfdesigner/viewers/showgraph.jsp?workflowid=<%=workflowVersion.getWorkflowid()%>&width=<%=divwidth%>&height=<%=divheight%>"></iframe>
				</td>
				<%
			}
		}
