...
<html>
	...
	<body>
		...
		<FORM NAME=EweaverForm STYLE="margin-bottom: 0"
			action="<%=request.getContextPath()%>/ServiceAction/com.eweaver.workflow.report.servlet.ReportAction?action=search"
			method="post">

			<input type=hidden name=reportid value="<%=formid%>">

			<b><%=labelService.getLabelNameByKeyId("402883d934d152ce0134d152cf290039") %><!-- 原报表条件 -->： <input type=text class=inputstyle
					value="<%=StringHelper.null2String(condition)%>" size=125 readonly>
			</b>
			<br />
			<b> <%=labelService.getLabelNameByKeyId("402883d934d152ce0134d152cf29003a") %><!-- 新报表条件 -->： </b>
			<br />
			...
			<table width=100% class=viewform id="<%=objtablename%>"
				...
				<B>
					<tr>
						<%=labelService.getLabelNameByKeyId("表单名称") %><!-- 表单名称 -->：<%=formname%>
					</tr> </B>
				...
				<tr class=title>
					...
					<td class="FieldName"><%=StringHelper.null2String(label)%></td>
...
sub onShowMultiPropValue(objval,inputname)
	...
	id1 = openDialog("<%=request.getContextPath()%>/systeminfo/BrowserMain.jsp?url=<%=request.getContextPath()%>/props/data/PropValueBrowser.jsp?sqlwhere= where propid="&objval&"&resourceids="&tmp)
...
sub getdate(inputname,spanname,isneed)
	returnvalue = openDialog("<%=request.getContextPath()%>/plugin/calendar.jsp",,"dialogHeight:320px;dialogwidth:275px")
...
sub gettime(inputname,spanname,isneed)
	returnvalue = openDialog("<%=request.getContextPath()%>/plugin/clock.jsp",,"dialogHeight:320px;dialogwidth:275px")
...
//	DataService.getValues(createList(fieldcheck,rowindex),"select id,objname from selectitem where pid = '"+elementvalue+"'");
	var sql ="<%=SQLMap.getSQLString("workflow/workflow/exportbrowser.jsp")%>";
