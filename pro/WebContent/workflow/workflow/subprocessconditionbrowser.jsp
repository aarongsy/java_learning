...
<%@ page import="com.eweaver.workflow.subprocess.model.SubprocesssetConditionDetail"%>
<%
    ...
    String subprocesssetid = StringHelper.null2String(request.getParameter("subprocesssetid"));
    
    String action = request.getContextPath() + "/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=getconditiondetail&subprocesssetid="+subprocesssetid;
    String selaction= request.getContextPath() + "/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=getselstore";
    ...
    Subprocessset subprocessset = subprocesssetService.getSubprocessset(subprocesssetid);
    String listrefstr="";
    String listselstr="";
    for(SubprocesssetConditionDetail detail:subprocessset.getConditionDetails()){
        ...
        Refobj refobj=refobjService.getRefobj(formfield.getFieldtype());
            ...
            String _viewfield = StringHelper.null2String(refobj.getViewfield());
          	String sql = "select " + _keyfield + " as objid," + _viewfield + " as objname from " + _reftable + "";
                ...
                for(int k=0;k<listval.size();k++){
                    ...
                    if(StringHelper.isEmpty(objid)&&StringHelper.isEmpty(objname)){
                        ...
                        Map valmap = (Map) listvalobjname.get(0);
                         objname=StringHelper.null2String((String)valmap.get(_viewfield));
                    }else{
                          objid+=","+id;
                        Map valmap = (Map) listvalobjname.get(0);
                         objname+=","+StringHelper.null2String((String)valmap.get(_viewfield));
     ...
     stripeRows: true,
      ...
      tbar: [{
          ...
          handler : function(){
              ...
              if(active){
                    Ext.Ajax.request({
              url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=validatedelete',
                        sync:true,
         ...
         handler : function(){
             location.href='<%=request.getContextPath()%>/workflow/workflow/subprocessconditionbrowser.jsp?formid=<%=formid%>&subprocesssetid=<%=subprocesssetid%>' ;
             }
      }],
     listeners : {
       ...
       grid.on("cellclick",function (grid, rowIndex, columnIndex, e) {
        var record = grid.store.getAt(rowIndex);
           if(columnIndex==0)
           {
               ...
               if(pid!=''){
                 Ext.Ajax.request({
              url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=validatefirst',
              params:{pid:pid},
              success: function(res) {
           ...
           if(columnIndex==2){
            ...
            if(htmltype==1){
                ...
                }else if(fieldtype==2||fieldtype==3||fieldtype==4||fieldtype==5){  //整数  浮点数 日期 时间
                      var store2=new Ext.data.SimpleStore({
                      id:0,
                      fields:['value', 'text'],
                      data: [['1','<%=labelService.getLabelNameByKeyId("402883d934d152ce0134d152cf28002f") %>'],['2','<%=labelService.getLabelNameByKeyId("402883d934d152ce0134d152cf29003c") %>'],['3','<%=labelService.getLabelNameByKeyId("402883d934d152ce0134d152cf28002c") %>'],['4','<%=labelService.getLabelNameByKeyId("402883d934d152ce0134d152cf29003d") %>'],['5','<%=labelService.getLabelNameByKeyId("402883d934cf81d90134cf81da0a002d") %>'],['6','<%=labelService.getLabelNameByKeyId("402883d934cf81d90134cf81da0a002e") %>']]
      ...
      function rightClickFn(grid, rowindex, e) {
          ...
          var flagall=false;
               Ext.Ajax.request({
              url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=validateaddgroup',
                   sync:true,                   
              params:{selectedids:selected.toString(),pids:selectedpid.toString(),subprocesssetid:'<%=subprocesssetid%>'},
              success: function(res) {
      ...
      function rMenu1Fn(item, checked) {
          ...
          for (i = 0; i < records.length; i++) {
            ...
            var jsonstr = Ext.util.JSON.encode(rowindexs);
                   Ext.Ajax.request({
              url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=removegroup',
              params:{jsonstr:jsonstr,selectedids:selected.toString(),subprocesssetid:'<%=subprocesssetid%>'},
              success: function(res) {
                  location.href='<%=request.getContextPath()%>/workflow/workflow/subprocessconditionbrowser.jsp?formid=<%=formid%>&subprocesssetid=<%=subprocesssetid%>' ;
      ...
      function addgroup(){
          ...
          var jsonstr = Ext.util.JSON.encode(rowindexs);
             Ext.Ajax.request({
              url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=creategroup',
              params:{jsonstr:jsonstr,selectedids:selected.toString(),subprocesssetid:'<%=subprocesssetid%>'},
              success: function(res) {
                  location.href='<%=request.getContextPath()%>/workflow/workflow/subprocessconditionbrowser.jsp?formid=<%=formid%>&subprocesssetid=<%=subprocesssetid%>' ;
              }
...
window.onbeforeunload =  function (e){
  ...
  <body>
  <%
  		String Condition = StringHelper.null2String(subprocessset.getCondition());
  %>
  <input type=hidden value="<%=Condition%>" name="oldcondition">
  <div id="divSearch">
  ...
  <script type="text/javascript">
   function onSubmit() {
       ...
       return;
          ...
          Ext.Ajax.request({
              url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=modifyexportdetail',
              params:{jsonstr:jsonstr,id:'<%=subprocesssetid%>',deletedstr:deleted.toString()},
              success: function(res) {
      ...
      function validatecondition(){
        var myMask = new Ext.LoadMask(Ext.getBody());
             myMask.show();
              Ext.Ajax.request({
              url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=makecondition',
              params:{subprocesssetid:'<%=subprocesssetid%>',formid:'<%=formid%>',checked:'1'} ,
                    success: function(res) {
                        myMask.hide();
      ...
      function makecondition(){
          ...
          var myMask = new Ext.LoadMask(Ext.getBody());
                      ...
                      Ext.Ajax.request({
                      url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.workflow.subprocess.servlet.SubprocessAction?action=makecondition',
                      params:{subprocesssetid:'<%=subprocesssetid%>',formid:'<%=formid%>'},
                      success: function(res) {
                           myMask.hide();
      ...
      var win;
       function makeconditionuser(){
		...
		var tmpTpl = new Ext.Template([
			'<div style="margin:5px"><center><br><table style="width:450px"><td class="value"><textarea name="conditions" cols=65 rows=10 >{oldcondition}</textarea></td></tr>',
