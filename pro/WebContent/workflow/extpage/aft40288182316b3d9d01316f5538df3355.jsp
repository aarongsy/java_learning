...
<%@page import="com.eweaver.interfaces.hessian.HessianService"%>
...
if(operatemode.equals("submit")){
    ...
    String sql="select  giftname,whname,belongdept,applyno,subtotal from uf_gift_receive_dt  where  requestid='"+requestid+"'";
    ...
    for(int i=0;i<list.size();i++){
	    ...
	    String SqlUpdate="update uf_gift_store set storeno=(storeno-"+applyno+"),total=(total-"+subtotal+"),numout=(nvl(numout,0)+"+applyno+") where id='"+giftname+"' and  whname='"+whname+"' and bdept='"+belongdept+"' ";
 ...
 String selectstore="select a.bdept,a.giftname,sum(storeno) as sumstoreno,b.safeline as sumsafeline from uf_gift_store a,uf_gift_safe_dt b where a.bdept=b.belongdept and a.giftname=b.giftname and a.id='"+giftname+"' and a.bdept='"+belongdept+"' group by a.bdept,a.giftname,b.safeline";   
    ...
    baseJdbc.update(SqlInsert);
		...
		hs.createStoreOutRequest(requestid);
    }
}
