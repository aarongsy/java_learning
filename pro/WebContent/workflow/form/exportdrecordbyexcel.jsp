...
<%
    pagemenustr +="addBtn(tb,'"+labelService.getLabelNameByKeyId("402883d934c05ea90134c05ea9a60000")+"','E','accept',function(){exportRecords()});";//导出
...
var pbar1;
    Ext.onReady(function(){
     ...
     var c = new Ext.Panel({
               title:'<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39330032") %>',iconCls:Ext.ux.iconMgr.getIcon('config'),//导出操作
               layout: 'border',
     ...
     addTab(contentPanel,'<%=request.getContextPath()%>/workflow/form/importdrecordbyexcel.jsp?requestid=<%=requestid%>&workflowid=<%=workflowid%>&categoryid=<%=categoryid%>&layoutid=<%=layoutid%>','<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39330037") %>','page_portrait');//导入操作
            ...
            <select id="form" name="form">
                <%
                    ...
                    if(workflowid!=null&&!workflowid.equals("")){
                        ...
                        f=forminfoDao.getForminfoById(workflowinfo.getFormid());
                        ...
                        f=forminfoDao.getForminfoById(category.getFormid());
                    }
                    ...
                    while(itObj.hasNext()){
                        ...
                        if(StringHelper.null2String(f.getObjtablename()).equals(form.getObjtablename())){
                            continue;
                        }
...
//            document.body.removeChild(msgObj);
    ...
    function doOutPutHumresExcel(){
        ...
        <%
            if(workflowid!=null&&!workflowid.equals("")){
        %>
        UsersWorkflowExcelService.outputExcelDetail(document.getElementById("form").value,'<%=requestid%>',returnTotal);
        <%
            }else if(categoryid!=null&&!categoryid.equals("")){
        %>
        UsersCategoryExcelService.outputExcelDetail(document.getElementById("form").value,'<%=requestid%>',returnTotal);
        <%
            }
        %>
