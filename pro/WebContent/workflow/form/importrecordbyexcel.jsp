...
.helpContainer{padding:2px;font-size:9pt;margin:10px;border:0px solid #3300FF;}
  ...
  <script type="text/javascript" src="<%=request.getContextPath()%>/js/ext/ux/TreeCheckNodeUI.js" ></script>
   ...
   <script type="text/javascript" src="<%=request.getContextPath()%>/js/ext/ux/iconMgr.js"></script>
      ...
      <%
        ...
        String trstyle="display:none;";
        if(StringHelper.isEmpty(workflowid)&&StringHelper.isEmpty(categoryid)){
        	trstyle="";
        }
      %>
...
<form action="<%=request.getContextPath()%>/ServiceAction/com.eweaver.workflow.workflow.servlet.WorkflowactingAction?action=create" name="EweaverForm" id="EweaverForm" method="post" target="upload_faceico" enctype="multipart/form-data">
        ...
        <table style="border:0">
            ...
            <TR><TD class=Spacing colspan=3 align="center"><%=labelService.getLabelNameByKeyId("402883d934ca4ec50134ca4ec7190070") %><!-- 请在下面选择需要导入的excel文档 --></TD><TR>
            <TR><TD class=Line colspan=3></TD><TR>
            <TR><TD colspan=3>&nbsp;</TD><TR>
            <TR><TD colspan=3 align="center"><input type="file" name="path" /></TD><TR>
            <TR style="<%=trstyle%>"><TD colspan=3>
            <div class="helpContainer">
     		<ul>
     			<li class="title">说明</li>
     			<li>导出的excel中的主键编号，所在部门，所在岗位，工作类型，人事状态，登录账号，登录密码，账号状态，是否删除，安全级别这几列的顺序不能更改</li>
     			<li>所在部门、所在岗位不能为空，如果部门和岗位在数据库中有相同的值 则在excel写入需要导入的部门id或者岗位id即可</li>
     			<li>多选browser框，下拉框类型，单选browser框 excel中的数据支持三种格式为 id,汉字_id,名称 其中如果数据库里面有2条或者2条以上的相同名称的数据 那么在excel中browser框列中写汉字是没法导入的 此时需要通过写id才可以实现</li>
     			<li>登录账号、员工号可以为空但不能重复</li>
     		</ul>
     		</div></TD><TR>
            <tr><td colspan=3 align="center">
                ...
                <div id="importMessage" style="display:none;">
                    ...
                    <a href="javascript:void(0)"><%=labelService.getLabelNameByKeyId("402883d934cfcad30134cfcad45a0031") %>: <span id="lostNum">0</span></a>&nbsp;&nbsp;<a id="lostRecord" href="javascript:downloadLostRecord()"></a>
                </div>
                </div>
                </div>
                <br/>
