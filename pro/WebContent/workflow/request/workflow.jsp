...
<%
    ...
    RequestOperatorService requestOperatorService=(RequestOperatorService)BaseContext.getBean("requestOperatorService");
  	//是否启用附件大小控件检测
	...
	String reftabid = StringHelper.null2String(request.getParameter("reftabid")).trim();
	...
	//处理权限
	//是否督办
	boolean issupervise = permissiondetailService.checkOpttype(requestid,13);
...
//公文盖章 时没有取相对路径 
	...
	String tempdir1 = "";
	String dir = props.getProperty("user.home");
	...
	int nodeAttachCanEdit=NumberHelper.string2Int(nodeinfo.getAttachCanEdit(),0);
	boolean isCurrentNodeUser=requestOperatorService.isCurrentNodeUser(requestid,nodeid,eweaveruser.getId());
	//附件是否可编辑
	int attachCanEdit=0;
	if(nodeAttachCanEdit==1&&isCurrentNodeUser){
		attachCanEdit=1;
	}
	...
	if(bNewworkflow.equals("1")){
		...
		if(!isauth){
			String byagentid = permissiondetailService.getRealPermissionAgentHumres(nodeid);			
			if(byagentid.equals("")){
				response.sendRedirect("/nopermit.jsp");
				return;
			}
	...
	if(isfinished == 1){//流程结束 ，肯定是 不能存在退回 
	   canreject = false;
	}
...
****/
	...
	Map selRemindMap = logService.getSelRemaindHistory(requestid,nodeid,currentuser.getId());
	int isemail = 0;
	int issms = 0;
	int ispopup = 0;
	int isrtx = 0;
	if(bRtx){
		isrtx = NumberHelper.string2Int(selRemindMap.get("selrtx"),0);
	}
	if(bSMS){
		issms = NumberHelper.string2Int(selRemindMap.get("selsms"),0);
	}
	if(bEmail){
		isemail = NumberHelper.string2Int(selRemindMap.get("selemail"),0);
	}
	if(bRtx||bSMS||bEmail){
		ispopup = NumberHelper.string2Int(selRemindMap.get("selpopup"),0);
	}
	StringBuffer remindBuf = new StringBuffer("");
	...
	//先查看节点提醒配置
	String nodeRemindtype = StringHelper.null2String(nodeinfo.getRemindtype());
	if(!bView.equals("1")){//显示布局不显示
	if(StringHelper.isEmpty(nodeRemindtype)){//继承流程提醒
		if((StringHelper.null2String(workflowinfo.getRemindtype()).equals("4028819d0e521bf9010e5238bec2000d")) && (bRtx || bSMS || bEmail)) {
		 pagemenustr += "addBtn(tb,'<span ext:qtip="+labelService.getLabelName("402883d93ebfd597013ebfd59b58029d")+">"+labelService.getLabelName("402883d93ebfd597013ebfd59b58029d")+"</span>','G','clock_edit',function(){showSelRemind(this);});";
	     String checkedStr = "";
	     if(NumberHelper.getIntegerValue(workflowinfo.getSelemail())==1 && bEmail){
	    	checkedStr = "";
	     	if(isemail==1) checkedStr = "checked";
	     	remindBuf.append("<input type='checkbox'  name='isemail_cbox' id='isemail_cbox' "+checkedStr+"  value='0' onClick=\"javascript:setRemindtype('isemail')\">邮件提醒");
	     }
	     if(NumberHelper.getIntegerValue(workflowinfo.getSelsms())==1 && bSMS){
	     	checkedStr = "";
	     	if(issms==1) checkedStr = "checked";
	     	if(!remindBuf.toString().equals("")){
	     		remindBuf.append("<br />");
	     	}
	     	remindBuf.append("<input type='checkbox'  name='issms_cbox' id='issms_cbox' "+checkedStr+"  value='0' onClick=\"javascript:setRemindtype('issms')\">短消息提醒");
	     }
	     if(NumberHelper.getIntegerValue(workflowinfo.getSelpopup())==1){
	     	checkedStr = "";
	     	if(ispopup==1) checkedStr = "checked";
	     	if(!remindBuf.toString().equals("")){
	     		remindBuf.append("<br />");
	     	}
	     	remindBuf.append("<input type='checkbox'  name='ispopup_cbox' id='ispopup_cbox' "+checkedStr+"  value='0' onClick=\"javascript:setRemindtype('ispopup')\">弹出式提醒");
	     }
	     if(NumberHelper.getIntegerValue(workflowinfo.getSelrtx())==1 &&bRtx){
	     	checkedStr = "";
	     	if(isrtx==1) checkedStr = "checked";
	     	if(!remindBuf.toString().equals("")){
	     		remindBuf.append("<br />");
	     	}
	     	remindBuf.append("<input type='checkbox'  name='isrtx_cbox' id='isrtx_cbox' "+checkedStr+"  value='0' onClick=\"javascript:setRemindtype('isrtx')\">即时通讯提醒");
	     }
    }}else if(nodeRemindtype.equals("4028819d0e521bf9010e5238bec2000d") && (bRtx || bSMS || bEmail)){//节点设置选择提醒
    	pagemenustr += "addBtn(tb,'<span ext:qtip="+labelService.getLabelName("402883d93ebfd597013ebfd59b58029d")+">"+labelService.getLabelName("402883d93ebfd597013ebfd59b58029d")+"</span>','G','clock_edit',function(){showSelRemind(this);});";
    	String checkedStr = "";
	     if(NumberHelper.getIntegerValue(nodeinfo.getSelemail())==1 && bEmail){
	    	checkedStr = "";
	     	if(isemail==1) checkedStr = "checked";
	     	remindBuf.append("<input type='checkbox'  name='isemail_cbox' id='isemail_cbox' "+checkedStr+"  value='0' onClick=\"javascript:setRemindtype('isemail')\">邮件提醒");
	     }
	     if(NumberHelper.getIntegerValue(nodeinfo.getSelsms())==1 && bSMS){
	     	checkedStr = "";
	     	if(issms==1) checkedStr = "checked";
	     	if(!remindBuf.toString().equals("")){
	     		remindBuf.append("<br />");
	     ...
	     if(NumberHelper.getIntegerValue(nodeinfo.getSelpopup())==1){
	     	checkedStr = "";
	     	if(ispopup==1) checkedStr = "checked";
	     	if(!remindBuf.toString().equals("")){
	     		remindBuf.append("<br />");
	     ...
	     if(NumberHelper.getIntegerValue(nodeinfo.getSelrtx())==1 &&bRtx){
	     	checkedStr = "";
	     	if(isrtx==1) checkedStr = "checked";
	     	if(!remindBuf.toString().equals("")){
	     		remindBuf.append("<br />");
    ...
    if(!nodeIsFree && !bNotify){
		...
		if("1".equals(isForceFinish)&& !StringHelper.isEmpty(requestid)){
			if(!StringHelper.isEmpty(nodeinfo.getIsForceFinishText())){
				pagemenustr +="addBtn(tb,'<span ext:qtip="+labelService.getLabelName("强制归档流程")+">"+StringHelper.null2String(nodeinfo.getIsForceFinishText())+"</span>','Q','arrow_turn_right',function(){forceFinishFlow()});";
	
			}else{
				pagemenustr +="addBtn(tb,'<span ext:qtip="+labelService.getLabelName("强制归档流程")+">"+labelService.getLabelNameByKeyId("4028832e3eef1b51013eef1b524c0277")+"</span>','Q','arrow_turn_right',function(){forceFinishFlow()});";
			}
		}
	}
    
	
	if(canfree && StringHelper.isID(requestid) && isfinished==0 && !bNotify){
...
if(isfinished ==1 && !editmode.equals("1") && righttype%165 == 0 &&!StringHelper.isEmpty(objid) && !StringHelper.isEmpty(objtable)){
   if(nodeinfo.getSharepmethod().intValue()==0){
	   tabStr +="addTab(contentPanel,'/base/security/addpermission.jsp?objtable="+objtable+"&istype=0&objid="+objid+"&nodeid="+nodeid+"','"+labelService.getLabelNameByKeyId("402881ed0c29ccef010c2a9592ac0019")+"','script_key');";

   }else{
	   pagemenustr +="addBtn(tb,'"+labelService.getLabelNameByKeyId("402881ed0c29ccef010c2a9592ac0019")+"','D','script_key',function(){onUrl('/base/security/addpermission.jsp?objtable="+objtable+"&istype=0&objid="+objid+"&nodeid="+nodeid+"','"+labelService.getLabelNameByKeyId("402881ed0c29ccef010c2a9592ac0019")+"','tab"+nodeinfo.getId()+"')});";
...
if(isfinished ==1 && !editmode.equals("1") &&!StringHelper.isEmpty(objid) && !StringHelper.isEmpty(objtable) ){
      if(nodeinfo.getPlistmethod().intValue()==0){
          ...
          pagemenustr +="addBtn(tb,'"+labelService.getLabelName("权限列表")+"','L','script_go',function(){onUrl('/ServiceAction/com.eweaver.base.security.servlet.PermissiondetailAction?objtable="+objtable+"&objid="+objid+"','"+labelService.getLabelName("权限列表")+"','tab"+nodeinfo.getId()+"')});";
...
if(!viewmode.equals("1")){
    ...
    if(graphmode.equals("0")){
        pagemenustr += "addBtn(tb,'"+labelService.getLabelName("流程图")+"','F','application_view_icons',function(){onUrl('/workflow/request/workflowchart.jsp?workflowid="+workflowid+"&requestid="+requestid+"','"+labelService.getLabelName("流程图")+"','tab"+nodeinfo.getId()+"')});";
    }else{
        pagemenustr += "addBtn(tb,'"+labelService.getLabelName("流程图")+"','F','application_view_icons',function(){onUrl('/wfdesigner/viewers/graphviewer.jsp?workflowid="+workflowid+"&requestid="+requestid+"','"+labelService.getLabelName("流程图")+"','tab"+nodeinfo.getId()+"')});";
...
if(!viewmode.equals("1")&&optlevel != 0){ //流程流转按钮
    if(nodeinfo.getPflowmethod().intValue()==1){
        pagemenustr += "addBtn(tb,'"+labelService.getLabelName("流程流转")+"','F','application_xp',function(){onUrl('/workflow/request/pflowdisplay.jsp?requestid="+requestid+"','"+labelService.getLabelName("流程流转")+"','tab"+nodeinfo.getId()+"')});";
...
//是否加载签章控件
boolean showHtmlSignature=showHtmlSignatureBtn||hasHtmlSignature;
if(showHtmlSignatureBtn){
	pagemenustr += "addBtn(tb,'"+labelService.getLabelName("电子签章")+"','K','user_key',function(){DoSignature()});";
}
//****************************流转公文下载菜单(start)*****************************//
//****************************流转公文下载菜单(end)*****************************//
...
String sqlmove="from Stampinfo where requestid='"+requestid+"' and nodeid='"+nodeid+"' and userid='"+eweaveruser.getId()+"'";
...
if(inOpenedInWindow){	//如果流程页面是在弹出窗体中打开的,那么此时将不使用网站模式
	currentSysModeIsWebsite = false;
}
...
//布局中受电子签章保护字段
String formfields=formlayoutfieldService.getHtmlSignatureProtectedFormfield(layoutid);
...
<link rel="stylesheet" type="text/css" id="workflow_css" href="<%=currentSkin.getBasePath() %>/<%=SkinConstant.WORKFLOW_CSS_NAME %>"/> 
<% } %>
...
#tabPanel .x-panel-tbar .x-toolbar{
	overflow-x: auto; 
}
#tabPanel .x-panel-tbar .x-toolbar-ieCompatibilityViewOverflow{	/*兼容性视图总toolbar超出时对应的样式*/
	+height: 40px;
}
</style>
...
//设置流程访问次数
function setWorkflowCount(){
	Ext.Ajax.request({
            url:"/ServiceAction/com.eweaver.workflow.workflow.servlet.WorkflowinfoAction?action=addtotalnum&workflowid=<%=workflowid%>",
            sync:false
        });
}
...
Ext.onReady(function(){
	...
	commentstore.on('load',function(commentstore,recs){
		...
		if (id!=null && id.length!=0){
			...
			html=html+'<th width="20%"><%=labelService.getLabelNameByKeyId("402883d934d152ce0134d152cf260002") %></th>';//操作时间
			html=html+'<th width="8%" ><%=labelService.getLabelNameByKeyId("402881eb0c9fadb1010c9fd1a069000e") %></th>';//操作
			html=html+'<th width="20%" style="border-right:solid 1px #ebe7d1;"><%=labelService.getLabelNameByKeyId("402883d934d152ce0134d152cf260003") %></th>';//接收
			html=html+'</tr>';
			for(var i = 0; i < recs.length; i++)
			{
				var oper=recs[i].get('nextopername');
				...
				//		oper+=opers[v]+",";
				//	}
				//	oper=oper.substr(0,oper.length-1)+'<input type=button value=显示所有 onclick=javascript:document.all.opers_'+i+'.innerHTML="'+recs[i].get('nextopername')+'">';
				//}
				
				oper="<a style='cursor:pointer;' onclick=javascript:document.all.opers_"+i+".innerHTML='"+recs[i].get('nextopername')+"'>显示</a>";
				
				var td_id='res'+i;
				html=html+'<tr>';  
				html=html+'<td align=left style="border-left:solid 1px #ebe7d1">'+recs[i].get('point')+'</td>'; 
				html=html+'<td align=left style="word-break:break-all;">'+recs[i].get('message')+'</td>'; 
				html=html+'<td align=left>'+recs[i].get('operator')+'  '+recs[i].get('Datatime')+'</td>';
				html=html+'<td align=left>'+recs[i].get('opertype')+'</td>'; 
				html=html+'<td id=opers_'+i+' align=left style="border-right:solid 1px #ebe7d1;word-break :break-all;">'+oper+'</td>'; 
			...
			<% if(currentSysModeIsWebsite){ //系统模式为网站模式时,在异步加载完签字意见之后重置页面高度%>
				resetHeight();
			<% } %>
		}
	});	
    ...
    if(layoutTabs.length==0){//如果没有自定义Tab页,则默认添加
    	<% if(currentSysModeIsWebsite){ //网站模式%>
			layoutTabs.push({title:"",tbar:['-'],autoScroll:false,contentEl:'tab1'});
		<% }else{ %>
	...
	//tb.add(btn);
	<%}else{
		if(optlevel != 1 && isfinished ==0&&!bNotify&&!StringHelper.isEmpty(requestid)) {
		isstamp=true;
		%>
		tb.add(btn);
	<%}}}}%>
	<%
	if(!StringHelper.isEmpty(workflowinfo.getHelpdoc())){
		Docbase docbase=docbaseService.getDocbase(workflowinfo.getHelpdoc());
	%>
		tb.add('->');
		tb.add("<span onclick=\"javascript:onUrl('/document/base/docbaseview.jsp?id=<%=docbase.getId()%>','<%=docbase.getSubject()%>','tab<%=docbase.getId()%>')\" title='<%=labelService.getLabelNameByKeyId("402881ee0c715de3010c725dd26f00a8") %>' style='cursor: hand;'><img src='/images/silk/help.gif'/></span>");
	<%
	}
	%>
	tb.add('->');//无论是否显示流程编号，都将此占位元素显示出来(兼容IE10去掉样式.xtoolbar table{width:0})
	...
	%>
		tb.add('<span class=formFlowNo><%=workflowtitle%> : <%=flowno%></span>'); 
	<%}}%>
	...
	dlg0 = new Ext.Window({
		...
		plain: true,
		modal :true,
		<% if(currentSysModeIsWebsite){ //系统模式为网站模式,调整纵轴的位置,居中显示的话可能因为页面太长而无法在可视区域显示出来%>
		y: 100,
		<% } %>
		width:400,
		height:300,
	...
	<% if(currentSysModeIsWebsite){ //系统模式为网站模式%>
		resetHeight();
		jQuery("#layoutFrame").resize(function(){
			resetHeight();
		});
	<% } %>
	...
	if(isIECompatibilityView()){	//IE兼容性视图
		var $theTB = jQuery("#tabPanel .x-panel-tbar .x-toolbar");
...
function resetHeight(){
	try {
		var height = Ext.get('tab1').getHeight()+70;
		height = Math.max(height,620);
		contentPanel.setHeight(height);
		jQuery(document.body).height(height);
...
var formfields="<%=StringHelper.null2String(formfields)%>";
//选择提醒
...
//选择提醒
function showSelRemind(btn){
	var $ = jQuery;
	var selRemindDiv = $("#selRemindDiv");
	var scrollDiv = $("#tab1").parent();//滚动条在div上
  	scrollDiv.scroll(function() {
 		selRemindDiv.slideUp();
		selRemindDiv.removeClass("selRemindDivShow");
		selRemindDiv.addClass("selRemindDivHide");
	});
	if(selRemindDiv.hasClass("selRemindDivHide") ){//隐藏时
		selRemindDiv.removeClass("selRemindDivHide");
		selRemindDiv.addClass("selRemindDivShow");
		selRemindDiv.css("z-index",999);
		var p = $(Ext.getDom(btn).container.dom).position();
		
		var top = scrollDiv.scrollTop();
		selRemindDiv.css("top",p.top+top);
		selRemindDiv.css("left",p.left);
		selRemindDiv.slideDown();
	}else{//显示时
		selRemindDiv.slideUp();
		selRemindDiv.removeClass("selRemindDivShow");
		selRemindDiv.addClass("selRemindDivHide");
...
<body  onload="getstamp();" <%if(isDocFlow){%>onunload="unLoadWebOffice();setDocexclusion();" <%}%>>
...
<input type="hidden" id="inOpenedInWindow" name="inOpenedInWindow" value="<%=inOpenedInWindow %>">
<input type="text" style="display:none"> 
<%--***********************如果该流程是从项目管理中任务新建流程创建的，给它传递若干个参数start****************************--%>
<% 	
		String messageKey = StringHelper.null2String(request.getParameter("messageKey"));
		if(messageKey.equals("flowMessage") || messageKey.equals("taskMessage")){
	%>
			<input type="hidden" name="messageKey" value="<%=messageKey%>" />
			<input type="hidden" name="taskid" value="<%=StringHelper.null2String(request.getParameter("taskid"))%>" />
			<input type="hidden" name="projectid" value="<%=StringHelper.null2String(request.getParameter("projectid"))%>" />
			<input type="hidden" name="objtype" value="<%=StringHelper.null2String(request.getParameter("objtype"))%>" />
	<%  } %>
<%--***********************如果该流程是从项目管理中任务新建流程创建的，给它传递若干个参数end******************************--%>
<div id="dialog0" style="display:none">
	...
	<%
	String rejectnodeid = "";
	Nodeinfo currentNode = nodeinfoService.get(nodeid);
	String strNodeReject = StringHelper.null2String(currentNode.getRejectnode());

	if(canreject && !"".equals(strNodeReject) && strNodeReject.length()>32){
		List listNodeReject = StringHelper.string2ArrayList(strNodeReject, ",");
		String strNodeThrough = dataService.getValues2String("select distinct nodeid from requeststep where requestid='"+requestid+"'","nodeid");
		List listNodeThrough = StringHelper.string2ArrayList(strNodeThrough, ",");
		
		List<String> autoflowNodeidList = ws.getFlowedNodeidsWidthAutoflow(requestid, currentNode.getId());
		for(String autoflowNodeid : autoflowNodeidList){
			boolean flag = true;
			for(int i=0;i<listNodeThrough.size();i++){
				if(listNodeThrough.get(i).equals(autoflowNodeid)){
					flag = false;
					break;
				}
			}
			if(flag){
				listNodeThrough.add(autoflowNodeid);
			}
		}
		
		listNodeThrough.retainAll(listNodeReject);
		boolean bNodeRejectChecked = true;
		for(int i=0;i<listNodeThrough.size();i++){
			Nodeinfo _nodeinfo = nodeinfoService.get(""+listNodeThrough.get(i));
			String _thisnodeid = StringHelper.null2String(_nodeinfo.getId());
			if(_thisnodeid.equals(currentNode.getId())){	//忽略当前节点
				continue;
			}
			if(bNodeRejectChecked){
				rejectnodeid = _thisnodeid;
			}
	%>
	<tr>
		<td>
			<input type="radio" value="<%=_thisnodeid%>" id="rejectednode1" name="rejectednode1" <%if(bNodeRejectChecked){%> checked <%}%> onclick="javascript:document.getElementById('rejectednode').value='<%=_thisnodeid%>';"><%=StringHelper.null2String(_nodeinfo.getObjname())%>
		</td>
	</tr>
		
	<%		bNodeRejectChecked = false;
		}
	}else if(canreject && rejectnodelist != null){
		String strNodeThrough = dataService.getValues2String("select distinct nodeid from requeststep where requestid='"+requestid+"'","nodeid");
		List listNodeThrough = StringHelper.string2ArrayList(strNodeThrough, ",");
		...
		for(String autoflowNodeid : autoflowNodeidList){
			...
			for(int i=0;i<rejectnodelist.size();i++){
				...
				if(_nodeinfo.getId().equals(autoflowNodeid)){
					flag = false;
					break;
				}
			}
			if(flag){
				rejectnodelist.add(nodeinfoService.get(autoflowNodeid));
			}
		}
		...
		for(int i=0;i<rejectnodelist.size();i++){
			...
			if(ischecked)
				rejectnodeid = _thisnodeid;
			if(!listNodeThrough.contains(_thisnodeid) && !autoflowNodeidList.contains(_thisnodeid)){	//忽略未流转到的节点, 并且也不在自动流转节点的集合中
				continue;
			}
			if(_thisnodeid.equals(currentNode.getId())){	//忽略当前节点
				continue;
	...
	%>
		<tr>
			<td>
				<input type="radio" value="<%=_thisnodeid%>" id="rejectednode1" name="rejectednode1" <%if(ischecked){%> checked <%}%> onclick="javascript:document.getElementById('rejectednode').value='<%=_thisnodeid%>';"><%=StringHelper.null2String(_nodeinfo.getObjname())%>
			</td>
		</tr>
	<%}
	}%>
		<tr>
			<td align="center">
				<button type="button" class='btn' accessKey='O' id="hider0" onclick="changeIsreject(1);dlg0.hide();"><U>O</U>--<%=labelService.getLabelName("确定")%></button>
				<button type="button" class='btn' accessKey='C' id="hider1" onclick="changeIsreject(0);dlg0.hide();"><U>C</U>--<%=labelService.getLabelName("取消")%></button>
			</td>
		</tr>
	</table>
	</div>
...
<input type="hidden" id="isemail" name="isemail" value="<%=isemail %>" >
<input type="hidden" id="issms" name="issms" value="<%=issms %>">
<input type="hidden" id="ispopup" name="ispopup" value="<%=ispopup %>">
<input type="hidden" id="isrtx" name="isrtx" value="<%=isrtx %>">
<!-- end 选择提醒方式 -->
<input type="hidden" id="attachcanedit" name="attachcanedit" value="<%=attachCanEdit%>">
</div>

<div id="tab1" style="display:none">
	<%
	boolean curNodeStamp=false;
	Stampinfo stampinfo = new Stampinfo();
	if (!StringHelper.isEmpty(requestid) && !StringHelper.isEmpty(nodeid) && workflowinfo.getIsDoc() == 0) {
	 String sql = "select nodeid from Stampinfo where requestid='" + requestid + "' group by nodeid";
	 List<Map> list = dataService.getValues(sql);
	 if(list!=null){
		 for(Map map:list){
			 String _nodeid=StringHelper.null2String(map.get("nodeid"));
			 if(StringHelper.isEmpty(_nodeid))continue;
			 Nodeinfo info=nodeinfoService.get(_nodeid);
			 String hql="from Stampinfo where requestid='"+requestid+"' and nodeid='"+_nodeid+"'";
			 List<Stampinfo> stampinfos=stampinfoService.getStampinfos(hql);
			 stampinfo=stampinfos.get(0);
			 if(StringHelper.isEmpty(info.getStampfield())){
			 %>
				 <div  id="stamp_<%=stampinfo.getNodeid()%>" style="position:absolute;content:'';left:<%=stampinfo.getStampx()%>;top:<%=stampinfo.getStampy()%>;" <%if(nodeid.equals(stampinfo.getNodeid())){%>onmousedown="MouseDown(this)" onmousemove="MouseMove()" onmouseup="MouseUp()"<%}%>>
				 </div>
			 <%
			 }
			 if(_nodeid.equals(nodeid)){
				 curNodeStamp=true;
				 for(Stampinfo _stampinfo:stampinfos){
					 if(eweaveruser.getId().equals(_stampinfo.getUserid())){
	...
	if(isstamp&&!curNodeStamp&&StringHelper.isEmpty(nodeinfo.getStampfield())){%>
		<div id="stamp_<%=nodeid%>" style="position:absolute;content:'';" onmousedown="MouseDown(this)" onmousemove="MouseMove()" onmouseup="MouseUp()">
		</div>
	<%}%>
	<div id="selRemindDiv" style="width:90px;padding:8px; background: none repeat scroll 0 0 white;" name="selRemindDiv" class="selRemindDivHide">
		<%=remindBuf.toString() %>
	</div>
	<div id="layoutFrame" style="margin:0 auto;">
	<%=formcontent%>
	...
	<table class="layouttable signature">
	<tr>
	<td class="FieldName fieldname"><%=labelService.getLabelName("签字意见")%><br/>(<input type=checkbox onclick="changeremarktype(this)" name="btnchangeremark" id="btnchangeremark"><%=labelService.getLabelName("超文本")%>)</td>
	<td class="FieldValue"><%if("1".equals(setitemService.getSetitem("4089487d23f9e66e0123ffe23303253b").getItemvalue())){%>
  	<select id="personvalue" style="width:770px;" name="personvalue" onchange="personalChange(this.options[this.options.selectedIndex].innerHTML)" >
  	<option></option>
        <%    
    ...
    <%}%>
	<textarea class="InputStyle2" style="width:764px; height:100px;" name="remark" id="remark" ></textarea>
	<div style="margin-top: 5px;">
		<%=labelService.getLabelNameByKeyId("402883153fcd6300013fcd63014e01ae")%>:<!-- 相关文档 -->
		<span>
			<button type="button" class=Browser style="vertical-align: middle;"
	...
	<div style="margin-top: 10px;">
		...
		<span>
		    ...
		    <input type="hidden" name="refworkflowid" id="refworkflowid" value=""/>
		    <span id="refworkflowidspan"></span>	
		</span>
	</div>
	<div style="margin-top: 10px;" id="filelist_remarkfiles">
	...
	WeaverUtil.load(function(){
			...
			CKEditorExt.initEditor_Form('remark', {
        	   config : {width : '770px', height : '100px'}, 
           	   instanceReadyFun : function(e){
        		   <%if(NumberHelper.getIntegerValue(nodeinfo.getRemarkRequired())==1) {%>
        		   CKEditorExt.checkText("remark");
        		   <%}%>
        		   CKEditorExt.switchTextMode("remark");
        		   <%
        		    /*加载默认签字意见start*/
        		    out.println("Ext.getDom('remark').value = ''");
					String pdefvalue = eweaveruser.getSysuser().getDefvalue();
				 	if(!bNewworkflow.equals("1")){	//非流程新建节点，带出个性签名
				 		String defremark="";
				 		Setitem isonlyonelog=setitemService.getSetitem("dd4851f9f7c84bcaa83f3f1273bdf869");
						if(isonlyonelog.getItemvalue().equals("1"))
						{
					 		/*取到当前人的签字意见*/
						 	if(requeststep !=null){
						 		List loglist = dataService.getValues("select remark from requestlog where stepid='"+StringHelper.null2String(requeststep.getId())+"' and requestid='"+requestid+"' and operator='"+currentuser.getId()+"' and logtype in ('402881e50c5b4646010c5b5afd17000a','402881e50c5b4646010c5b5afd17000e') order by operatedate desc,operatortime desc");
								String stepremark="";
								if(loglist.size()>0){
									defremark=StringHelper.null2String(((Map) loglist.get(0)).get("remark"));
								}
						 	}
						}
						if(StringHelper.isEmpty(defremark)){
						 	if(!StringHelper.isEmpty(pdefvalue)&&nodeinfo.getRemarkRequired().intValue()==0){
						 		//如果签字意见为必填，则不带出个性签名。
						  		...
						  		defremark=StringHelper.null2String(personalSignature.getObjvalue());
						  	}
					  	}
						defremark=defremark.replaceAll("[\\t\\n\\r]", "");//去掉换行符等，防止文本内容导致错误
						out.println("Ext.getDom('remark').value = '"+defremark+"'");
						out.println("Ext.getDom('remark_mappingText').value = '"+defremark+"'");
	       			 	out.println("CKEditorExt.setHtml('remark', '"+defremark+"');");
				 	}
				    /*加载默认签字意见end*/
				   %>
           	   }
		...
		function personalChange(objtext){
			CKEditorExt.setHtml("remark", objtext);
			Ext.getDom('remark_mappingText').value=objtext;
			
			var btnchangeremark=document.getElementById('btnchangeremark');
			if(!(btnchangeremark&&btnchangeremark.checked))
			{
			 	Ext.getDom('remark_mappingText').focus();
			}
		}
	...
	%>
		<div id="requestlogsdiv" <%=(StringHelper.null2String(nodeinfo.getPflowmethod()).equals("2")) || setitem74.getItemvalue().equals("0")?" style=\"display:none;\"":"" %>></div>
	<% }%>
	<!--审批意见end-->
	</div>
	</div>
...
<%} %>
  ...
  <%if(isRedHead){%>
  	<!--公文套红/加载Word模板时，由于正文已经有WebOffice控件，不能在同一页面内-->
	<iframe src="weboffice.jsp" id="webOfficeFrame" style="display:none;width:100px;height:150px;"></iframe>
	<%}%>
	<script type="text/javascript">
	var tabPanel = null;
	var moveble=false;
	...
	function insertImage(data){
	  ...
	  if(!Ext.isEmpty(<%=isDocFlow%>) && <%=isDocFlow%>){//公文盖章
		...
		var temp1  = '<%=tempdir1%>';
		var tempdir  = temp1+':';
		var hasimg=office.WebDownLoadFile(imgUrl,tempdir+'\\temp_stamp.gif');
		if(hasimg){
			var inlineShape=office.WebObject.Shapes.AddPicture(tempdir+'\\temp_stamp.gif',true,true);
			office.WebDelFile(tempdir+'\\temp_stamp.gif','');
			try{
		...
		Ext.getDom('DocWebOffice').style.display='block';
	  	...
	  	<%if (nodeinfo.getIsstamp() == 1) {%>
			  ...
			  btn.focus();
			  <%} else {%>
			  if (Ext.isEmpty(window['stampfieldparent'])) {
				var obj = jQuery('input[name^=field_<%=nodeinfo.getStampfield()%>], textarea[name^=field_<%=nodeinfo.getStampfield()%>]');
				if (obj.size() == 0) {
				  	obj = jQuery('span[id^=field_<%=nodeinfo.getStampfield()%>]');
				}
				if(obj.length>0){
					window['stampfieldparent'] = obj.parent();
					window['stampfieldparent'].empty();
				}else{
					var stampobj=jQuery("#stamp_"+nodeid);
					if(stampobj.length>0){
						window['stampfieldparent']=stampobj;
					}
				}
			  }
			  window['stampfieldparent'].html(currentNodeStampHtml).append("<img src='" + data.url + "'>");
			<%}}%>
	  }//endif.
	};
	...
	function _addPerson(persontype){
		...
		if (!remark) {
			remark = FCKEditorExt.getHtml();
		}
		EditOK(persontype,encode(remark));
	}
	
	//强制归档流程
	function forceFinishFlow(){
		disabledToolbar();
		var $ = jQuery;
		var remark = CKEditorExt.getText('remark');
		var remarkfiles = $("input[name=remarkfiles]").val();//签字意见附件
		if(remark.trim()==''){
	        alert("请填写签字意见");
	        enabledToolbar();
	        return;
		...
		if(confirm("确定要强制归档该流程吗？")){
			DWREngine.setAsync(false);
			WorkflowService.modifyWorkflowForceFinish('<%=requestid%>',remark,remarkfiles);
			...
			if(commonDialog){
				...
				if(!commonDialog.hidden){
					...
					tabWin.location.href="/workflow/request/close.jsp?mode=forceFinish&requestname=<%=requestname%>";
				}
			}
		}else{
			enabledToolbar();
   ...
   function EditOK(persontype,remark) {
	   ...
	   if (!Ext.isSafari) {
		  ...
		  if(persontype==1){
		   ...
		   var currentuserid = getEncodeStr(document.getElementById("currentuserid").value);
		   ...
		   if (objid != "" && objname != "" && objid != undefined && objname != undefined) {
			   ...
			   var refworkflowid = getEncodeStr(getValidStr(b[6]));//相关流程id
			   var refdocbaseid = getEncodeStr(getValidStr(b[5])); //相关文档id
			   var isforwordEmail = getEncodeStr(getValidStr(b[4]));//流程转发邮件提醒
			   ...
			   updatestring += "<currentuserid>" + currentuserid + "</currentuserid>";
			   updatestring += "<isforwordEmail>" + isforwordEmail + "</isforwordEmail>";
			   ...
			   updatestring += "<refdocbaseid>" + refdocbaseid + "</refdocbaseid>";
			   updatestring += "<refworkflowid>" + refworkflowid + "</refworkflowid>";
				   ...
				   success: function(result) {
					   ...
					   if (id != "") {
						   ...
						   if (persontype == 4) {
							   ...
							   pop(showmessage);
							   myMask.hide();
							   top.window['TabPortlet'].refreshRequestTab();
							   top.window['TodoWorkflowPortlet'].refresh();
							   document.location.reload();	//call /js/main.js
							   return;
						   }
						   ...
						   pop("<%=labelService.getLabelName("操作失败")%>!");
						   myMask.hide();
					   }
				   }
		  ...
		  // }
		   ...
		   win.close = function() {
			   ...
			   if (objid != "" && objname != "" && objid != undefined && objname != undefined) {
				   ...
				   var isforwordEmail = getEncodeStr(getValidStr(id[4]));
				   var refdocbaseid = getEncodeStr(getValidStr(id[5]));
				   var refworkflowid = getEncodeStr(getValidStr(id[6]));
				   var redirectattachid=getEncodeStr(getValidStr(id[3]));//流程转发附件
				   var updatestring = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
				   updatestring += "<data>";
				   updatestring += "<requestid>" + requestid + "</requestid>";
				   updatestring += "<persontype>" + persontype + "</persontype>";
	...
	function MouseMove()
	{
	  ...
	  if(Obj!=null)
	   {
		 Obj.style.left = (event.x-Obj.l)+"px";
		 Obj.style.top = (event.y-Obj.t)+"px";
	   }
	  }
	}
	function MouseUp()
	...
	var currentNodeStampHtml="";
	function getstamp(){
		<%
		if (workflowinfo.getIsDoc() == 0) {
		...
		if (!StringHelper.isEmpty(requestid) && !StringHelper.isEmpty(nodeid)) {
		 ...
		 for (int i = 0; i < list23.size(); i++) {
			...
			if(nodeid.equals(stampinfo2.getNodeid())&&eweaveruser.getId().equals(stampinfo2.getUserid())) {
				stampinfoid=stampinfo2.getId();
			}
			boolean isCurrentNode=nodeid.equals(stampinfo2.getNodeid());
			...
			if(!StringHelper.isEmpty(stampinfo2.getImginfoid())) {
				...
				if(!StringHelper.isEmpty(imginfo.getAttachid())){
					...
					%>
					var obj = jQuery('#field_<%=noinfo.getStampfield()%>') ;
					if(obj.length==0){
					  obj=jQuery('#field_<%=noinfo.getStampfield()%>span') ; //显示布局的多行文本框
					}
					if(obj.length>0){
						obj.parent().html('<div id="stamp_<%=stampinfo2.getNodeid()%>"><img src=/ServiceAction/com.eweaver.document.file.FileDownload?attachid=<%=imginfo.getAttachid()%>&amp;download=1 ></div>');
		  				<%if(isCurrentNode){%>
		  				currentNodeStampHtml='<img src=/ServiceAction/com.eweaver.document.file.FileDownload?attachid=<%=imginfo.getAttachid()%>&amp;download=1 >';
		  				<%}%>
					}else{
		  				jQuery("#stamp_<%=stampinfo2.getNodeid()%>").append("<img src=/ServiceAction/com.eweaver.document.file.FileDownload?attachid=<%=imginfo.getAttachid()%>&amp;download=1 >");
		  				<%if(isCurrentNode){%>
		  				currentNodeStampHtml=jQuery("#stamp_<%=stampinfo2.getNodeid()%>").html();
		  				<%}%>
					}
		<%}	}  }}  }%>
	}
...
function checkUnique(obj,param,isonly,fieldname,tablename){
	...
	{	
		...
		// param="select "+fieldname+" from "+tablename+" where "+fieldname+" = ?"; // 这一条判断了所有  包括逻辑删除的
		if('<%=requestid%>'!=''){
			addsql = " and requestid!='<%=requestid%>'";
			oldparam="select "+fieldname+" from "+tablename+" where requestid='<%=requestid%>'";
		}
		param="select "+fieldname+" from "+tablename+" where "+fieldname+" = ? and requestid in (select id from requestbase where isdelete=0)"+addsql;
...
function SQL(param,dbtype,dbname,ismutisql){
	param = encode(param);
...
function onSubmit(issave){
	...
	if(issave == 0){
	    ...
	    <%if(nodeinfo.getRemarkRequired().intValue()==1){%>
	    	var remarkvalue = CKEditorExt.getText('remark');
	        if(remarkvalue.trim().ReplaceAll("　",'')==''||remarkvalue==''||remarkvalue=='null'){
	...
	if(issave==2){
		document.all("isnotify").value = "0";//如果知会和编辑权限一起时按照编辑权限提交处理
	}
	disabledToolbar();
...
function onDelete(){
    ...
    Ext.Msg.confirm('', '<%=labelService.getLabelName("您确定要删除吗")%>?', function (btn, text) {
                if (btn == 'yes') {
                    location.href='/ServiceAction/com.eweaver.workflow.request.servlet.WorkflowAction?action=delete&curnodeid=<%=curnodeid%>&requestid=<%=requestid%>&workflowid=<%=workflowid%>';
                }
    });
...
//                    alert(imgsrc);
                ...
                var temp2  = '<%=tempdir1%>';
                 var hasimg=office.WebDownLoadFile(imgsrc,temp2+':/'+name+'.gif');
                if(hasimg){
                    bookmark.Select();
//                    alert(bookmark.InlineShapes.Count);
                    inlineShape=bookmark.InlineShapes.AddPicture(temp2+':/'+name+'.gif',false,true);
...
//                    inlineShape.Select();
            ...
            if(document.getElementById(name)!=null){
                ...
                office.WebObject.Bookmarks.Add(name,bookmark);
            	    ...
            	    defaultValue="";
			        var wordtag = name+"";
                    if(wordtag.indexOf("log")>-1 && wordtag.length==40){ //签字日志取法 cjl 2012-12-09 
					  var nodeid=wordtag.substring(4,36);
                      var logtype=wordtag.substring(37,38);
                      var logstatus=wordtag.substring(39,40);
                      var order = "desc";
                      var m ="min"
                      if(nodeid!="" && logtype!="" && logstatus!=""){
                         if(logstatus=="1"){//第一条
                            order = "asc";
                            m ="min"
                         }else if(logstatus=="2"){//最后一条
                            order = "desc"
                            m ="max"
                         }
                         if(logtype=='1'){//创建
					        logtype='402881e50c5b4646010c5b5afd170008';
					     }
					     if(logtype=='2'){//保存
					        logtype='402881e50c5b4646010c5b5afd17000e';
					     }
					     if(logtype=='4'){//提交
					        logtype='402881e50c5b4646010c5b5afd17000a';
					     }
					     if(logtype=='5'){//退回
					        logtype='402881e50c5b4646010c5b5afd17000b';
					     }
					     if(logtype=='6'){//转发
					        logtype='402881e50c5b4646010c5b5afd17000d';
					     }
					     if(logtype=='9'){//撤回
					        logtype='4028690a0f60fbe6010f6122b45a0040';
					     }
					     <%if(SQLMap.getDbtype().equals(SQLMap.DBTYPE_SQLSERVER)){%>
					     var sql0="select submiter,datestr from (select submiter,"+m+"(submitdate+' '+submittime) as datestr from requeststep WHERE nodeid = '"+nodeid+"'and requestid = '<%=requestid%>' AND submitdate is not null and submittime is not null and isdelete = 0 group by submiter) tname order by datestr desc";
					     <%}else{%>
					     var sql0="select submiter,datestr from (select submiter,"+m+"(submitdate||' '||submittime) as datestr from requeststep WHERE nodeid = '"+nodeid+"'and requestid = '<%=requestid%>' AND submitdate is not null and submittime is not null and isdelete = 0 group by submiter) tname order by datestr desc";
					     <%}%>
						 DWREngine.setAsync(false);
                         DataService.getValues(sql0,{                                               
					          callback: function(data0){
							        if(data0 && data0.length>0){ //
										for(var z=0;z<data0.length;z++){
										    var sql="SELECT r1.remark as remark,(select objname from humres where id=r1.operator) as objname,r1.operatedate as operatedate,r1.operatortime as operatortime FROM requestlog r1,requeststep r2 WHERE r1.requestid=r2.requestid  AND r1.stepid = r2.id AND r2.submiter ='"+data0[z].submiter+"' AND r1.requestid='<%=requestid%>' AND r1.logtype='"+logtype+"' AND r2.nodeid ='"+nodeid+"' AND r1.isdelete=0 AND r2.submitdate is not null AND r2.submittime is not null AND r2.isdelete=0 ORDER BY r1.operatedate "+order+",r1.operatortime "+order;
											DWREngine.setAsync(false);
											DataService.getValues(sql,{                                               
												  callback: function(data){   
													    ...
													    var operatedate="";
													    var operatortime="";
													    if(data && data.length>0 && logstatus!="0"){ //只取一条
														    remark = data[0].remark;
														    if(remark==null){
															   remark=" ";
														    }else{
														    	remark = remark.replace(/<p>/ig,'');
														    	remark = remark.replace(/<\/p>/ig,'');
														    	remark = remark.replace(/<br \/>/ig,'');
														    	remark = remark.replace(/<BR>/ig,'');
														    	remark = remark.replace(/^\r\n/ig,''); 
														    	remark = remark.replace(/^\r/ig,''); 
														    	remark = remark.replace(/^\n/ig,''); 
														    }
														    if(logtype=='402881e50c5b4646010c5b5afd17000d'){//转发 过滤超链接内容
														          if(remark.toUpperCase().indexOf("</A>")>-1){
														              remark = remark.substring(remark.toUpperCase().indexOf("</A>")+9,remark.length)
														          }
														    }
														    objname = data[0].objname;
														    operatedate = data[0].operatedate;
														    operatortime = data[0].operatortime;
														    defaultValue += remark+" "+objname+" "+operatedate+" "+operatortime+" \n";
													    }
													    if(data && data.length>0 && logstatus=="0"){//取所有
															 for(var i=0;i<data.length;i++){
																...
																operatedate = data[i].operatedate;
																operatortime = data[i].operatortime;
																defaultValue += remark+" "+objname+" "+operatedate+" "+operatortime+" \n";
															 }
													    } 
												}                 
		...
		if(buttonObj.sapflag){
				...
				for(var i=0;i<rvjson.length;i++){
					...
					if(formfieldid.indexOf("field_")>-1&&formfieldid.indexOf("con")>-1){
						...
						if(fieldvalue=='eweaverclear'){
							$(trobj).find("[name="+formfieldid+"]").val('');
							if($(trobj).find("#"+formfieldid+"span")){
								$(trobj).find("#"+formfieldid+"span").html('');
							}
							continue;
						}
						if($(trobj).find("[name="+formfieldid+"]").val()==''){
							$(trobj).find("[name="+formfieldid+"]").val(fieldvalue);
						}else{
							$(trobj).find("[name="+formfieldid+"]").val($(trobj).find("[name="+formfieldid+"]").val()+","+fieldvalue);
						}
						if($(trobj).find("#"+formfieldid+"span")){
							if($(trobj).find("#"+formfieldid+"span").text()==""){
								$(trobj).find("#"+formfieldid+"span").html(fieldvalue);
							}else{
								$(trobj).find("#"+formfieldid+"span").html($(trobj).find("#"+formfieldid+"span").html()+','+fieldvalue);
							}
						}
					}
				}
...
//签字意见显示隐藏js
jq(document).ready(function(){
    	 jq("#signViewC").css("cursor","hand");
    	 jq('#signViewC').mouseover(function(event) {
        	 if(jq(this).attr("flag")){
  				jq(this).removeClass('down').addClass('down-over');
            }else{
  				jq(this).removeClass('up').addClass('up-over');
            }
  		});
  		jq('#signViewC').mouseout(function() {
  			if(jq(this).attr("flag")){
	  			jq(this).removeClass('down-over').addClass('down');
  	  	  	}else{
	  			jq(this).removeClass('up-over').addClass('up');
  	  	  	 }
  		});
    	 
    	 jq("#signViewC").click(function(){
    		 if(!blockSignView()){
    	            jq("#signViewC").attr("flag",true);
    	            jq("#signViewC").attr("title","<%=labelService.getLabelNameByKeyId("40288035248eb3e801248ee4664d0016")%>");
    	            jq("#signView").slideUp("fast",function(){
    	            	jq('#signViewC').removeClass('up-over').removeClass("up").addClass('down');
        	        });
    	         }
         });
      });
	...
	//网站模式下左右滚动条
	jQuery(function($){
		if(currentSysModeIsWebsite()){//网站模式下
			var detailtables = $(".detailtable");//所有的子表
			if(detailtables){
				var p = $("#layoutFrame");
				//p.get(0).style.overflow ="auto";
				p.css("overflow","auto");
				p.css("overflow-y","hidden");
				p.css("padding-bottom","25px");
				//alert(p.get(0).offsetHeight);
			}
		}
		var selRemindDiv = $("#selRemindDiv");
		selRemindDiv.hover(
     ...
     function blockSignView(){
    	 if (jq("#signViewC").attr("flag") == "true") {
    		 jq("#signViewC").attr("flag",false);
    	            jq("#signViewC").attr("title","<%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd68004400003")+labelService.getLabelNameByKeyId("40288035248eb3e801248edfa9a80014")%>");
    		 jq("#signView").slideDown("fast",function(){
    			 jq('#signViewC').removeClass('down-over').removeClass("down").addClass('up');
        	 });
...
/*************************该流程是从项目管理中任务新建流程创建的刷新任务卡片start**************************/
function ifRefreshTab(tabid) {
	if(top && typeof(top.refreshTab) == 'function'){
	    top.refreshTab('tab'+tabid);
	    top.refreshTab('exec_'+tabid);
	    top.refreshTab('plan_'+tabid);
	} else{
		var tabpanel=top.frames[1].contentPanel;
		if(tabpanel.getItem('tab'+tabid)){
		    tabpanel.getItem('tab'+tabid).getFrameWindow().location.reload();
		}
		if(tabpanel.getItem('exec_'+tabid)){
		    tabpanel.getItem('exec_'+tabid).getFrameWindow().location.reload();
		}
		if(tabpanel.getItem('plan_'+tabid)){
		    tabpanel.getItem('plan_'+tabid).getFrameWindow().location.reload();
		}
	}
}
<%if (!StringHelper.isEmpty(reftabid)) {%>
ifRefreshTab("<%=reftabid%>");
<%}%>
/*************************该流程是从项目管理中任务新建流程创建的刷新任务卡片end**************************/
...
<%
//流程全局脚本引用。
String extScriptPath = workflowinfoService.getExtScriptPathByWorkflowid(workflowid);
File jsCode = new File(extScriptPath);
if(jsCode.exists()){
%>
<script type="text/javascript" src="<%=request.getContextPath() %>/workflow/extscript/<%=workflowid %>.js"></script>
<%}%>
</body>
</html>
