...
<div  class="connectedSortable">
	<%for(int j=0;j<list.size();j++){
		Map m = (Map)list.get(j);
		String dsporder = StringHelper.null2String(m.get("dsporder"));
		if(dsporder.equals("300")){
			continue;
		}
		List workflows = (List)m.get("workflows");
		List workList = new ArrayList();
		if(workflows.size()>0){
			for(int k=0;k<workflows.size();k++){
				Map map = (Map)workflows.get(k);
				String id = (String)map.get("id");
				Workflowinfo workflowinfo = workflowinfoService.get(id);
				int dsp = workflowinfo.getDsporder();
				if(dsp==300){
					continue;
				}
				workList.add(map);
