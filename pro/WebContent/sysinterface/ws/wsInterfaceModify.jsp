...
<head>  
 ...
 <body>
		<div>
			<ew:entity 
			    fields="name,code,dsid,isSecurity,username,password,isNeedReturn,wsdlUrl,endpoint,method,requestType,paramType,paramLevel,fields,parameters,description,interfaceTypeId,objid,objtype" 
			    tags="text,text,select,select,text,text,select,text,text,text,select,select,select,select,textarea,textarea,hidden,hidden,hidden" 
			    subject="WebService接口配置" 
			    showtest="true"
			    contents="[{dsid:\"select id as objid,dsName as objname from dsentity where dstypeid = '4028825732056bd40132057012260003' \"},
			    {isNeedReturn:\"1__是,0__否\"},{isSecurity:\"1__是,0__否\"},{requestType:\"1__API,2__SOAP 1.2\"},{paramType:\"0__JSON格式,1__SQL格式,2__文本格式\"},{paramLevel:\"0__全参,1__变量\"}]"
			    action="/ServiceAction/com.eweaver.sysinterface.base.servlet.InterfaceConfigAction?action=save" 
			    names="接口名称,接口编码,选择已有配置,是否需要安全访问,用户名,密码,是否需要返回值,WSDL地址,Endpoint地址,方法名称,请求类型,参数类型,参数级别,可选字段,接口参数,接口描述,接口类型ID,关联对象ID,关联类型"
			>
			</ew:entity>
		</div>
	</body>
	...
	var otherids = new Array('username_tr','password_tr','wsdlUrl_tr','endpoint_tr');
	$(document).ready(	   
		function(){		    
	...
	function onAddDs() {
		window.showModalDialog("wsModify.jsp?dsTypeId=4028825732056bd40132057012260003","","dialogHeight:600px;dialogWidth:900px;status:no;center:yes;resizable:yes");    
	}
	</script>
</html> 