package com.eweaver.sysinterface.extclass; 
 ...
 public class Ewv20141031111116 extends EweaverExecutorBase{
    ...
    /* EweaverUser currentuser = BaseContext.getRemoteUser();//获取当前用户对象 
	 ...
	 if(list.size()>0){
		...
		if(feetype.equals("40285a904931f62b01495059a83228fa")) //如果费用类型为出差培训类
		{
			...
			{
				String upsql = "update uf_hr_businesstrip set zjkmark = '',zjkreq = '' where requestid = '"+businessreqno+"' ";
				baseJdbc.update(upsql);
            }
			else //如果关联的是外训申请单
			{
				String upsql = "update uf_hr_outlesson  set jkmark = '',jkreq = '' where requestid = '"+outtrainreqno+"' ";
				baseJdbc.update(upsql);
			}
		}
	  }//if end
