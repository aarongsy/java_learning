...
public class Ewv20141107144902 extends EweaverExecutorBase{
	     ...
	     String field1 = params.getParamValueStr("FIELD1");//获取表单中的字段值,字段名参数要大写 		
	        ...
	        if(list.size()>0){ 
	        	...
	        	for(int i=0;i<fflist.length;i++){
	        		...
	        		//循环明细表，并创建至组合报价明细至供应商报价表中
	        		String sqldt = "select id,costname,operateport,cabinettype,danordtype,remarks from uf_tr_ffdetail where requestid = '"+requestid+"'";
	        		...
	        		if(listdt.size()>0){
	        			for(int j=0;j<listdt.size();j++){
	        				...
	        				String inertsql = "insert into uf_tr_ffpricedetail(id,requestid,priceman,costname,operateport,cabinettype,danordtype,remarks,groupid) " +
	        						"values('"+IDGernerator.getUnquieID()+"','"+requestid+"','"+priceman+"','"+costname+"','"+operateport+"','"+cabinettype+"','"+danordtype+"','"+remarks+"','"+id+"')";
