...
import java.io.OutputStream; 
 ...
 public class Ewv20141218092436 extends EweaverExecutorBase{ 
 ...
 public void doExecute (Param params) {
	...
	BaseJdbcDao baseJdbc=(BaseJdbcDao)BaseContext.getBean("baseJdbcDao");
		...
		String sql = "select jcjxy,flowno,tohr from uf_hr_monthreward where requestid='"+requestid+"'";
        String upsql =""; 
		System.out.println("YDJC CLOSED:"+sql);
		List tlist = baseJdbc.executeSqlForList(sql);
		if(tlist.size()>0){
			Map map = (Map)tlist.get(0);
            String tohr = StringHelper.null2String(map.get("tohr"));//sap返回 
			String theMonth = StringHelper.null2String(map.get("jcjxy"));//生效月份
			String flowno =  StringHelper.null2String(map.get("flowno"));//生效月份
          if(tohr.equals("I")){
			upsql="update uf_hr_monthreward set stateflag='3' where requestid='"+requestid+"'";
			...
			upsql="update uf_hr_monthreward set errmsg='"+MESSAGE+"',tohr='"+MSGTY+"',stateflag='3' where requestid='"+requestid+"'";
			System.out.println("upsql="+upsql);		
			baseJdbc.update(upsql);
        }
	}
