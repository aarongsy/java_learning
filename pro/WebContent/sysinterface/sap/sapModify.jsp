...
<head>  
 ...
 <body>
		<div>
			<ew:entity 
			    fields="dsName,dsType,minClients,maxClients,isStart,user,passwd,client,codepage,sysnr,ashost,description,dsTypeId" 
			    tags="text,text,text,text,select,text,text,text,text,text,text,textarea,hidden" 
			    subject="SAP数据源配置" 
			    contents="[{isStart:\"1__是,0__否\"}]"
			    action="/ServiceAction/com.eweaver.sysinterface.base.servlet.InterfaceConfigAction?action=save" 
			    names="数据源名称,数据源编码,最小连接数,最大连接数,是否随系统启动,用户名,密码,客户端编号,代码页,SYSNR,地址,数据源描述,接口类型ID"
			>
			</ew:entity>
		</div>
	</body>
