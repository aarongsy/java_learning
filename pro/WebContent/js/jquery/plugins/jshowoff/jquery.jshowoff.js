...
(function($) {
	...
	$.fn.jshowoff = function(settings) {
		...
		this.each(function(i) {
			...
			var uniqueClass = 'jshowoff-instance-'+config.portletResponseId;
			...
			function play(src) {
				if(!isBusy()){
					...
					if(!isPlaying()){
						$('.'+uniqueClass+'-play').html(config.controlText.pause).removeClass('jshowoff-paused '+uniqueClass+'-paused');
					}
				};
			};
			...
			function pause(src) {
				clearInterval(timer);
				if(!src || src=='playBtn') $('.'+uniqueClass+'-play').html(config.controlText.play).addClass('jshowoff-paused '+uniqueClass+'-paused');
			...
			function addSlideLinks() {
				$wrap.append('<p class="jshowoff-slidelinks '+uniqueClass+'-slidelinks" id="jshowoff-slidelinks_'+config.portletResponseId+'"></p>');
				$.each(gallery, function(i, val) {
					var linktext = $(this).attr('title') && $(this).attr('title') != '' ? $(this).attr('title') : i+1;
					if(config.cssClass == "thumbFeatures"){	//缩略图导航
						var radFlag = new Date().getTime();
						linktext = '<img src="/js/jquery/plugins/jshowoff/images/jshowoffslidelink_'+config.portletResponseId+'_'+i+'.jpg?'+radFlag+'"/>';
					}
