...
(function($) {
  ...
  $.fn.extend({
    buildMbMenu: function(op,m,type,e){
	  ...
	  var $cancheMenuObj = findCancheMenuObj(m);
      if($cancheMenuObj != null){
		  this.menu = $cancheMenuObj;
		  this.menuContainer = $(this.menu).find(op.options.menuSelector);
		  if (!type || type=="cm") op.rootMenu=this.menu;
		  if (op.options.fadeInTime>0) $(this.menu).fadeIn(op.options.fadeInTime);
		  else $(this.menu).show();
		  $(this.menu).find(".menuDiv").hide();
	  }else{
	      var menuClass= op.options.menuSelector.replace(".","");
	      this.menu  = $("<div class='menuDiv'><div class='"+menuClass+"' style='display:table'></div></div>");
	      where.append(this.menu);
	      ...
	      $(this.voices).each(function(i){
	        ...
	        if (op.options.hasImages && !isText){
	          ...
	          var imgPath=$(voice).attr("img")?$(voice).attr("img"):"/js/jquery/plugins/mb.menu/ico/blank.gif";
	        ...
	        if(isSeparator)
	          line="<div class='separator' style='width:100%; display:inline-block'><img src='"+op.options.iconPath+"/js/jquery/plugins/mb.menu/ico/blank.gif' width='1' height='1'></div>";
	        if(isText)
	                ...
	                over:function(event){
	                  ...
	                  if ($(voice).attr("menu")){
	                    ...
	                    if (!opener.menuContainer.actualSubmenu || opener.menuContainer.actualSubmenu!=this){
	                      ...
	                      opener.menuContainer.actualSubmenu=true;
	                      $(this).buildMbMenu(op,$(voice).attr("menu"),"sm",event);
	                      //opener.menuContainer.actualSubmenu=this;
	                    }
	                    $(this).attr("isOpen","true");
	      ...
	      menuCaches.push({'menuId' : m, 'menuObj' : this.menu});
      }
      function findCancheMenuObj(m){
      	for(var i = 0; i < menuCaches.length; i++){
      		if(m == menuCaches[i]['menuId']){
      			return menuCaches[i]['menuObj'];
      		}
      	}
      	return null;
      }
    ...
    removeMbMenu: function(op,fade){
      ...
      if (op.rootMenu) {
        ...
        if (fade) $(op.rootMenu).fadeOut(op.options.fadeOutTime,function(){$(this).hide();});
        else $(op.rootMenu).hide();
        op.rootMenu=false;
        op.clicked=false;
        $(document).unbind("click");
      }
