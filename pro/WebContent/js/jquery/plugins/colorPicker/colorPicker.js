...
function initColor(){
    $("body").append('<div id="colorpanel" style="position: absolute; display: none;width:260px"></div>');
    var colorTable='';
    for(i=0;i<2;i++){
        for(j=0;j<6;j++){
...
function showColorPanel(obj,txtobj){
    $('#'+obj).click(function(){
        ...
        $("#CT tr td").unbind("click").mouseover(function(){
            var aaa=$(this).attr("style").replace("background-color:","").replace("BACKGROUND-COLOR:","").toUpperCase();
            var index = aaa.indexOf("#");
            if(index!=-1){
	            aaa=aaa.substring(index);
            }
            ...
            var index = aaa.indexOf("#");
            if(index!=-1){
	            $('#'+txtobj).val(aaa.substring(index+1));
            }
            $("#colorpanel").hide();
        })
