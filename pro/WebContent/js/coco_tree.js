...
{try{
	...
	this.add=function(src,key)
	{
		...
		{
			alert("coco_imagelist cause run time error:\nImage file "+img.src+" not found!")
	...
	iml.err="coco_imagelist cause run time error!\nError number:"+e.number+".\nError description:"+e.description;
	iml.err+=""
	alert(iml.err);
	return iml;
}
...
function coco_tree(imageList,indent,toObject,labelStyle)
{try{
	if(!document.getElementById){throw new Error(999,"Your browser dosen't support coco_tree(),IE5.0+ is recommended.");}
	var count=0,depth=0,nodes=[],tree=this
	this.copywrite=""
	this.err=""
	imageList=imageList==null?new coco_imagelist():imageList
	...
	var addNode=function(toNode,relation,text,key,ico,exeCategory,exeArg)
	{
		...
		//?? ??????? ??????????---- ?? 2006-09-07
		node.getPath=function(separator){separator=separator==null?"/":separator;var path="",oNode=node;for(var i=tier;i>0;i--){var text =oNode.label.innerText; var len = text.indexOf("(",0); if (len != -1) text = text.substr(0,len);path=separator+text+path;oNode=oNode.parent;}return path;}
	...
	tree.err="coco_tree cause run time error!\nError number:"+e.number+".\nError description:"+e.description;
	tree.err+=""
	alert(tree.err);
	return tree;
}
