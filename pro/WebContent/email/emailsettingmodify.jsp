...
<%
    ...
    String esettingid=StringHelper.null2String(request.getParameter("id"));
    Emailsetinfo emailsetinfo=new Emailsetinfo();
    if(!StringHelper.isEmpty(esettingid)){
        emailsetinfo=emailsetinfoService.getEmailsetinfo(esettingid);
    }
%>
<head>
...
<form id="eweaverForm" name="EweaverForm">
        <h3><%=labelService.getLabelNameByKeyId("402881e70b7728ca010b772e24f50009")%></h3><!-- 基本信息 -->
        <hr >
        <table>
                  <colgroup>
                      <col width="20%">
                  ...
                  <input type="hidden" name="id" id="id"  value="<%=esettingid%>" />
                  <tr>
                      <td class="FieldName" nowrap >
                           <%=labelService.getLabelNameByKeyId("402883de35273f910135273f954c000b")%><!-- 账户名称 -->
                      </td>
                      <td class="FieldValue">
                          <input type="text" name="objname" id="objname" value="<%=StringHelper.null2String(emailsetinfo.getObjname())%>" style="width:90%;"  onChange="checkInput('objname','objnamespan')" onkeypress="checkQuotes_KeyPress()">
            ...
            <tr>
                      ...
                      <td class="FieldValue">
                          <input  type="text" name="emailaddress" style="width:90%;"   id="emailaddress" value="<%=StringHelper.null2String(emailsetinfo.getEmailaddress())%>"  onChange="checkInput('emailaddress','emailaddressspan')" onkeypress="checkQuotes_KeyPress()"/>
          ...
          <tr>
                      ...
                      <td class="FieldValue">
                          <input  type="password" name="emailpass" style="width:90%;"   id="emailpass" value="<%=StringHelper.null2String(emailsetinfo.getEmailpass())%>"  onChange="checkInput('emailpass','emailpassspan')" onkeypress="checkQuotes_KeyPress()"/>
                      ...
                      <td class="FieldValue">
                          <select id="serverid" name="serverid">
                             ...
                             for(int i=0;i<list.size();i++){
                                 ...
                                 String selected="";
                                  if(emailsetinfo.getServerid().equals(eserverinfo.getId())){
                                   selected="selected";
                                  }
                             %>
                              <option value="<%=eserverinfo.getId()%>" <%=selected%>><%=StringHelper.null2String(eserverinfo.getObjname())%></option>
                              <%}%>
                          </select>
...
<script type="text/javascript">
   function onSubmit(){
       ...
       Ext.Ajax.request({
        ...
        success: function() {
            if(parent.emailTree) parent.emailTree.topToolbar.items.item('getmail').menu.items.item(id).setText(objname);
