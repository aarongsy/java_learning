...
%>
<head>
<script src='<%=request.getContextPath()%>/dwr/interface/GetemailsService.js'></script>
  ...
  var store1;
    ...
    this.grid = new FeedGrid(this, {
...

        {
            ...
            hidden:true,
            tooltip: {title:'<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f8003b")%>',text:'<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80042")%>'},//收取邮件//收取不同服务器上的邮件
         ...
         Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39380075")%>', function (btn, text) {//您确定要删除吗?
             if (btn == 'yes') {
                 Ext.Ajax.request({
                     url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.email.servlet.EmailAction?action=delgetemails',
            ...
            },'-',
       {
         ...
         Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883693f3b3a46013f3b3a470c0000")%>', function (btn, text) {//您确定要全部删除吗?
             if (btn == 'yes') {
                 Ext.Ajax.request({
                     url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.email.servlet.EmailAction?action=deleteall&from=delgetemails',
...
FeedGrid = function(viewer, config) {
    ...
    this.store = new Ext.data.Store({
        proxy: new Ext.data.HttpProxy({
            url: '<%=request.getContextPath()%>/ServiceAction/com.eweaver.email.servlet.EmailAction?action=inbox'
        }),
              reader: new Ext.data.JsonReader({
               root: 'result',
               totalProperty: 'totalcount',
               fields: ['isnew','sendfrom','subject','datetime','accessorystr','id',,'accountname','content','attachid','attachnamestr']
    ...
    this.store.load();
        this.store.on('load',function(st,recs){
          ...
          sm.on('rowselect',function(selMdl,rowIndex,rec ){
              var row = _this.view.getRow(rowIndex);
              var innerhtml = row.innerHTML;
              innerhtml = innerhtml.replace(/unread/gm, '');
              innerhtml = innerhtml.replace(/topic/gm, '');
              row.innerHTML = innerhtml;
              var reqid=rec.get('id');
              GetemailsService.setisnew(0,reqid);
...
Ext.extend(FeedGrid, Ext.grid.GridPanel, {
            ...
            this.menu = new Ext.menu.Menu({
                ...
                },
                {
                    ...
                    handler: function() {
                        ...
                        Ext.Msg.buttonText = {yes:'<%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd6d13003000c")%>',no:'<%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd6d19cf5000d")%>'};//是//否
                        _this=this;
                        Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39380075")%>', function (btn, text) {
                            if (btn == 'yes') {
                                Ext.Ajax.request({
    ...
    formatTitle: function(value, p, record) {
        if(record.data.isnew&&record.data.isnew=='1')
        return String.format(
                '<div class="topic"><span class=unread>{0}</span></div>',
...
<script>Ext.BLANK_IMAGE_URL = '<%=request.getContextPath()%>/js/ext/resources/images/default/s.gif';</script>
    ...
    <div id="divloadinga" >
     <span id="divloading" style="display:none"><img src="<%=request.getContextPath()%>/images/base/loading.gif" alt="" > <%=labelService.getLabelNameByKeyId("402883de35273f910135273f954c0002")%><!-- 正在加载 请稍候...... --></span>
    </div>
 </form>
