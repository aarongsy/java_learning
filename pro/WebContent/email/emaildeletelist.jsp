...
<html>
<%
    String action=request.getContextPath()+"/ServiceAction/com.eweaver.email.servlet.EmailAction?action=email&from=delete";
%>
<head>
 ...
 <script language="javascript" src="<%=request.getContextPath()%>/js/ext/feed-viewer/SessionProvider.js"></script>
  ...
  <script type="text/javascript">
      ...
      var flagdel=new Array();
      var allflagdel=new Array();
FeedViewer = {};
  var store1;
    ...
    this.grid = new FeedGrid(this, {
        ...
        },
       {
          ...
          '-',
             ...
             pressed: true,
            enableToggle:true,
            text:'<%=labelService.getLabelNameByKeyId("402883d934cf81d90134cf81da0a0018")%>',//彻底删除
         ...
         Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80032")%>', function (btn, text) {//您确定要彻底删除吗?
             if (btn == 'yes') {
                 Ext.Ajax.request({
                     url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.email.servlet.EmailAction?action=delall',
                     params:{ids:selected.toString(),flagdel:flagdel.toString()},
                     success: function() {
                         selected = [];
          ...
          ,'-',
       {
             pressed: true,
            enableToggle:true,
            text:'<%=labelService.getLabelNameByKeyId("402881ee0c715de3010c72432b110066")%><%=labelService.getLabelNameByKeyId("402883d934cf81d90134cf81da0a0018")%>',//全部彻底删除
            iconCls: Ext.ux.iconMgr.getIcon('delete'),
            scope:this,
            toggleHandler: function(btn, pressed){
                //if (selected.length == 0) {
         ...
         Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883693f3b3a46013f3b3a470c0000")%>', function (btn, text) {//您确定要全部删除吗?
             if (btn == 'yes') {
                 Ext.Ajax.request({
                     url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.email.servlet.EmailAction?action=deleteall&from=delete',
                     params:{ids:allselected.toString(),flagdel:allflagdel.toString()},
                     success: function() {
                         selected = [];
                           mainPanel.grid.store.load({params:{start:0, limit:20}});
                     }
    ...
    MainPanel.superclass.constructor.call(this, {
        ...
        items: {
            id:'main-view',
            layout:'border',
            title:'<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80033")%>',//已删除
            hideMode:'offsets',
            items:[
                this.grid, {
                id:'bottom-preview',
    ...
    this.gsm = this.grid.getSelectionModel();
       this.grid.store.on('load',function(st,recs){
                  if(recs.length<=0){
        ...
        proxy: new Ext.data.HttpProxy({
            url: '<%=action%>'
              ...
              reader: new Ext.data.JsonReader({
               ...
               fields: ['sendtostr','subject','datetime','accessorystr','id',,'accountname','content','getemails','attachid','attachnamestr']
           })
    });
   ...
   this.store.load({params:{start:0, limit:20}});
      ...
      this.store.on('load',function(st,recs){
              for(var i=0;i<recs.length;i++){
                  ...
                  var getemails=recs[i].get('getemails');
                  allselected.push(reqid);
                  allflagdel.push(getemails);
          ...
          sm.on('rowselect',function(selMdl,rowIndex,rec ){
              ...
              var getemails=rec.get('getemails');
            ...
            this.menu = new Ext.menu.Menu({
                ...
                },
                {
                    ...
                    handler: function(){
                        ...
                        var getemails=this.ctxRecord.data.getemails;
         Ext.Msg.buttonText={yes:'<%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd6d13003000c")%>',no:'<%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd6d19cf5000d")%>'};//是    否
         Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80032")%>', function (btn, text) {//您确定要彻底删除吗?
             if (btn == 'yes') {
                Ext.Ajax.request({
                                    url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.email.servlet.EmailAction?action=deleted',
                                    params:{id:id,getemails:getemails},
                                    success: function() {
...
--%>
  ...
  <div class="post-body">
      ...
      <br>
      <br>
      <b><%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80034")%>:</b> &nbsp;&nbsp;&nbsp;  {sendtostr}<!-- 发件人 -->
      <br>
      <br>
