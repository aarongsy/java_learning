...
<html>
<%
    String action=request.getContextPath()+"/ServiceAction/com.eweaver.email.servlet.EmailAction?action=email&from=draft";
%>
<head>
  ...
  var store1;
    ...
    this.grid = new FeedGrid(this, {
            ...
            ,'-',
       {
         ...
         Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883693f3b3a46013f3b3a470c0000")%>', function (btn, text) {//您确定要全部删除吗?
             if (btn == 'yes') {
                 Ext.Ajax.request({
                     url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.email.servlet.EmailAction?action=deleteall&from=deldraft',
    ...
    MainPanel.superclass.constructor.call(this, {
        ...
        items: {
            id:'main-view',
            layout:'border',
            title:'<%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80037")%>',//草稿箱
            hideMode:'offsets',
            items:[
                this.grid, {
                id:'bottom-preview',
            ...
            this.menu = new Ext.menu.Menu({
                ...
                },
                {
                    ...
                    handler: function() {
                       ...
                       text: '<%=labelService.getLabelNameByKeyId("402883d934cf81d90134cf81da0f0049")%>',//发送
                    iconCls: 'new-tab',
                    scope:this,
                    handler: function(){
                         var id=this.ctxRecord.data.id;
                         location.href='<%=request.getContextPath()%>/email/sendemail.jsp?id='+id;
                    }
                    }]
            });
...
<div id="divSearch">
    ...
    <div id="divloading" style="display:none">
     <img src="<%=request.getContextPath()%>/images/base/loading.gif" alt=""> <%=labelService.getLabelNameByKeyId("402883c134f8ef330134f8ef33f80038")%><!-- 正在加载 请稍后...... -->
    </div>
 </form>
</div>
