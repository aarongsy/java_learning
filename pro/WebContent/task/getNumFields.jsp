...
<%
	...
	htmltype = htmltype == null?"1":htmltype;
	...
	%>
	 <select id="<%=fieldname %>" name="<%=fieldname %>" onchange="myChange()">						  						   
	<%
	String fieldtypesql = " and fieldtype = '"+fieldtype+"'";
	if(fieldtype == null) {
		fieldtypesql = "";
	}
	for (int k = 0; k < forminfolists.size(); k++) {
		...
		String formfieldsql = "from Formfield where formid = '"+tempformid+"' and htmltype = '" + htmltype +"' "+ fieldtypesql + "  and (isdelete is null or isdelete = 0)";
		List formfieldlist = formfieldService.findFormfield(formfieldsql);
