...
var IMAGESPATH = '/ecology/wui/theme/ecology7/skins/default/rightbox/'; //图片路径配置
  ...
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0"  style=" font-size:12px; line-height:1.4;">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      ...
      <td height="33" style="background-image:url(' + IMAGESPATH + 'dialog_ct.png) !important;background: url(' + IMAGESPATH + 'dialog_ct.gif) repeat-x top;"><div style="padding: 9px 0 0 4px; float: left; font-weight: bold; color:#fff;"><img align="absmiddle" src="' + IMAGESPATH + 'icon_dialog.gif"/><span id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_closebtn.gif)\'" onmouseover="this.style.backgroundImage=\'url(' + IMAGESPATH + 'dialog_closebtn_over.gif)\'" style="margin: 8px 0 0; position: relative; cursor: pointer; float: right; height: 17px; width: 28px; background-image: url(' + IMAGESPATH + 'dialog_closebtn.gif);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div></td>\
...
Dialog.prototype.close = function () {
	...
	thisdialogDiv=null; 
	//解决firefox下窗口无法关闭问题
	jQuery("#_DialogDiv_"+this.ID, window.top.document).remove();
    this.closed = true;
};
...
Dialog.alert = function (msg, func, w, h,m) {
    var w = w || 300,
        h = h || 110,
    ...
    diag.show();
    diag.getDialogDiv().style.zIndex = 10000;
    diag.okButton.parentNode.style.textAlign = "right";
    diag.okButton.style.display = "none";
    diag.cancelButton.value = "  确 定  ";
    diag.cancelButton.focus();
};
Dialog.confirm = function (msg, funcOK, funcCal, w, h,m) {
    var w = w || 300,
        h = h || 110,
        m=m==false?false:true;
