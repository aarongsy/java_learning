...
<%!public String grtlftMenu(List elements, User user, boolean isPotal) {
		...
		String tmpLi = "<li>";
		tmpLi += "<a class='lfMenuItem' href='";
		tmpLi += "/homepage/Homepage.jsp?hpid=66&subCompanyId=64&isfromportal=0";
	...
	public String getChildMenu(Object obj, String sbgPosition) {
		...
		sfcm.append(" target='" + map.get("target") + "' >");
	...
	public List getPortalMenus(User user) throws Exception {
		...
		if (user.getUID() == 1) {
			sql = "select id,infoname,subcompanyid from hpinfo h where showtype=1 and isuse=1 order by ordernum,id";
		}
...
jQuery(document).ready(function(event) {
    ...
    var myLftMenu = new drilldownmenu(
        {menuid: 'drillmenu',menuheight: 'auto',breadcrumbid: 'drillcrumb',persist: {enable: false, overrideselectedul: false},speed:0}
    );
...
function dymlftenu(ele, isOpenFirstMenu) {
    ...
    if (menuMap.get(paramValue) != undefined && menuMap.get(paramValue) != null && menuMap.get(paramValue) != "" && paramValue != "111") {
        ...
        jQuery.ajax({
            url: "/wui/theme/ecology7/page/ajaxGetMenu.jsp?typeid=" + paramValue + "&parentid=" + (paramValue == "114" ? "0" : "10") + "&lftmn" + new Date().getTime() + "=",
            dataType: "text", 
...
function callback(dz, parent, liparent) {
    ...
    //回到父菜单
		...
		jQuery(".leftMenuItemNavCenter").css("background-position", abg).html(htmText);
