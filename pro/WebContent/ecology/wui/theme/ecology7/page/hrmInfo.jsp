...
<div style="position:relative;height:100%">
    ...
    <div  style="position:absolute;left:63px;top:5px;font-size:12px;height:16px;right:25px;">
       ...
       %>
           <img src="/wui/theme/ecology7/page/images/skin/skinSetting.png" title="<%=SystemEnv.getHtmlLabelName(27714, user.getLanguage())%>" style="height:16px;widht:16px;display:block;" style="cursor:pointer;" onclick="javascript:showSkinListDialog();" id="themeQh"/>
       <%
       }
       %>    
       </span>
...
<%} %>
	...
	<div  style="position:absolute;width:41px;height:41px;left:14px;top:17px;">
		<%
			...
			if(messagerUrl.indexOf("dummyContact.png")==-1) isHaveMessagerUrl=true;
		%>
		<%if(isHaveMessagerUrl){%>
		<img src="<%=ResourceComInfo.getMessagerUrls(""+userId)%>" border="0" width="38" height="37">
		<%}%>
	</div>
	<div  style="position:absolute;width:170px;height:18px;left:8px;top:66px;">
		<table width="100%" cellpadding="0px" cellspacing="0px" id="tblHrmToolbr" align="left">
			<tr>
				...
				<td width="24px"><a  href="/newportal/contactslist.jsp"  title='<%=SystemEnv.getHtmlLabelName(1515,user.getLanguage())%>'   target="mainFrame" style><span style="cursor:pointer;height:16px;width:16px;display:block;overflow:hidden;background:url(/wui/theme/ecology7/page/images/hrminfo/address.png) no-repeat;"></span></a></td>
				<td width="24px"><a  href="/hrm/resource/HrmResource.jsp?id=<%=userId%>"title='<%=SystemEnv.getHtmlLabelName(16415,user.getLanguage())%>'    target="mainFrame" style><span style="cursor:pointer;height:16px;width:16px;display:block;overflow:hidden;background:url(/wui/theme/ecology7/page/images/hrminfo/card.png) no-repeat;"></span></a></td>
				<td width="24px"><a  href="/hrm/resource/HrmResourcePassword.jsp?id=<%=userId%>" title='<%=SystemEnv.getHtmlLabelName(17993,user.getLanguage())%>'   target="mainFrame" style><span style="cursor:pointer;height:16px;width:16px;display:block;overflow:hidden;background:url(/wui/theme/ecology7/page/images/hrminfo/psw.png) no-repeat;"></span></a></td>
				<td width="24px"><a  href="/org/OrgChartHRM.jsp" title='<%=SystemEnv.getHtmlLabelName(25332,user.getLanguage())%>'  target="mainFrame" style><span style="cursor:pointer;height:16px;width:16px;display:block;overflow:hidden;background:url(/wui/theme/ecology7/page/images/hrminfo/org.png) no-repeat;"></span></a></td>
				<!--  <td width="24px"><a  href="/workplan/data/WorkPlan.jsp" title='<%=SystemEnv.getHtmlLabelName(2211,user.getLanguage())%>'  target="mainFrame" style><img  src="/wui/theme/ecology7/page/images/hrminfo/cal.gif" border="0"></a></td>-->
				<td width="*" align="left">
					<%if(canMessager){%>					
