...
function _initDrawTree(){//111113
	if(!$('treeContainer')){WeaverUtil.log("树视图构造失败！");return;}
	var nodes=Ext.query('div[@class$=title]');//document.getElementsByClassName("title");
	...
	if(level=='2' && sumWidth>maxWidth){//只有两级的话，则平行显示两行。
		...
		var nodes=Ext.query('div[@class$=column]');//document.getElementsByClassName("column");
	...
	for(var i=0;i<nodes.length;i++){
		var node=nodes[i];
		var pos1=WeaverUtil.getPosition(node);
		_drawRootLine(root,pos1);
	}
	
	
	var nodes=Ext.query('div[@class$=node]');//document.getElementsByClassName("node");
