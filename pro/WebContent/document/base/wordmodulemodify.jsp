...
<%
    ...
    String wordmoduleid = StringHelper.null2String(request.getParameter("wordmoduleid"));
    WordModule wordModule = wordModuleService.getWordModule(wordmoduleid);
...
<fieldset style="width: 98%"><!-- 模板创建 -->
	<form action="<%=request.getContextPath()%>/ServiceAction/com.eweaver.word.servlet.WordModuleAction?action=modify"  name="EweaverForm" id="EweaverForm" method="post">
		<input type="hidden" id="attachid" name="attachid" value="<%=StringHelper.null2String(wordModule.getAttachid())%>"/>
		<input type="hidden" name="wordmoduleid" value="<%=wordmoduleid%>"/>
			...
			<tbody>
				<tr>
					...
					<td class=FieldValue >
						...
						<input type="radio" name="docTemplateType" id="html" value="3" onclick="onChangeDocType(3)" <%if("3".equals(StringHelper.null2String(wordModule.getDocTemplateType()))){%>checked="checked"<%}%>><%=labelService.getLabelNameByKeyId("402881eb0bcd354e010bcda34a970018")%><!-- HTML文档 -->
						<input type="radio" name="docTemplateType" id="word" value="4" onclick="onChangeDocType(4)" <%if("4".equals(StringHelper.null2String(wordModule.getDocTemplateType()))){%>checked="checked"<%}%>><%=labelService.getLabelNameByKeyId("402881eb0bcd354e010bcda450910019")%><!-- WORD文档 -->
						<input type="radio" name="docTemplateType" id="excel" value="5" onclick="onChangeDocType(5)" <%if("5".equals(StringHelper.null2String(wordModule.getDocTemplateType()))){%>checked="checked"<%}%>><%=labelService.getLabelNameByKeyId("402881eb0bcd354e010bcda4b538001a")%><!-- EXCEL文档 -->
					</td>
				</tr>
				<tr>
					<td class=FieldName noWrap><%=labelService.getLabelNameByKeyId("402881eb0bcbfd19010bcc16ecb1000b")%></td><!-- 名称 -->
					<td class=FieldValue >
					    <input type=text class=inputstyle style="width:60%" name="objname" value="<%=StringHelper.null2String(wordModule.getObjname())%>"/>
					</td>
				</tr>
				<tr>
					<td class=FieldName noWrap><%=labelService.getLabelNameByKeyId("402883d934c15c570134c15c57f50000")%></td><!-- 描述 -->
					<td class=FieldValue >
					    <input type=text class=inputstyle name="objdesc" style="width:100%" value="<%=StringHelper.null2String(wordModule.getObjdesc())%>"/>
					</td>
				</tr>
				<%
				
						String orgnames="";
						String  _viewurl="/base/orgunit/orgunitview.jsp?id=";
						String sql = "select id as objid,objname as objname from orgunit where id in(" + StringHelper.formatMutiIDs(wordModule.getOrgids()) + ")";
						List valuelist = baseJdbcDao.executeSqlForList(sql);
						for (int j = 0; j < valuelist.size(); j++) {
							Map refmap = (Map) valuelist.get(j);
							...
							if (!StringHelper.isEmpty(_viewurl))
								orgnames += "<a href=javascript:onUrl('" + _viewurl + _objid + "','" + _objname
										+ "','tab" + _objid + "') >";
							orgnames += "&nbsp;" + _objname + "&nbsp;";
							if (!StringHelper.isEmpty(_viewurl))
								orgnames += "</a>";
				...
				<tr>
					...
					<td class=FieldValue>
						...
						<input type="hidden" id="orgids" name="orgids" value="<%=StringHelper.null2String(wordModule.getOrgids())%>"  style='width: 80%'  >
						<span id="orgidsspan" name="orgidsspan" ><%=orgnames%></span><br><span style="color:red">此项为空,表示公用!</span>
					</td>
				</tr>
				<tr>
					<td>&nbsp;</td>
	   ...
	   <div id="htmlDIV">
			<textarea name="htmlContent" id="htmlContent" style="height: 50px;"><%=StringHelper.null2String(wordModule.getHtmlContent())%></textarea>
	   </div>
   </form>
   <form action="<%=request.getContextPath()%>/ServiceAction/com.eweaver.word.servlet.WordModuleAction?action=upload"  name="UpLoadForm" id="UpLoadForm" target="upload_wordmodule" enctype="multipart/form-data" method="post">
		<table width="100%" class=layouttable>
			...
			<tbody>
				<tr id="showFile" <%if(StringHelper.null2String(wordModule.getAttachid()).equals("")){%>style="display:none"<%}else{%>style="display:block"<%}%>>
				    <td class=FieldName noWrap><%=labelService.getLabelNameByKeyId("402881eb0bcd354e010bcd9dfe6b0017")%></td><!-- 附件 -->
				    <td class=FieldValue >
				        <a href="javascript:void(0)"><%
						Attach attach=attachService.getAttach(StringHelper.null2String(wordModule.getAttachid()));
						out.print("<a href=\""+request.getContextPath()+"/ServiceAction/com.eweaver.document.file.FileDownload?attachid="+attach.getId()+"&download=1&nIndex=1\">");
						out.print(attach.getObjname());
						out.print("</a>");
						%></a>&nbsp;&nbsp;<a href="javascript:changedFile()"><%=labelService.getLabelNameByKeyId("4028831334d4c04c0134d4c04e980036")%></a><!-- 更改 -->
				    </td>
				</tr>
				<tr id="uploadFile" <%if(StringHelper.null2String(wordModule.getAttachid()).equals("")){%>style="display:block"<%}else{%>style="display:none"<%}%>>
				    <td class=FieldName noWrap><%=labelService.getLabelNameByKeyId("402881eb0bcd354e010bcd9dfe6b0017")%></td><!-- 附件 -->
				    <td class=FieldValue >
				        <input type="file" id="path" style="width:60%" name="path" onchange="toUpLoad()"/>&nbsp;&nbsp;
...
<script language="javascript">
    ...
    WeaverUtil.load(function() {
		...
		jQuery("input[name='docTemplateType']").each(function(){
			if(jQuery(this).is(":checked")){
				onChangeDocType(jQuery(this).val());
			}
		});
	});
    ...
    function toSave(){
        ...
        if(docTemplateType == 4 || docTemplateType == 5){
        	if((file==null||file=="") && (attach==null||attach=="")){
