...
function createEl(parentObj,data){
	...
	for(var i=0;i<data.length;i++){
		...
		l.id = data[i].id;
		l.innerHTML = data[i].col3=="1" ?"<a href=<%=url%>" + data[i].id +"<%=paramater%>"+" >"+data[i].objname+"</a>": data[i].objname;
...
window.onload = function(){
	...
	if(size>0){
		for(int i=0;i<size;i++){
			...
			html += "1".equals(c.getCol3()) ? "<a href=\""+url+c.getId()+paramater+"\" target =\"_blank\"><b>"+c.getObjname()+"</b></a>" : "<b>"+c.getObjname()+"</b>";
			html += "</li></ul>";
						if(i%2==0){
...
<table style="margin-bottom:10px">
	<tr>
		<td class="FieldName" width="10%"><%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd6d34f87000f")%>:<!-- 展开 --></td>
		<td class="FieldValue">
			<select onchange="javascript:location.href='categoryselect.jsp?model=<%=model %>&categoryid=<%=id %>&level='+this.value;">
			<%for(int i=1;i<10;i++){
