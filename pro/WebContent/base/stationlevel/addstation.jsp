...
<%
String levelid=StringHelper.null2String(request.getParameter("levelid"));//岗位级别
...
<!--页面菜单开始-->     
<%
pagemenustr += "{O,"+labelService.getLabelName("402881eb0bcbfd19010bcc6e71870022")+",javascript:onSubmit();}";
//pagemenustr += "{C,关闭,javascript:window.close();}";
%>
...
<%		
	...
	if(selectlist.size()>0){%>
			<SELECT name="reftype" id="reftype" onChange="window.location.href='addstation.jsp?orgid=<%=orgid %>&levelid=<%=levelid %>&type=<%=type %>&reftype='+this.value;">			
	...
	<table  cellspacing="1" align="left" width="100%" class=noborder>
		<tr>
			<td align="center" valign="top" width="10%">
				<img src="<%= request.getContextPath()%>/images/arrow_u.gif" style="cursor:hand" title="<%=labelService.getLabelName("402881e70cbb193c010cbb39e1dd0007")%>" onclick="javascript:upFromList();">
				<br><br>
			...
			<td align="center" valign="top" width="80%">
				<font style="color:red;"><b>已选择</b></font>
				<div style="overflow-x:scroll;width:400;">
					<select size="20"  name="srcList" id="srcList" multiple="true" style="width:800" class="InputStyle">
					</select>
				</div>
			</td>
		</tr>
...
function onSubmit() {
	var ids=getStr("key");
    EweaverForm.action="<%= request.getContextPath()%>/ServiceAction/com.eweaver.base.stationlevel.servlet.StationlevellinkAction?action=addlink&ids="+ids;
	EweaverForm.submit();
}
...
function doUrl(key){
	...
	if(!isExistEntry(key,document.getElementById("srcList"))){
		document.getElementById("srcList").options.add(oOption);
		oOption.value = key;
		oOption.innerHTML = getNodetitle(key);	
	}		
}
