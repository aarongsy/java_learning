...
<%
  ...
  String byagentName = StringHelper.null2String(humresService.getHumresById(byagent).getObjname());
    String action=request.getContextPath()+"/ServiceAction/com.eweaver.base.log.servlet.UpgradeAction?action=search";
          ...
          Ext.LoadMask.prototype.msg='加载中,请稍后...';
             ...
             store = new Ext.data.Store({
                 ...
                 reader: new Ext.data.JsonReader({
                    ...
                    fields: ['status','errordate','errortime','errorfile','errorrow','errorsql','errorstr','id']
...
//                         {header: "编号", width:50, sortable: false,  dataIndex: 'objno'},
                         {header: "脚本升级状态", width:50, sortable: false,  dataIndex: 'status'},
                         {header: "日期", sortable: false, width:50,dataIndex: 'errordate'},
                         {header: "时间", width:50, sortable: false, dataIndex: 'errortime'},
                         {header: "文件名", width:50, sortable: false, dataIndex: 'errorfile'},
                         {header: "出错行", width:50, sortable: false, dataIndex: 'errorrow'},
                         {header: "出错语句", width:50, sortable: false, dataIndex: 'errorsql'},
                         {header: "错误信息", width:50, sortable: false, dataIndex: 'errorstr'}
                         ]);
...
<!--页面菜单开始-->
  ...
  <div id="divSearch">
    <div id="pagemenubar"></div>
     <form  name="EweaverForm" id="EweaverForm" method="post" onsubmit="return false">
      ...
      <table id=searchTable>
        ...
        <tr>
                  ...
                  <td class=FieldValue colspan="5">
                    <input type=text class=inputstyle size=10 name="errordate"  value="" onclick="WdatePicker()" >
                    <span id="errordatespan"   ></span>-
                    <input type=text class=inputstyle size=10 name="errordate1"  value="" onclick="WdatePicker()" >
                    <span id="errordate1span"  >
        ...
        <tr>
                  ...
                  <td class=FieldValue colspan="5">
                    <input type=text class=inputstyle size=10 name="errortime"  value="" onclick="WdatePicker({skin:'whyGreen',dateFmt:'H:mm:ss'})" >
                    <span id="errortimespan"   ></span>-
                    <input type=text class=inputstyle size=10 name="errortime1"  value="" onclick="WdatePicker({skin:'whyGreen',dateFmt:'H:mm:ss'})" >
                    <span id="errortime1span"  >
        ...
        Ext.MessageBox.confirm('', '您确定要删除吗?', function (btn, text) {
            if (btn == 'yes') {
                Ext.Ajax.request({
                    ...
                    url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.base.log.servlet.UpgradeAction?action=delete',
