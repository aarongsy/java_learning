...
<%
   ...
   String action=request.getContextPath()+"/ServiceAction/com.eweaver.base.security.servlet.PermissionBatchActionAction?action=getPermGroupList";
%>
<%
    pagemenustr +="addBtn(tb,'快捷搜索','S','zoom',function(){search()});";
   ...
   <%}%>
       store = new Ext.data.Store({
           ...
           reader: new Ext.data.JsonReader({
               ...
               fields: ['objname','permissionBatchActions','id']
       ...
       var cm = new Ext.grid.ColumnModel([sm, {header: "名称", width:180, sortable: false,  dataIndex: 'objname'},
           {header: "包含任务", sortable: false, width:620,   dataIndex: 'permissionBatchActions'}
       ]);
    ...
    function onDelete()
        {
            ...
            Ext.MessageBox.confirm('', '您确定要删除吗?', function (btn, text) {
                if (btn == 'yes') {
                    Ext.Ajax.request({
                        url: '<%=request.getContextPath()%>/ServiceAction/com.eweaver.base.security.servlet.PermissionBatchActionAction?action=deletegroup',
