...
<%
String action=request.getContextPath()+"/ServiceAction/com.eweaver.base.ipset.servlet.IpsetAction?action=getipsetlist";
   ...
   <script type="text/javascript" src="<%= request.getContextPath()%>/js/ext/ux/miframe.js"></script>
         <script language="javascript">
          Ext.LoadMask.prototype.msg='<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>';//加载中,请稍后...
             ...
             store = new Ext.data.Store({
                 ...
                 reader: new Ext.data.JsonReader({
                     ...
                     totalProperty: 'totalcount',
                       fields: ['ipname','ipstart','ipend','modify','id']
             ...
             var cm = new Ext.grid.ColumnModel([sm, {header: "<%=labelService.getLabelNameByKeyId("402883d7353baf7101353baf7379007d")%>", width:150, sortable: false,  dataIndex: 'ipname'},//内网IP名称
                         {header: "<%=labelService.getLabelNameByKeyId("402883d7353baf7101353baf7379007e")%>", sortable: false, width:100 ,dataIndex: 'ipstart'},//内网IP开始地址
                         {header: "<%=labelService.getLabelNameByKeyId("402883d7353baf7101353baf7379007f")%>", width:100, sortable: false, dataIndex: 'ipend'},//内网IP结束地址
                         {header:"" , width:100, sortable: false, dataIndex: 'modify'}
...
<script language="javascript">
 <!--
    function onCreate(){
     var url="<%= request.getContextPath()%>/base/ipset/ipsetcreate.jsp";
	 window.location.href=url;
   }
   function onModify(id){                                                                                 
     var url="<%= request.getContextPath()%>/base/ipset/ipsetmodify.jsp?id="+id;
	 window.location.href=url;
   }
   function onDelete(id){
        ...
        Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39380075")%>', function (btn, text) {//您确定要删除吗?
                     if (btn == 'yes') {
                         Ext.Ajax.request({
                             url: '<%= request.getContextPath()%>/ServiceAction/com.eweaver.base.ipset.servlet.IpsetAction?action=delete',
