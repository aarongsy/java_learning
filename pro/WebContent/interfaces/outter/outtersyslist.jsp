...
<%
    pagemenustr += "addBtn(tb,'"+labelService.getLabelName("402881eb0bcbfd19010bcc7bf4cc0028")+"','C','add',function(){onCreate('/interfaces/outter/outtersyscreate.jsp')});";
   ...
   <%}%>
    store = new Ext.data.Store({
        ...
        reader: new Ext.data.JsonReader({
            ...
            fields: ['objname','inneradd','outteradd','username','pass','sysid','id']
        })
    ...
    {header: "<%=labelService.getLabelNameByKeyId("402883d7353baf7101353baf7378000f")%>", sortable: false,   dataIndex: 'username'},//外网地址
  ...
  function ondelete()
  {
     var totalsize = selected.length;
              if (totalsize == 0) {
               ...
               Ext.Msg.buttonText={yes:'<%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd6d13003000c")%>',no:'<%=labelService.getLabelNameByKeyId("402881eb0bd66c95010bd6d19cf5000d")%>'};//是
              Ext.MessageBox.confirm('', '<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39380075")%>', function (btn, text) {//您确定要删除吗?
                  if (btn == 'yes') {
                      Ext.Ajax.request({
                          url: ' <%= request.getContextPath()%>/ServiceAction/com.eweaver.interfaces.outter.servlet.OuttersysAction?action=delete',
