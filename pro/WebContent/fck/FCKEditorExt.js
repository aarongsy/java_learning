...
(function(){
	...
	FCKEditorExt={
		...
		insertDOC:_insertDOC, //插入文档标题连接
		docinser_show:_docinser_show, //插入文档标题连接(指定鼠标位置)
		Ok:_Ok,
		Cancel:_Cancel,
		show:_show,
		...
		DEFAULT:'eweaver',
		NO_IMAGE:'eweaver',//'eweaverNoImage',
		WEB_IMAGE:'eweaver',
		REPORT:'report',
...
function _initEditor(name,isEnglish,toolbar){
	isEnglish=false;
...
*/		
	...
	function _InsertFlashObject(ename){//插入Flash操作执行
		...
		if($('isFlashVideoUrl').checked){
			...
			if (win.FCKFlashProcessor && typeof(win.FCKFlashProcessor) != "undefined") {
	...
	/**
	** 从brow框中选择文档（多选）插入到文档编辑器中 cjl 2011-08-10
	**/
	function _insertDOC(htmlStr){
            var sHtml = htmlStr;
	...
	/**
	** 从brow框中选择文档（多选）插入到文档编辑器中 使sel不为空 cjl 2011-08-10
	**/
	function _docinser_show(sel){
		this._sel=sel;
	}
