...
FCKConfig.FontSizes		= '8pt;9pt;10pt;11pt;12pt;14pt;16pt;18pt;20pt;24pt;28pt;32pt;36pt;48pt;60pt' ;

FCKConfig.ToolbarSets["eweaver"] = [
	...
	'/',
	['Bold','Italic','Underline','StrikeThrough'],['FontFormat','FontName','FontSize'],
...
/*add by cjl 2011-08-10*/
if(typeof(window.parent.insertDOC)=="string" && window.parent.insertDOC!="" && window.parent.insertDOC=="true"){//检测是否在编辑文档时添加 插入文档按钮
	var btns=FCKConfig.ToolbarSets["eweaver"][9];
	FCKConfig.ToolbarSets["eweaver"][9]=new Array(btns[0],btns[1],btns[2],btns[3],'DocInsert',btns[4]);
}

if(typeof(window.parent.flvBrowserUrl)=="string" && window.parent.flvBrowserUrl!=""){//检测是否加载视频按钮
	var btns=FCKConfig.ToolbarSets["eweaver"][5];
	FCKConfig.ToolbarSets["eweaver"][5]=new Array(btns[0],btns[1],'FlashVideo',btns[2],btns[3],btns[4]);
}

FCKConfig.ToolbarSets["eweaverNoImage"] = [
	['Bold','Italic','Underline','StrikeThrough'],
	['Cut','Copy','Paste','Undo','Redo','-','PasteText','PasteWord','SelectAll','RemoveFormat'],
	['OrderedList','UnorderedList','-','Outdent','Indent'],
	['JustifyLeft','JustifyCenter','JustifyRight','JustifyFull'],
...
//FCKConfig.ToolbarSets["Default"]=FCKConfig.ToolbarSets["eweaverNoImage"];
...
/****begin 从browres框中选择文档添加******/
function FCKInsertDocCommand(){//插入自定义文档添加命令
  this.name='DocInsert';
  this.Execute=function(){
    if(document.all)
	  sel=FCK.EditorDocument.selection.createRange();
	  ...
	  sel=FCK.EditorWindow.getSelection();
    window.parent.FCKEditorExt.docinser_show({selection:sel,ename:FCK.Name});
    var htmlStr = window.parent.FCKEditorExt.getHtml(); //去到编辑区域的源文件内容
	...
	if(_id==""&&_name==""){
	    ...
	    var htmlStr = [_name];
	    window.parent.FCKEditorExt.insertDOC(htmlStr);
	}
  };
  this.GetState=function(){return 0;};
};
/*****end******/
/****begin 注册右键菜单 （右键删除文档链接）*******/
...
FCKCommands.RegisterCommand('DocInsert',new FCKInsertDocCommand());
...
FCKToolbarItems.RegisterItem('DocInsert',new FCKToolbarButton( 'DocInsert',	FCKLang.DocInsert,
																null,2, false, true, 5 ));
/******注册右键菜单******/																
