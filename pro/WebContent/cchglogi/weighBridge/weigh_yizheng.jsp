...
<div style="width: 1160px;height: 490px;">
	<div id="header">
		...
		<div id="headerdw" class="headerdw"><span>kg</span><input type="text" onchange="cg(this)" />
		
		<input type="button" value="open" onclick="OpenPort()"/>
		<input type="button" value="close" onclick="ClosePort()"/>
	...
	<div id="right">
		<div class="right_bb">
			<table>
				...
				<tr>
					<td><input class="button blue" id="inweigh" type="button" value="计重" onclick="inweigh()"/></td>
				</tr>
				<tr>
					<td><input class="button blue" id="outweigh" type="button" value="过磅" onclick="outweigh()"/></td>
				</tr>
				<tr>
...
function MSComm1_OnComm(){    
	if(MSComm1.CommEvent==1){//如果是发送事件    
		...
		we2 = we1.trim() + we.trim();
		we1 = we.trim();
		if(we2.trim().indexOf(" ")>2 && we2.trim().indexOf(" ")<8 && we2.trim().length >8){
			//$("#weight").html($("#weight").html()+"|"+we2.trim()+"-"+we2.trim().indexOf(" "));
			$("#weight").html(we2.trim().substr(we2.trim().indexOf(" ")+1,5));
		}
	}
}    
