...
<%
String action=request.getContextPath()+"/ServiceAction/com.eweaver.app.sap.orgunit.OrgunitSyncAction?action=search";
%>
<%
	...
	//if(rolist.size()>0){
		pagemenustr +=  "addBtn(tb,'同步该组织下的组织','T','erase',function(){syncOrg()});";	
		pagemenustr +=  "addBtn(tb,'同步该组织下的岗位','T','erase',function(){syncSta()});";	
	//}
         ...
         <script language="javascript">
          Ext.LoadMask.prototype.msg='<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>';//加载中,请稍后...
			...
			var cm = new Ext.grid.ColumnModel([sm,
			         {header:'对象标识',dataIndex:'objid',hidden:false,width:30,sortable:true},
			         {header:'对象名称',dataIndex:'stext',hidden:false,width:30,sortable:true},
			         {header:'公司代码',dataIndex:'bukrs',hidden:false,width:30,sortable:true},
			         {header:'对象缩写',dataIndex:'short',hidden:true,width:30,sortable:true},
			         {header:'对象类型',dataIndex:'otype',hidden:false,width:30,sortable:true},
			         {header:'开始日期',dataIndex:'begda',hidden:true,width:30,sortable:true},
			         {header:'结束日期',dataIndex:'endda',hidden:true,width:30,sortable:true},
			         {header:'负责岗位',dataIndex:'plans',hidden:true,width:30,sortable:true},
			         {header:'负责岗位所在组织',dataIndex:'orgeh',hidden:true,width:30,sortable:true},
			         {header:'组织层级',dataIndex:'omlvl',hidden:false,width:30,sortable:true},
			         {header:'上级组织',dataIndex:'parent',hidden:false,width:30,sortable:true},
			         {header:'顺序号',dataIndex:'seqnr',hidden:false,width:30,sortable:true}
			         ]);       
...
<div id="divSearch">
 ...
 <table>
   ...
   <tr>
 	<td  class="FieldName" nowrap>组织对象标示</td>
    <td  class="FieldValue"  nowrap>
           <input type=text class=inputstyle style="width:80%" name="orgeh" id="orgeh" value="10100000" >
    </td>
    <td  class="FieldName" nowrap>关键日期</td>
    <td  class="FieldValue"  nowrap>
...
<script language="javascript">
   ...
   function syncOrg(){
	   myMask1.show();
	   setTimeout(syncDo('<%=request.getContextPath()%>/ServiceAction/com.eweaver.app.sap.orgunit.OrgunitSyncAction'),1000);
   }
   function syncSta(){
	   myMask1.show();
	   setTimeout(syncDo('<%=request.getContextPath()%>/ServiceAction/com.eweaver.app.sap.orgunit.StationSyncAction'),1000);
   }
