...
if(submitaction!=null&&submitaction.equals("submit"))
{
	...
	**/
	...
	nuserid=StringHelper.null2String(request.getParameter("nuserid"));
	workflowids=StringHelper.null2String(request.getParameter("workflowids"));
	String[] reqs = workflowids.split(",");
	int nums=0;
	for(int i=0;i<reqs.length;i++){
		String requestid = reqs[i];
		workflowService.replaceOperator(requestid,ouserid,nuserid);
		nums=nums+1;
	}
	throwstr="工作移交成功! 本次移交"+nums+"项!";
}
submitaction="";
%>
<html>
...
<br><br><br>
        ...
        <table style="border:0">
			...
			<TR><TD class=Line colspan=3 style="	height: 1px;	background-color: #C2D5FC;"></TD><TR>
			<TR >
			<TD class=FieldName noWrap>待办流程</TD>
			<TD class=FieldValue colspan=2 style="height:100px" >
			<button type=button  class=Browser name="button_workflowids" onclick="javascript:getFlows();"></button><input type="hidden" id="workflowids" name="workflowids" value="<%=workflowids%>"  style='width: 80%'  ><span id="workflowidsspan" name="workflowidsspan" ><%=getBrowserDicValue("workflowinfo","id","objname",workflowids)%><%if(workflowids.length()<1){%><img src="/images/base/checkinput.gif" align=absMiddle><%}%></span>
			</tr>
		</TR>
		<TR><TD class=Line colspan=3 style="	height: 1px;	background-color: #C2D5FC;"></TD><TR>
		<tr><td colspan=3 align="center"  ><br><button onclick="onSubmit()" class="btn" >提交处理</button>&nbsp;&nbsp;&nbsp;
		<!-- <button onclick="document.forms[0].reset();" class="btn">取消处理</button></td></tr> -->
...
function getFlows(){
	var params = 'sqlwhere=userid=\''+document.getElementById('ouserid').value+'\'';
	getrefobj('workflowids','workflowidsspan','40285a904ab33d9e014ab9631a677bbd',params,'/workflow/request/workflow.jsp?requestid=','0');
}
</script>
</html>
