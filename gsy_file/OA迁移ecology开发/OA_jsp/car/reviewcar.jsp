...
<%	int pagesize=20;
   ...
   // String isapprove = StringHelper.null2String(request.getParameter("isapprove"));
    ...
    String sql="select state from uf_lo_dgcar   where requestid='"+id+"'";
    //System.out.println(sql);
    String  state=(String)baseJdbcDao.getJdbcTemplate().queryForObject(sql, String.class);
    if(state.equals("402864d14931fb790149328a92bd0015"))
        pagemenustr +="addBtn(tb,'"+labelService.getLabelName("审核")+"','R','accept',function(){onSubmit(2)});";
    if(state.equals("402864d14931fb790149328a92bd0016")){
    	pagemenustr +="addBtn(tb,'"+labelService.getLabelName("撤销")+"','C','cancel',function(){cancel()});";
    	layoutid="40285a904931f62b01493c4b7f4558c8";
    	editmode="0";
    }
...
if(bNewworkflow.equals("1")||editmode.equals("1")){
	//System.out.println(layoutid+"-------------"+categoryid);
	//保存
 ...
 //*************************提交之前将用户没有修改的当前时间字段标识一下,交给后台更新(start)**************************//
    ...
    //*************************提交之前将用户没有修改的当前时间字段标识一下,交给后台更新(end)**************************//
               disabledToolbar();
           	 ...
           	 document.getElementById("field_402864d14931fb790149328e47f50023").value="402864d14931fb790149328a92bd0016";
          // 	document.getElementById("field_402864d14931fb790149328e47f50023span").innerText="402864d14931fb790149328a92bd0016";
           	var jq = jQuery.noConflict();    
           	jq.ajax({
           					cache: true,
           					type: "POST",
           					url:'/ServiceAction/com.eweaver.workflow.request.servlet.FormAction',
           					data:jq('#EweaverForm').serialize(),// 你的formid
           					async: false,
           				    ...
           				    success: function(data) {
           				  	...
           				  	jq.ajax({
               					cache: true,
               					type: "POST",
               					url:'/app/car/updatedata.jsp?editmode=3',
               					data:{"requestid":"<%=id%>"},
               				    error: function(request) {
               				        alert("回写订单数据错误");
               				    },
           				    	...
           				    	window.location.href='/app/car/reviewcar.jsp?editmode=0&categoryid=40285a904931f62b01493c2a3998560f&layoutid=40285a904931f62b01493c4b7f4558c8&requestid=<%=id%>';
           				    }
           				});
...
function cancel(){
  	var jq = jQuery.noConflict();    
	  	jq.ajax({
			cache: true,
			type: "POST",
			url:'/app/car/updatedata.jsp?editmode=4',
			data:{"requestid":"<%=id%>"},
		    error: function(request) {
		        alert("回写订单数据错误");
		    },
	    	...
	    	window.location.href='/app/car/reviewcar.jsp?editmode=1&categoryid=40285a904931f62b01493c2a3998560f&layoutid=40285a904931f62b01493c2b7de55610&requestid=<%=id%>';
}
...
<script type="text/javascript" src="/js/browserHackOfForm.js"></script>t.PermissiondetailAction?objtable=formbase&objid="+id+"','"+labelService.getLabelName("权限列表")+"','script_go');";
	}
    if(permissiondetailService.checkOpttype(id, 105)){
		pagemenustr += "addBtn(tb,'"+labelService.getLabelName("删除")+"','D','delete',function(){onDelete('"+request.getContextPath()+"/workflow/request/formbase.jsp?requestid=del')});";
	}
