...
<%
	...
	if(action.equals("getData"))
	{
		...
		String gs = StringHelper.null2String(request.getParameter("gs"));//公司代码
                //String sql = "select objno from getcompanyview where requestid = '"+gs+"'";
		...
		if(FLAG.equals("X"))
		{
			...
			String VKORG = newretTable.getString("VKORG");//xs

			String LGORT = newretTable.getString("LGORT");//ckdd

			String LGFSB = newretTable.getString("LGFSB");//ylkcdd

			String LGPRO = newretTable.getString("LGPRO");//fhckd

			String BZJG = newretTable.getString("BZJG");//bzjg

			String upsql = "update uf_oa_bppbkgs set saperrinfo ='"+ERR_MSG+"',sapreturnflag='"+FLAG+"',sappl='"+DISLS+"',saplrzx='"+PRCTR+"',sapmz='"+BRGEW+"',sapwlhms='"+MAKTX+"',sapsrz='"+BSTRF+"' ";
			upsql = upsql +",sapcgjzdm='"+EKWSL+"',sapjz='"+NTGEW+"',sapcprkclsj='"+WEBAZ+"',sapzxl='"+BSTMI+"',sapzdl='"+BSTMA+"',sapcpjhsj='"+PLIFZ+"',sapjwlh='"+BISMT+"'";
			upsql = upsql +",sapgdl='"+BSTFE+"',sapwlz='"+MATKL+"',sapcgz='"+EKGRP+"',newsapwlh= '"+MATNR+"',sapxszz='"+LGPRO+"',sapckdh='"+LGORT+"',sapylck='"+LGFSB+"',sapbzj= '"+BZJG+"'";
