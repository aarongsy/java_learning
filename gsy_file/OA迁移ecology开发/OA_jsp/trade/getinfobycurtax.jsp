...
String str = StringHelper.null2String(request.getParameter("str"));//币种与税码的拼接字段
...
<%
String delsql = "delete uf_tr_kjmxtemp where requestid = '"+requestid+"'";
baseJdbc.update(delsql);

String sql = " select sno,accountcode,subject,taxcode,estamount,estcurrency,costcenter,purorder,purorderitem,text1,flag from uf_tr_imfeedtitem where requestid = '"+requestid+"' and flag = '"+str+"' order by estamount asc,accountcode asc";
List sublist2 = baseJdbc.executeSqlForList(sql);
if(sublist2.size()>0){
	for(int k=0,sizek=sublist2.size();k<sizek;k++){
		...
		String insql = "insert into uf_tr_kjmxtemp(id,requestid,sno,accountcode,subject,taxcode,estamount,estcurrency,costcenter,purorder,purorderitem,text1,flag)values('"+IDGernerator.getUnquieID()+"','"+requestid+"','"+no+"','"+accountcode+"','"+subject+"','"+taxcode+"','"+estamount+"','"+estcurrency+"','"+costcenter+"','"+purorder+"','"+purorderitem+"','"+text1+"','"+flag+"')"; 
