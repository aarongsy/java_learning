	...
	//if (action.equals("getData")){
		...
		for (int i = 0; i < pzstr.length; i++)
		{
			...
			*/
			...
			function.getImportParameterList().setValue("LIFNR",supstr[i]);//供应商
			//建表
			...
			System.out.println(companycode);
			System.out.println(supstr[i]);
			System.out.println(pzstr[i]);
			System.out.println(yearstr[i]);
			try {
			...
			if(function.getExportParameterList().getValue("ERR_MSG").toString().indexOf("失败")!=-1)
			{
				ERR_MSG = ERR_MSG+function.getExportParameterList().getValue("ERR_MSG").toString();
				FLAG = FLAG+function.getExportParameterList().getValue("FLAG").toString();
			}
