...
//31行项目 发票明细
int count=0;
String sql = "select b.suppliercode,a.invoicemoney,b.patitemcode,b.paybenchdate,b.payfreeze,(select objdesc from selectitem where id=b.paytype) as paytype,b.paycurrency,a.paymoney,b.banktype  from uf_tr_exfeeinvoiceinfo  a left join uf_tr_exfeeclearmain b on a.requestid = b.requestid where  a.requestid = '"+requestid+"' and a.invoicemoney is not null and a.invoicemoney>0 ";
...
//汇兑差=本位币差额-（业务货币差额*清账汇率） //qzhl
	...
	if(sublist.size() >0)
	{
		Double hdc = 0.00;//汇兑差
		String jzcode="";//记账码
		String hdaccount = ""; //汇兑损益科目 xxy 20180118 add	若为贷方（记账码50）则是汇兑收益-已实现（ 55030311 ），若为借方（记账码40）则为汇兑损失-已实现(55030312)
		...
		if(requestid.equals("40285a905c6ca930015c800fa33056cd"))
		{
			hdc=hdc+0.01;
		}
		if(!fpcurren.equals(zgcurren))
		{
			...
			if(!jzcode.equals(""))
			{
				...
				insql = "insert into uf_tr_exfeeclearitem(id,requestid,sno,accountcode,subject,money,taxcaode,costcenter,text1)values('"+IDGernerator.getUnquieID()+"','"+requestid+"',"+count+",'"+jzcode+"','"+hdaccount+"',"+Math.abs(Double.valueOf(hdc))+",'','"+cscen+"','汇兑差')";
				baseJdbc.update(insql);
				System.out.println(insql);
			}
