...
if(sublist.size()>0){
	for(int k=0,sizek=sublist.size();k<sizek;k++){
		...
		if(no <sublist.size())//
		{
			...
			else //如果下一个费用名称与当前的费用名称一致，则说明当前的费用名称不是最后一个。
			{
				...
				if(sharebase.equals("40285a90497a8f7801497d9670120069")&&!(netpricesums.equals("")||netpricesums.equals("0")))//按照发票金额进行分摊、（净价值/发票金额）
				{
					 
					 amort = (Double.valueOf(netpricesum)/Double.valueOf(netpricesums));//金额
				...
				else//按照发票数量进行分摊\（净价值/发票金额）（本次出货数量/发票数量）*暂估金额
				{
					...
					amort = (Double.valueOf(shiptotal)/Double.valueOf(shiptotals));//数量
					 zgmon= Double.valueOf(amount) *amort;//暂估金额
					 zgrmbmon = Double.valueOf(rmbamount) *amort;//暂估本位币金额
				}
			}
	...
	<%
		...
		String zxsql="select a.cabtype,count(distinct(a.cabno)) as num,(select objname from selectitem where id=a.isdager) as isdager,a.dangelv from uf_tr_packtype a left join uf_tr_expboxmain  b on a.requestid=b.requestid left join requestbase c on c.id =b.requestid where instr ('"+str+"',a.requestid)>0 and 0=c.isdelete  and (b.isvalid='40288098276fc2120127704884290210' or b.isvalid is null) group by a.cabtype,a.isdager,a.dangelv";
		...
		if(sublist1.size()>0){
			for(int k=0,sizek=sublist1.size();k<sizek;k++)
			{
				...
				String isdager = StringHelper.null2String(mk1.get("isdager"));//
				String wxlevel= StringHelper.null2String(mk1.get("dangelv"));//
				String insql ="insert into uf_tr_exfeedtpacking  (id,requestid,sno,cabtype,cabno,wpdis,wxlevel) values('"+IDGernerator.getUnquieID()+"','"+requestid+"','"+(k+1)+"','"+cabtype+"','"+num+"','"+isdager+"','"+wxlevel+"')";
				baseJdbc.update(insql);
