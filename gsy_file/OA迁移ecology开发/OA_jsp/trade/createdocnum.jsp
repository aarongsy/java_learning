...
<%
	...
	if (action.equals("getData")){
		...
		String tax = StringHelper.null2String(request.getParameter("taxname"));//税码
		System.out.println(tax);
		String pztitle = StringHelper.null2String(request.getParameter("pztitle"));//抬头文本
		String strflag =cur+"_"+tax;
		...
		String sql = "select accountcode,subject,taxcode,estamount,estcurrency,costcenter,purorder,purorderitem,text1,flag from uf_tr_imfeedtitem  where requestid='"+requestid+"' and flag = '"+strflag+"' order by sno asc";
		...
		if(list.size()>0){
			for(int i=0;i<list.size();i++){
				...
				String purorder = StringHelper.null2String(map.get("purorder"));
				String purorderitem = StringHelper.null2String(map.get("purorderitem"));
				...
				retTable.setValue("TAX_CODE", taxcode);//税码
