...
<%
String action=request.getContextPath()+"/ServiceAction/com.eweaver.app.trade.servlet.SelSaleOrderAction?action=search";
%>
<%
         ...
         <script language="javascript">
          Ext.LoadMask.prototype.msg='<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>';//加载中,请稍后...
			...
			var cm = new Ext.grid.ColumnModel([sm,
			                                   {header:'销售订单编号',dataIndex:'sono',hidden: false ,width:30,sortable: true},
											   {header:'项次',dataIndex:'soitem',hidden: false ,width:30,sortable: true},
											   {header:'物料号',dataIndex:'wlh',hidden: false ,width:30,sortable: true},
											   {header:'物料描述',dataIndex:'shortxt',hidden: false ,width:30,sortable: true},
											   {header:'订单数量',dataIndex:'quantity',hidden: false ,width:30,sortable: true},
											   {header:'单位',dataIndex:'unit',hidden: false ,width:30,sortable: true},
											   {header:'工程修改',dataIndex:'gcxg',hidden: true ,width:30,sortable: true},
											   {header:'库位',dataIndex:'whloc',hidden: true ,width:30,sortable: true},
											   {header:'批次',dataIndex:'batchno',hidden: true ,width:30,sortable: true},
											   {header:'预定交货日',dataIndex:'shipdate',hidden: false ,width:30,sortable: true},
											   {header:'基本单位数量',dataIndex:'basequan',hidden: true ,width:30,sortable: true},
											   {header:'基本单位',dataIndex:'baseunit',hidden: true ,width:30,sortable: true},
											   {header:'净价',dataIndex:'price',hidden: true ,width:30,sortable: true},
											   {header:'净价值',dataIndex:'amount',hidden: true ,width:30,sortable: true},
											   {header:'包装描述',dataIndex:'packdesc',hidden: false ,width:30,sortable: true},
											   {header:'佣金',dataIndex:'commission',hidden: true ,width:30,sortable: true},
			                                   {header:'销售订单类型',dataIndex:'ordertype',hidden: true ,width:30,sortable: true},
			                                   {header:'类型描述',dataIndex:'ordertypedes',hidden: false ,width:30,sortable: true},
			                                   {header:'采购订单编号',dataIndex:'pono',hidden: true ,width:30,sortable: true},
			                                   {header:'利润中心',dataIndex:'profit',hidden: true ,width:30,sortable: true},		                                   
			                                   {header:'特殊处理标记',dataIndex:'speflag',hidden: true ,width:30,sortable: true},
			                                   {header:'特殊处理标记描述',dataIndex:'speflagdesc',hidden: true ,width:30,sortable: true},
			                                   {header:'售达方',dataIndex:'sellto',hidden: true ,width:30,sortable: true},
			                                   {header:'售达方名称',dataIndex:'selltoname',hidden: false ,width:30,sortable: true},
			                                   {header:'售达方国别',dataIndex:'selltocontry',hidden: true ,width:30,sortable: true},
			                                   {header:'售达方地址',dataIndex:'selltoaddr',hidden: true ,width:30,sortable: true},
			                                   {header:'送达方',dataIndex:'sendto',hidden: true ,width:30,sortable: true},
			                                   {header:'送达方名称',dataIndex:'sendtoname',hidden: false ,width:30,sortable: true},
			                                   {header:'送达方国别',dataIndex:'sendtocontry',hidden: true ,width:30,sortable: true},
			                                   {header:'送达方地址',dataIndex:'sendtoaddr',hidden: true ,width:30,sortable: true},
			                                   {header:'付款条款代码',dataIndex:'paytermcode',hidden: true ,width:30,sortable: true},
			                                   {header:'付款条款描述',dataIndex:'paytermdesc',hidden: false ,width:30,sortable: true},
			                                   {header:'国贸条件1',dataIndex:'icon1',hidden: true ,width:30,sortable: true},
			                                   {header:'国贸条件描述',dataIndex:'icon1desc',hidden: false ,width:30,sortable: true},
			                                   {header:'国贸条件2',dataIndex:'icon2',hidden: false ,width:30,sortable: true},
			                                   {header:'币种	',dataIndex:'currency',hidden: true ,width:30,sortable: true},
			                                   {header:'付款方式代码',dataIndex:'paycode',hidden: true ,width:30,sortable: true},
			                                   {header:'付款方式描述',dataIndex:'paycodedesc',hidden: false ,width:30,sortable: true},
											   {header:'公司代码',dataIndex:'comcode',hidden: false ,width:30,sortable: true},
			                                   {header:'公司名称',dataIndex:'company',hidden: true ,width:30,sortable: true}
			         							]);       
...
//'sono','soitem','wlh','shortxt','quantity','unit','gcxg','whloc','batchno','shipdate','basequan','baseunit','price','amount','packdesc',                 	      	'commission','ordertype','ordertypedes','pono','profit','speflag','speflagdesc','sellto','selltoname','selltocontry','selltoaddr',							'sendto','sendtoname','sendtocontry','sendtoaddr','paytermcode','paytermdesc','icon1','icon1desc','icon2','currency','paycode',							'paycodedesc','comcode','company'
		...
		'^@^'+rec.get('sendtoaddr')+'^@^'+rec.get('paytermcode')+'^@^'+rec.get('paytermdesc')+'^@^'+rec.get('icon1')+'^@^'+rec.get('icon1desc')+'^@^'+rec.get('icon2')+'^@^'+rec.get('currency')+'^@^'+rec.get('paycode')+'^@^'+rec.get('paycodedesc')+'^@^'+rec.get('comcode')+'^@^'+rec.get('company');
...
<div id="divSearch">
 ...
 <form id="EweaverForm" name="EweaverForm" action="<%=action%>">
  <table>
   ...
   <tr>
 	<td  class="FieldName" nowrap>销售订单号</td>
