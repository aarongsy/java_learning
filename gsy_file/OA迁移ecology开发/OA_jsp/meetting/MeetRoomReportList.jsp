...
String where=" and requestid in ("+whereids+")";
	...
	if(SQLMap.DBTYPE_ORACLE.equals(dbtype)){
		sql = "select t.requestid,t.objname,t.place objid,t.starttime,t.finishtime,t.contel,host,handler,style,place,place2,Mannumber,sump from (select requestid,objname,host,handler,style,place,place2,Mannumber,sump, beginDate,beginTime,endDate,endTime,description,beginDate||' '||beginTime  starttime,endDate||' '||endtime finishTime,contel from uf_meeting  x where 1=1 "+where+" and exists(select id from requestbase where id=x.requestid and isdelete=0) ) t order by starttime";
	}
	...
	for(int k=0,size=dataList.size() ; k<size ; k++) {
		...
		String Mannumber = StringHelper.null2String(m.get("sump")) ;
		String contel = StringHelper.null2String(m.get("contel")) ;
		...
		buf1.append("<td align=\"center\">"+contel+"</td>");
		buf1.append("</tr>");
	}
%>
<table cellspacing="0" cellpadding="0" border="1" style="border-collapse:collapse;width:100%;BORDER-RIGHT: 1px solid;" bordercolor="#333333" id="mainTable">
 ...
 <tr style="background:#C7CFC5;border:1px solid #c3daf9;" height="20">
	...
	<td colspan="1" align="center" rowspan="2" >联系电话</td>
 </tr>
 <tr style="background:#C7CFC5;border:1px solid #c3daf9;" height="20">
