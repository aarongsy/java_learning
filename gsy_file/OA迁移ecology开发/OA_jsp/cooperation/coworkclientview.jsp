...
String coworkid = StringHelper.null2String(request.getParameter("id"));
String coworkurl = "/app/cooperation/showcowork.jsp";
if(!StringHelper.isEmpty(coworkid)) {
	coworkurl = "/app/cooperation/coworkview.jsp?id="  + coworkid;
}
%>
  <frameset BORDER=0 FRAMEBORDER=no  cols="*" noresize="noresize" name="frameBottom" id="frameBottom"  border="0" style="height: auto;">
	<frame BORDER="0" FRAMEBORDER="no" noresize="noresize" src="<%= request.getContextPath() + coworkurl%>" name="frameRight" id="frameRight" <%if(currentSysModeIsWebsite){%>scrolling="no"<%}else{%>scrolling="auto"<%}%> >
  </frameset>