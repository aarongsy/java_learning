			...
			try {
				...
				if ( today.equals(lastday)  ) {
					if ( createdate.equals(today) ) {
						String createtime =  ds.getSQLValue("select (createdate || ' ' ||  createtime) cretime from formbase where id='"+requestid+"'");
						...
						//System.out.println("currentime="+currentime+"  chktime="+chktime);
						String testsql = "select case when to_date('"+currentime+"','yyyy-MM-dd HH24:MI:SS')>to_date('"+chktime+"','yyyy-MM-dd HH24:MI:SS') then '1' else '0' end from dual";						
						chktime = ds.getSQLValue(testsql);						
						//System.out.println("currentime="+currentime + " chktime="+chktime);
						if ( "0".equals(chktime) ) { 
							tosapflag = false;							
						}
				...
				if ( tosapflag) {
					String a = app.budgetToSapByRequestid(requestid, username, force);
					//System.out.println("tosapflag="+tosapflag);
					//System.out.println("str="+str + "  a="+a);
					jsonObject.put(str, a);
				} else{
					jsonObject.put(str, "不符合执行时间");
				}
			} catch (Exception e) {
