...
if("isInRole".equals(action)){
	...
	response.getWriter().close();
	return;
}else if("needClose_v".equals(action)){
	jsonObject.put("success", "0");
	String requestid = StringHelper.null2String(request.getParameter("requestid"));
	String sql = "update uf_lo_dgcar set vover='40288098276fc2120127704884290210' where requestid in(select requestid from uf_lo_dgcardetail where id='"+requestid+"')";
	BaseJdbcDao baseJdbcDao = (BaseJdbcDao)BaseContext.getBean("baseJdbcDao");
	int i = baseJdbcDao.update(sql);
	if(i>0){
		jsonObject.put("success", "1");
	...
	response.getWriter().close();
	return;
}else if("needOpen_v".equals(action)){
	jsonObject.put("success", "0");
	String requestid = StringHelper.null2String(request.getParameter("requestid"));
	String sql = "update uf_lo_dgcar set vover='40288098276fc2120127704884290211' where requestid in(select requestid from uf_lo_dgcardetail where id='"+requestid+"')";
	BaseJdbcDao baseJdbcDao = (BaseJdbcDao)BaseContext.getBean("baseJdbcDao");
	int i = baseJdbcDao.update(sql);
	if(i>0){
		jsonObject.put("success", "1");
