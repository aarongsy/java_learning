...
<%
	...
	//if (action.equals("getData")){
		...
		String delsql="delete uf_dmsd_exfeeqzno where requestid='"+requestid+"'";
		...
		for (int i = 0; i < pzstr.length; i++)
		{
			...
			retTable.appendRow();
			System.out.println("123333333--------------------"+pzstr[i]);
			...
			if(newretTable.getNumRows() >0)
			{
				for(int j = 0;j<newretTable.getNumRows();j++)
				{
					...
					SGTXT = SGTXT.split("\\,")[0];
					String VBEL2 = newretTable.getString("VBEL2");//销售订单号
					String POSN2 = newretTable.getString("POSN2");//销售订单项次
					//更新数据库中对应的行项信息
					String upsql = "insert into uf_dmsd_exfeeqzno (id,requestid,sno,ordno,orditem,custsuppcode,custsuppflag,ledgerflag,rmbamount,fiscalyear,clearreceiptitem,clearreceiptid,surplusmoney,cleartext)values((select sys_guid() from dual),'"+requestid+"',"+(count+1)+",'"+VBEL2+"','"+POSN2+"','"+HKONT+"','"+KOART+"','"+UMSKZ+"','"+DMBTR+"','"+GJAHR+"','"+EBELP+"','"+BELNR+"','"+WRBTR+"','"+SGTXT+"')";
