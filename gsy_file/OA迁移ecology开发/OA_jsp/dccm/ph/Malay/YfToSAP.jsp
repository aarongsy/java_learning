...
<%
	...
	{	
		...
		String sql = "select a.amount,a.payway,a.expirydate,b.paycode,a.purorder,a.orderitem,a.text,a.banktype from uf_dmph_cwpzdetail a left join uf_tr_paydetail b on a.payway=b.id where a.requestid='"+requestid+"' order by serialno asc";
		...
		if(list2.size()>0)
		{
			for(int m=0;m<list2.size();m++)
			{
				...
				String money = StringHelper.null2String(map2.get("amount"));
				String payment = StringHelper.null2String(map2.get("payway"));
				String todate = StringHelper.null2String(map2.get("expirydate"));
				todate = todate.replace("-", "");
				String paycode = StringHelper.null2String(map2.get("paycode"));
				String therow = StringHelper.null2String(map2.get("orderitem"));
				String conno = StringHelper.null2String(map2.get("purorder"));
				String thetext = StringHelper.null2String(map2.get("text"));
	...
	if (action.equals("WlSAP")){	
		...
		String sql = "select serialno,materno,amount from uf_dmph_wlpzdetail where requestid='"+requestid+"' order by serialno";
		List list2 = baseJdbc.executeSqlForList(sql);
		if(list2.size()>0)
		{
			for(int m=0;m<list2.size();m++)
			{
				Map map2 = (Map)list2.get(m);
				String sno = StringHelper.null2String(map2.get("serialno"));//序号
				String materialno = StringHelper.null2String(map2.get("materno"));
				String money = StringHelper.null2String(map2.get("amount"));
				retTable.appendRow();
		...
		//String insql = "update uf_dmph_sjzgyfmain  set wlproofno = '"+cwpz+"',wlsuccess='"+msgty+"',wlerrormsg='"+message+"' where requestid = '"+requestid+"'";
