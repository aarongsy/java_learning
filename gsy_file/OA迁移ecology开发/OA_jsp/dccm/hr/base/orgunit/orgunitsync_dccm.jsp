...
<%
String action=request.getContextPath()+"/ServiceAction/com.eweaver.app.dccm.dmhr.base.orgunit.DMHR_OrgunitSyncAction?action=search";
%>
<%
	...
	//if(rolist.size()>0){
		pagemenustr +=  "addBtn(tb,'Syn all the organizations belong this Org','T','application_start',function(){syncOrg()});"; //同步该组织下的组织	//Synchronize the organization under the organization
		pagemenustr +=  "addBtn(tb,'Syn all the jobs belong this Org','T','application_start',function(){syncSta()});";	//Synchronize the position of the organization 
	//}
         ...
         <script language="javascript">
          Ext.LoadMask.prototype.msg='<%=labelService.getLabelNameByKeyId("402883d934cbbb380134cbbb39320021")%>';//加载中,请稍后...
			...
			var cm = new Ext.grid.ColumnModel([sm,
			         {header:'OrgSAPId',dataIndex:'objid',hidden:false,width:30,sortable:true},
			         {header:'OrgSAPName',dataIndex:'stext',hidden:false,width:30,sortable:true},
			         {header:'Company Code',dataIndex:'bukrs',hidden:false,width:30,sortable:true},
			         {header:'OrgShort',dataIndex:'short',hidden:true,width:30,sortable:true},
			         {header:'OrgType',dataIndex:'otype',hidden:false,width:30,sortable:true},
			         {header:'Start at',dataIndex:'begda',hidden:true,width:30,sortable:true},
			         {header:'End at',dataIndex:'endda',hidden:true,width:30,sortable:true},
			         {header:'MainJob',dataIndex:'plans',hidden:true,width:30,sortable:true},
			         {header:'MainJobOrg',dataIndex:'orgeh',hidden:true,width:30,sortable:true},
			         {header:'OrgLevel',dataIndex:'omlvl',hidden:true,width:30,sortable:true},
			         {header:'ParentOrg',dataIndex:'parent',hidden:false,width:30,sortable:true},
			         {header:'Serial No.',dataIndex:'seqnr',hidden:false,width:30,sortable:true}
			         ]);       
...
<div id="divSearch">
 ...
 <table>
   ...
   <tr>
 	<td  class="FieldName" nowrap>OrgSAPId</td>
    <td  class="FieldValue"  nowrap>
           <input type=text class=inputstyle style="width:80%" name="orgeh" id="orgeh" value="11700000" >
    </td>
    <td  class="FieldName" nowrap>Key Date</td>
    <td  class="FieldValue"  nowrap>
           <input type=text class=inputstyle style="width:80%" name="overdate" id="overdate" value="" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" >
    </td>
...
<script language="javascript">
   ...
   function onSearch(){
	...
	//   }
	  ...
	  if(!tmpflag){
		alert('please input condition \'OrgSAPId\' at first!')
	  }else{
   ...
   function syncOrg(){
	if(<%=rolist.size() %>==0) {
	   ...
	   myMask1.show();
	   //setTimeout(syncDo('<%=request.getContextPath()%>/ServiceAction/com.eweaver.app.dccm.dmhr.base.orgunit.DMHR_OrgunitSyncAction?p1=1','synchronous'),1000);
	   syncDo('<%=request.getContextPath()%>/ServiceAction/com.eweaver.app.dccm.dmhr.base.orgunit.DMHR_OrgunitSyncAction');
	}
   }
   function syncSta(){
	if(<%=rolist.size() %>==0) {
	   ...
	   myMask1.show();
	   //setTimeout(syncDo('<%=request.getContextPath()%>/ServiceAction/com.eweaver.app.dccm.dmhr.base.orgunit.DMHR_StationSyncAction'),1000);
	   syncDo('<%=request.getContextPath()%>/ServiceAction/com.eweaver.app.dccm.dmhr.base.orgunit.DMHR_StationSyncAction');
	}
   }
   function syncDo(surl){
	if(selected==''){
		alert('Please select the orgunit at first!');
		myMask1.hide();
		return;
	}else{
	   try{
	       ...
	       Ext.Ajax.request({
	    	    ...
	    	    failure: function(){
	    	     Ext.Msg.show({
	    	      ...
	    	      msg: 'error, unable to access the databse!', //'访问数据库时发生错误!'
	    	      buttons: Ext.Msg.OK,
	    	      icon: Ext.Msg.ERROR
	    	     });
	    	    }
