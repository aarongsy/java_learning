...
<%
	...
	String addr=StringHelper.null2String(request.getParameter("addr"));
	String addr2=StringHelper.null2String(request.getParameter("addr2"));
	String postcode=StringHelper.null2String(request.getParameter("postcode"));
	String city=StringHelper.null2String(request.getParameter("city"));
	String phoneno=StringHelper.null2String(request.getParameter("phoneno"));
	...
	if (action.equals("toSAP")){	//修改Address抛SAP
		...
		if( "1".equals(chgflag) ) { //add
			...
			try {
				...
				functionName = "ZHR_PA002_EMERCONTACT_INS_MY"; //PA002:员工个人信息-Emergency Contact新增
				function = null;
				try {
				...
				function.getImportParameterList().setValue("NAME2", name);
				function.getImportParameterList().setValue("STRAS", addr);
				function.getImportParameterList().setValue("LOCAT", addr2);
				function.getImportParameterList().setValue("PSTLZ", postcode);
				function.getImportParameterList().setValue("ORT01", city);
				function.getImportParameterList().setValue("NUM01", phoneno);
		...
		if( "2".equals(chgflag) ) {	
			...
			try {
				...
				functionName = "ZHR_PA002_EMERCONTACT_MOD_MY"; //PA002:员工个人信息-Emergency Contact修改
				function = null;
				try {
					function = SapConnector_EN.getRfcFunction(functionName);
				} catch (Exception e) {
				...
				function.getImportParameterList().setValue("PAKEY_PERNR", oempsapid);
				function.getImportParameterList().setValue("PAKEY_SUBTY", subtype);
				function.getImportParameterList().setValue("PAKEY_OBJPS", objtype);
				function.getImportParameterList().setValue("PAKEY_SPRPS", hrlock);
				function.getImportParameterList().setValue("PAKEY_BEGDA", osdate);
				function.getImportParameterList().setValue("PAKEY_ENDDA", oedate);
				function.getImportParameterList().setValue("PAKEY_SEQNR", seqnr);
				function.getImportParameterList().setValue("NAME2", name);
				function.getImportParameterList().setValue("STRAS", addr);
				function.getImportParameterList().setValue("LOCAT", addr2);
				function.getImportParameterList().setValue("PSTLZ", postcode);
