...
<%
	...
	String birthday=StringHelper.null2String(request.getParameter("birthday"));
	String gender=StringHelper.null2String(request.getParameter("gender"));
	String spousestatus=StringHelper.null2String(request.getParameter("spousestatus"));
	...
	if (action.equals("toSAP")){	//修改Address抛SAP
		...
		if( "1".equals(chgflag) ) { //add
			...
			birthday = ds.getSQLValue("select to_char(to_date('"+birthday+"','YYYY-MM-DD'),'YYYYMMDD') birthday from dual");
			try {
				String functionName = "";
				JCoFunction function = null;
				String errorMessage = "";
			  
				functionName = "ZHR_PA002_SPOUSE_INS_MY"; //PA002:员工个人信息-Spouse新增
				function = null;
				try {
				...
				function.getImportParameterList().setValue("FANAM", name);
				function.getImportParameterList().setValue("FGBDT", birthday);
				function.getImportParameterList().setValue("FASEX", gender);
				function.getImportParameterList().setValue("SEMPS", spousestatus);
		...
		if( "2".equals(chgflag) ) {	
			...
			try {
				...
				functionName = "ZHR_PA002_SPOUSE_MOD_MY"; //PA002:员工个人信息-Spouse修改
				function = null;
				try {
					function = SapConnector_EN.getRfcFunction(functionName);
				} catch (Exception e) {
