...
String sql ="";
	...
	sql= "select id,sno,(select objno from humres where id=supcode)as supcode,supname,supacc,pm,hxarea,yjno,yjrequestid,gxty from uf_tr_bijiazhx a where a.requestid='"+requestid+"' order by a.sno asc";
...
if(sublist.size()>0){
	for(int k=0,sizek=sublist.size();k<sizek;k++){
		...
		String yjno=StringHelper.null2String(mk.get("yjno"));
		String yjrequestid=StringHelper.null2String(mk.get("yjrequestid"));
	%>
	...
	<%
		if(!yjrequestid.equals(""))
		{
	%>
			<TD noWrap  class="td2"  align=center><A href="javascript:newyj('<%=yjrequestid%>','<%=yjno%>');"><FONT color=#ff0000><%=yjno%></FONT></A></TD>
	<%
		}
		else
		{
	...
	<TR><TD class="td2" colspan="8">没有信息</TD></TR>
<%} 
