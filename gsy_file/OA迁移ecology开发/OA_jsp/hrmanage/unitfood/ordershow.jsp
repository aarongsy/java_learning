...
<table id="dataTable" style="BORDER-COLLAPSE: collapse" border=1 cellSpacing=0 cellPadding=0   style="width:2000px;font-size:12" bordercolor="#adae9d">
		...
		<TD class="td2"  align=center>填单人</TD>
		<TD class="td2"  align=center>填单日期</TD>
		<TD class="td2"  align=center>订餐部门</TD>
...
String sql = "";
	sql = "select a.title,(select objname from humres where id=a.reqman) as reqman,(select objname from orgunit where id=a.reqdept) as reqdept,c.objname,a.reqdate,d.objname as reqcom,a.comtype,a.costcenter,a.startdate,a.enddate,a.name,a.customer,a.nums,a.out1,a.out2,a.out3,a.resta1,a.resta2,a.resta3 from uf_hr_customfood a, REQUESTINFO b,nodeinfo c,orgunit d where b.CURRENTNODEID=c.id and b.requestid=a.requestid and d.id=a.reqcom  and  ((substr(a.startdate,0,10) between '"+startdate+"' and '"+enddate+"') or (substr(a.enddate ,0,10) between '"+startdate+"' and '"+enddate+"') or (substr(a.startdate,0,10) <='"+startdate+"' and substr(a.enddate ,0,10)>='"+enddate+"')) and a.requestid in( select rb.id from requestbase rb where rb.isdelete<>1 ) and exists (select   rb.id from requestbase rb  where rb.isdelete=0) and a.comtype='"+comptype+"' ";
...
if(sublist.size()>0){
	for(int k=0,sizek=sublist.size();k<sizek;k++){
		...
		String title =StringHelper.null2String(mk.get("title"));//标题
		String objname =StringHelper.null2String(mk.get("objname"));//当前节点
		String reqdate =StringHelper.null2String(mk.get("reqdate"));//填单日期
		String reqcom =StringHelper.null2String(mk.get("reqcom"));//公司--删
		String costcenter =StringHelper.null2String(mk.get("costcenter"));//成本中心--删
		String startdate1 =StringHelper.null2String(mk.get("startdate"));//订餐开始时间
		String enddate1 =StringHelper.null2String(mk.get("enddate"));//订餐结束时间
		...
		String reqman =StringHelper.null2String(mk.get("reqman"));
	...
	%>
		...
		<TD   class="td2"  align=center><input type="hidden" class="InputStyle" id="<%="count12_"+k%>" style="width:80%" value="<%=reqman%>"><span id="<%="count12_"+k+"span"%>"><%=reqman%></span></TD>
		...
		<TD   class="td2"  align=center><input type="hidden" class="InputStyle" id="<%="count3_"+k%>" style="width:80%" value="<%=reqdept%>"><span id="<%="count3_"+k+"span"%>"><%=reqdept%></span></TD>
