...
<SCRIPT>
function getdata(type1)
{
	
	var startdate=document.getElementById('field_sdate').value;//订餐日期
	var enddate=document.getElementById('field_edate').value;
	var foodtype=document.getElementById('foodtype').value;//餐别
	var comptype=document.getElementById('comptype').value;//厂区别
	if(comptype=='')
	{
		alert('请选择厂区别');
	}
	...
	//刷新
        ...
        myMask.show();          
        Ext.Ajax.request({                                      
             url: '/app/hrmanage/unitfood/showorderinfo.jsp',                                     
             params:{startdate:startdate,foodtype:foodtype,comptype:comptype,enddate:enddate,type1:type1},                                      
...
<TR>
  ...
  <option value=""    ></option>
  <option value="4028804d2083a7ed012083ebb988005b" selected  >常熟厂</option>
  <option value="40285a90488ba9d101488bbd09100007"  >盘锦厂</option>
  <option value="40285a90488ba9d101488bbdeeb30008"  >厂沙厂</option>
  </select>
  </td>
<TD colSpan=2>部门订餐明细<input type="button"  value="查 询"  id="getdate" onclick="getdata('0')"/>
<input type="button" value="外籍用餐查询" onclick="getdata('1')"/> <input type="button" value="导 出" onclick="javascript:exportExcel1('dataTable')"/> </td>
</TR>
