...
<%
	...
	baseJdbc.update(sql);
	if(comtype.equals("4028804d2083a7ed012083ebb988005b")){//常熟厂		
		...
		**/
		...
		int dnums = (int)((ed.getTime() - bd.getTime()) / 1000 / 60 / 60 / 24);		
		Calendar cd= Calendar.getInstance();	
		//根据厂区别循环员工 
		sql = "select a.objno,a.id hid,a.objname hname,a.orgid oid,b.objname oname,a.extmrefobjfield9 comid,c.objname comname from humres a left join orgunit b on a.orgid=b.id left join orgunit c on a.extmrefobjfield9=c.id where a.isdelete=0 and a.hrstatus='4028804c16acfbc00116ccba13802935' and a.extrefobjfield5='"+comtype+"' order by a.objno asc";
		...
		if(list.size()>0){
			for(int n=0;n<list.size();n++){				
				...
				for(int i=0;i<dnums;i++){//循环天数 					
					...
					if(flist.size()>0){
						m = (Map)flist.get(0);
						b = StringHelper.null2String(m.get("breakfast"));//早餐是否登记 
						l = StringHelper.null2String(m.get("lunch"));//午餐是否登记 
						d = StringHelper.null2String(m.get("dinner"));//晚餐是否登记 
					}
					...
					if(b.equals("1")){//早餐有登记 
						...
						//1 判断是否有订餐 
						boolean bredc= false;
						fsql = "select a.breakfast from uf_hr_unitfootsub a where a.breakfast='40285a90495b4eb001496408814f5995' and a.objname='"+hid+"' and a.thedate='"+nday+"' and exists(select b.id from formbase b where a.requestid=b.id and b.isdelete=0)";
						flist = baseJdbc.executeSqlForList(fsql);
						if(flist.size()>0){//有订餐
							bredc = true;
						}
						...
						}else if(!bredc && bresk){//无订餐有刷卡-->未订有用
						...
						}else if(bredc && !bresk){//有订餐无刷卡-->订而未用
					...
					if(d.equals("1")){//晚餐有登记 
						...
						if(fsql.equals("")){//没订餐没刷卡
							...
							if(flist.size()>0){
								m = (Map)flist.get(0);
								String mon = StringHelper.null2String(m.get("money"));
								lm = Double.valueOf(mon).doubleValue();
							}else dm = 0.0;
						}
					}//晚餐 dm 计算结束 
				...
				//把数据插入数据库中
				if(total>0.0){
					String usql = "insert into uf_hr_monthtotalsub (id,requestid,objname,jobno,objdept,objcom,total) values (sys_guid(),'"+requestid+"','"+hid+"','"+objno+"','"+oid+"','"+comid+"',to_number('"+String.format("%.2f",total)+"'))";
