...
<%
//System.out.println("哈哈哈哈哈哈哈哈--------------------------CCPJ");
...
List list;
Map map;
if(action.equals("base1"))
{
	System.out.println("员工信息------------------------------------------------------------------------------------------------------------");
	String delsql="delete from uf_hr_employinfoexport where instr('"+comtype+"',factype)>0 and checkyear='"+curyear+"'";
	...
	for(int i=0;i<sublist.size();i++)
	{
		...
		String insql="insert into uf_hr_employinfoexport (id,requestid,employname,employno,sex,birthday,age,entrydate,workyear,education,title,currentdate,depart,frontscore,lastscore,comtype,factype,checkyear,ssmxid,checktype) values((select sys_guid() from dual),(select sys_guid() from dual ),'"+humresid+"','"+objno+"','"+employsex+"','"+exttimefield9+"','"+age+"','"+extdatefield0+"','"+String.format("%.1f",workyear)+"','"+education+"','"+position+"','"+currectdate+"','"+orgid+"','"+score+"','"+score1+"','"+extrefobjfield5+"','"+extrefobjfield5+"','"+curyear+"','"+ssmxid+"','"+checktype+"')";
		//System.out.println("姓名："+objname+","+"工号："+objno);
...
//考勤信息
if(action.equals("attend1"))
{
	    System.out.println("考勤信息------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		String sapid = "";//sap编号
		...
		sql="select employname as objname,(select exttextfield15 from humres where id=employname) as sapid from uf_hr_employinfoexport where instr('"+comtype+"',factype)>0 and checkyear='"+curyear+"'";
		...
		if(list.size()>0)
		{
			for(int i=0;i<list.size();i++)
			{
				...
				String upsql="update uf_hr_employinfoexport set Thinghours="+hours1+",Sickhours="+hours2+",Latetimes="+hours6+",Learlytimes="+hours7+",Absenthours="+sumhours+",Tdelayhours="+hours3+",Weekhours="+hours4+",Legalhours="+hours5+",Rannualdays="+Remaindays+" where  employname='"+objname+"' and checkyear='"+curyear+"'";
				baseJdbc.update(upsql);//执行SQL语句
				System.out.println(upsql);
			}
		}
	}
	if(action.equals("reward1"))
	{
		System.out.println("奖惩信息------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		//奖惩信息
		int nums1=0;//大功次数
		int nums2=0;//小功次数
		int nums3=0;//嘉奖次数
		...
		if(list.size()>0)
		{
			for(int i=0;i<list.size();i++)
			{
				...
				String sql2="select (select objname from selectitem where id=b.pubtype) objname,sum(nums) nums from v_hr_ydjc b where  b.jobname='"+objname+"' and b.tomonth<='"+curyear+"-10' and b.tomonth>='"+lastyear+"-11' group by b.pubtype";
				...
				String upsq2="update uf_hr_employinfoexport set largepower="+nums1+",smallpower="+nums2+",awardtimes="+nums3+",largefault="+nums4+",smallfault="+nums5+",admontimes="+nums6+",rewardbalance="+RewPunScore+" where  employname='"+objname+"' and checkyear='"+curyear+"'";
				baseJdbc.update(upsq2);//执行SQL语句
				System.out.println(upsq2);
			}
		}
	}
	if(action.equals("syscore1"))
	{
		System.out.println("系统评分------------------------------------------------------------------------------------------------------------------------------------------------------------------------");
		String checkscore="3";//考勤分数
		String SumScore="3";//系统评分
		String rpScore="3";//初始化根据奖惩规则强制后的分数
		sql="select ssmxid,employname as objname,rewardbalance from uf_hr_employinfoexport where instr('"+comtype+"',factype)>0 and checkyear='"+curyear+"'";
		list= baseJdbc.executeSqlForList(sql);
		if(list.size()>0)
		{
			for(int i=0;i<list.size();i++)
			{
				...
				String upsq3="update uf_hr_employinfoexport set attendscore="+checkscore+",repunscore="+rpScore+",systemscore="+SumScore+" where  employname='"+objname+"' and checkyear='"+curyear+"'";
