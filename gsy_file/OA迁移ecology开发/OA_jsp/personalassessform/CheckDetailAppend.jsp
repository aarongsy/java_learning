...
<%
  ...
  //查询绩效考核汇总子表中的明细
  String upsql="update uf_checkcollectchild set dirmodifyscore=score,lastscore=score where requestid='"+requestid+"'";
...
<%}else{
		if(dirmodifyscore.equals("0")||dirmodifyscore.equals("0.00")||dirmodifyscore.equals("0.0"))
		 {
			 selected1="selected";

		 }
		 else if(dirmodifyscore.equals("0.5")||dirmodifyscore.equals("0.50"))
		 {
			 selected2="selected";
		 }
...
<TD   class="td2"  align=center>
<select id="<%="select_"+k%>" style="width:120px" onchange="ChangeScore('<%="id1"+k%>','<%=employno%>','<%=k%>')" >
<option value="<%=score%>" <%=selected12%>></option>
...
<%}
%>
<TD   class="td2"  align=center><input id=<%="id2"+k%> type="text" style="width:120px;" onchange="ChangeComment('<%="id2"+k%>','<%=employno%>')" value="<%=dirmodifycomment%>"></TD><!--人事主管修改评语 -->
</tr>
<%		
	}
}else{%> 
