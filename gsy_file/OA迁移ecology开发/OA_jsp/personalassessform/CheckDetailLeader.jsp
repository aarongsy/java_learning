...
<%
  ...
  //查询绩效考核汇总子表中的明细
  String sql="select checkprocess,checkplan,director,(select objname from orgunit where id=firstdepart) as dept,employno,employname,score,dirmodifyscore,dirmodifycomment,leadermodifyscore,leadermodifycomment from uf_checkcollectchild where requestid='"+requestid+"' order by employno";
  ...
  if(sublist.size()>0){
	 for(int k=0,sizek=sublist.size();k<sizek;k++){
		 ...
		 //初始化分管领导分数即人事主管的最终评分
		 //System.out.println("dirmodifyscore"+dirmodifyscore);
		 String jxsql="update uf_checkcollectchild set initscore='"+dirmodifyscore+"',leadermodifyscore="+dirmodifyscore+",lastscore="+dirmodifyscore+" where requestid='"+requestid+"' and employno='"+employno+"'";
		 baseJdbc.update(jxsql);
%>
<TR   class="tr1"  id="dataDetail">
...
<option value="<%=dirmodifyscore%>"></option>
...
if(dirmodifyscore.equals("0"))
		...
		<TD   class="td2"  align=center>
		<select id="<%="select_"+k%>" style="width:120px" onchange="ChangeScore('<%="id1"+k%>','<%=employno%>','<%=k%>')">
		<option value="<%=dirmodifyscore%>" <%=selected12%>></option>
...
<TD   class="td2"  align=center><input id=<%="id2"+k%> type="text" style="width:120px;" onchange="ChangeComment('<%="id2"+k%>','<%=employno%>')" value="<%=leadermodifycomment%>"></TD><!--人事分管领导修改评语 -->
</tr>
<%		
	}
}else{%> 
	<TR class="tr1"><TD class="td2" colspan="11">无对应员工信息!</TD></TR>
<%} 
%>
</table>
</div>
