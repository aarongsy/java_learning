...
String sql="select (select objname from orgunit where id=a.firstdepart)as employdepart,a.employno,(select objname from humres where id=a.employname) as employname,a.intofactorydate,(select objname from selectitem where id=a.education) as education,(select nametext from uf_profe where requestid=a.currecttitle) as currecttitle,a.currectrank,a.currectdate,a.yearscore,(select nametext from uf_profe where requestid=a.recommtitle) as recommtitle,a.recommrank,a.addrank,(select nametext from uf_profe where requestid=a.hrmodifytitle) as hrmodifyposition,a.hrmodifyrank,a.addrankaftermod from uf_hr_promochild a where a.requestid='"+requestid+"' and a.checkyear='"+year+"' order by a.employno";
 ...
 if(sublist.size()>0)
 {
	 for(int i=0;i<sublist.size();i++)
	 {
		 ...
		 String Employdepart=StringHelper.null2String(map.get("employdepart"));//所属部门
			...
			if(Hrmodifytitle.equals(Recommtitle))
		 {
		  ...
		  <input type="hidden" id="<%="field_advicetitle"+i%>" name="<%="field_advicetitle"+i%>" value="" ><span id="<%="field_advicetitle" +i+"span"%>" name="<%="field_advicetitle" +i+"span"%>"></span></TD><!--人事修改升调职称-->

          <TD class="td2" align=center><input type="hidden" id="<%="field_zj"+i%>" name="<%="field_zj"+i%>" value=""><span id="<%="field_zj"+i+"span"%>" name="<%="field_zj"+i+"span"%>"></span></TD><!-- 人事修改升调职级数-->

          <TD class="td2" align=center><input type="hidden" id="<%="field_lastrank"+i%>" name="<%="field_lastrank"+i%>" value=""><span id="<%="field_lastrank"+i+"span"%>" name="<%="field_last"+i+"span"%>"></span></TD><!-- 修改后增加级数-->
		 ...
		 else
		 {
			 %>
		  <TD class="td2" align=center>
		  <input type="hidden" id="<%="field_advicetitle"+i%>" name="<%="field_advicetitle"+i%>" value="" ><span id="<%="field_advicetitle" +i+"span"%>" name="<%="field_advicetitle" +i+"span"%>"><%=Hrmodifytitle%></span></TD><!--人事修改升调职称-->

          <TD class="td2" align=center><input type="hidden" id="<%="field_zj"+i%>" name="<%="field_zj"+i%>" value=""><span id="<%="field_zj"+i+"span"%>" name="<%="field_zj"+i+"span"%>"><%=Hrmodifyrank%></span></TD><!-- 人事修改升调职级数-->
          ...
          <TD class="td2" align=center><input type="text" id="<%="field_diradvice"+i%>" name="<%="field_diradvice"+i%>" value="" onchange="HrDirectorAdvice('<%="field_diradvice"+i%>','<%=Employno%>')"><span id="<%="field_diradvice"+i+"span"%>" name="<%="field_diradvice"+i+"span"%>"></span></TD><!-- 人事主管修改建议-->
...
<%	   }
	       ...
	       <TR class="tr1">
		   <TD class="td1" colspan="16">无对应汇总明细！</TD>
		   </TR>
	<%}%>
</TABLE>
</DIV>